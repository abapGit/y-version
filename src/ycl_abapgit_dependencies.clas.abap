CLASS ycl_abapgit_dependencies DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS resolve
      CHANGING
        !ct_tadir TYPE yif_abapgit_definitions=>ty_tadir_tt
      RAISING
...
CLASS ycl_abapgit_dependencies IMPLEMENTATION.
...
* in case they have dependencies with .INCLUDE
    ...
    FIELD-SYMBOLS: <ls_tadir_ddls>      TYPE yif_abapgit_definitions=>ty_tadir,
                   <ls_dependency>      TYPE ty_dependency,
                   <ls_tadir_dependent> TYPE yif_abapgit_definitions=>ty_tadir,
    ...
    LOOP AT lt_nodes ASSIGNING <ls_node>.
      ...
      LOOP AT lt_founds ASSIGNING <ls_found>.
        ...
        CASE <ls_found>-object_cls.
          ...
          WHEN OTHERS.
            ycx_abapgit_exception=>raise( 'resolve_ddic, unknown object_cls' ).
        ENDCASE.
      ENDLOOP.
...
* build DDLS edges
  ...
  METHOD resolve_packages.
    ...
    DATA: lt_subpackages TYPE yif_abapgit_sap_package=>ty_devclass_tt.
    ...
    LOOP AT ct_tadir ASSIGNING <ls_tadir>
      ...
      lt_subpackages = ycl_abapgit_factory=>get_sap_package( |{ <ls_tadir>-obj_name }| )->list_subpackages( ).
