CLASS ycl_abapgit_objects DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_deserialization,
        obj     TYPE REF TO yif_abapgit_object,
        xml     TYPE REF TO ycl_abapgit_xml_input,
        package TYPE devclass,
        item    TYPE yif_abapgit_definitions=>ty_item,
    ...
    CLASS-METHODS deserialize
      ...
      RETURNING
        VALUE(rt_accessed_files) TYPE yif_abapgit_definitions=>ty_file_signatures_tt
      RAISING
        ycx_abapgit_exception .
    ...
    CLASS-METHODS delete
      IMPORTING
        ...
        !is_checks TYPE yif_abapgit_definitions=>ty_delete_checks OPTIONAL
      RAISING
        ycx_abapgit_exception .
    CLASS-METHODS jump
      IMPORTING
  ...
  PRIVATE SECTION.
    TYPES: BEGIN OF ty_obj_serializer_map,
             item     TYPE yif_abapgit_definitions=>ty_item,
             metadata TYPE yif_abapgit_definitions=>ty_metadata,
    ...
    CLASS-METHODS files_to_deserialize
      IMPORTING
        !io_repo          TYPE REF TO ycl_abapgit_repo
      RETURNING
        VALUE(rt_results) TYPE yif_abapgit_definitions=>ty_results_tt
      RAISING
        ycx_abapgit_exception .
    ...
    CLASS-METHODS prioritize_deser
      IMPORTING
        !it_results       TYPE yif_abapgit_definitions=>ty_results_tt
      RETURNING
        VALUE(rt_results) TYPE yif_abapgit_definitions=>ty_results_tt .
    ...
    CLASS-METHODS warning_overwrite_adjust
      IMPORTING
        !it_overwrite TYPE yif_abapgit_definitions=>ty_overwrite_tt
      CHANGING
        !ct_results   TYPE yif_abapgit_definitions=>ty_results_tt
      RAISING
        ycx_abapgit_exception .
    ...
    CLASS-METHODS warning_overwrite_find
      IMPORTING
        !it_results         TYPE yif_abapgit_definitions=>ty_results_tt
      RETURNING
        VALUE(rt_overwrite) TYPE yif_abapgit_definitions=>ty_overwrite_tt
      RAISING
        ycx_abapgit_exception .
    ...
    CLASS-METHODS compare_remote_to_local
      IMPORTING
        !ii_object TYPE REF TO yif_abapgit_object
        !it_remote TYPE yif_abapgit_definitions=>ty_files_tt
        !is_result TYPE yif_abapgit_definitions=>ty_result
      RAISING
        ycx_abapgit_exception .
    CLASS-METHODS deserialize_objects
      ...
      CHANGING
        ct_files   TYPE yif_abapgit_definitions=>ty_file_signatures_tt
      RAISING
        ycx_abapgit_exception .
    CLASS-METHODS check_objects_locked
      IMPORTING
        iv_language TYPE spras
        it_items    TYPE yif_abapgit_definitions=>ty_items_tt
      RAISING
        ycx_abapgit_exception.
    CLASS-METHODS create_object
      IMPORTING
        is_item        TYPE yif_abapgit_definitions=>ty_item
        iv_language    TYPE spras
        is_metadata    TYPE yif_abapgit_definitions=>ty_metadata OPTIONAL
        iv_native_only TYPE abap_bool DEFAULT abap_false
      RETURNING
        VALUE(ri_obj)  TYPE REF TO yif_abapgit_object
      RAISING
        ycx_abapgit_exception .
    CLASS-METHODS map_tadir_to_items
      IMPORTING
        it_tadir        TYPE yif_abapgit_definitions=>ty_tadir_tt
      RETURNING
        VALUE(rt_items) TYPE yif_abapgit_definitions=>ty_items_tt.
...
CLASS YCL_ABAPGIT_OBJECTS IMPLEMENTATION.


  METHOD changed_by.

    DATA: li_obj TYPE REF TO yif_abapgit_object.
...
* todo, fallback to looking at transports if rv_user = 'UNKNOWN'?
  ...
  METHOD check_duplicates.
    ...
    IF lines( lt_files ) <> lines( it_files ).
      ycx_abapgit_exception=>raise( 'Duplicates' ).
    ENDIF.
  ...
  METHOD check_objects_locked.
    ...
    LOOP AT it_items ASSIGNING <ls_item>.
      ...
      IF li_obj->is_locked( ) = abap_true.
        ycx_abapgit_exception=>raise( |Object { <ls_item>-obj_type } { <ls_item>-obj_name } |
...
* only the main XML file is used for comparison
    ...
    DATA: ls_remote_file       TYPE yif_abapgit_definitions=>ty_file,
          lo_remote_version    TYPE REF TO ycl_abapgit_xml_input,
          lv_count             TYPE i,
          li_comparison_result TYPE REF TO yif_abapgit_comparison_result.
    ...
    IF is_result-filename CS '.XML' AND lv_count = 2.
      ...
      IF sy-subrc = 0.
        CREATE OBJECT lo_remote_version
          EXPORTING
            iv_xml = ycl_abapgit_convert=>xstring_to_string_utf8( ls_remote_file-data ).
        ...
        IF li_comparison_result->is_result_complete_halt( ) = abap_true.
          ycx_abapgit_exception=>raise( 'Deserialization aborted by user' ).
        ENDIF.
      ENDIF.
    ENDIF.
...
*    IF ycl_abapgit_persist_settings=>get_instance( )->read( )->get_experimental_features( ) = abap_true
...
*    ENDIF.
      ...
      CATCH cx_sy_create_object_error.
        ...
        IF iv_native_only = abap_false.
          TRY. " 2nd step, try looking for plugins
              CREATE OBJECT ri_obj TYPE ycl_abapgit_objects_bridge
                EXPORTING
                  is_item = is_item.
            CATCH cx_sy_create_object_error.
              ycx_abapgit_exception=>raise( lv_message ).
          ENDTRY.
  ...
  METHOD delete.
    ...
    DATA: ls_item     TYPE yif_abapgit_definitions=>ty_item,
          ...
          lt_items    TYPE yif_abapgit_definitions=>ty_items_tt,
    ...
    IF is_checks-transport-required = abap_true.
      ycl_abapgit_default_transport=>get_instance( )->set( is_checks-transport-transport ).
    ENDIF.

    TRY.
        ycl_abapgit_dependencies=>resolve( CHANGING ct_tadir = lt_tadir ).
        ...
        check_objects_locked( iv_language = yif_abapgit_definitions=>c_english
                              it_items    = lt_items ).
...
* make sure to save object deletions
  ...
  METHOD deserialize.
    ...
    DATA: ls_item     TYPE yif_abapgit_definitions=>ty_item,
          li_obj      TYPE REF TO yif_abapgit_object,
          lt_remote   TYPE yif_abapgit_definitions=>ty_files_tt,
          lv_package  TYPE devclass,
          lo_files    TYPE REF TO ycl_abapgit_objects_files,
          lo_xml      TYPE REF TO ycl_abapgit_xml_input,
          lt_results  TYPE yif_abapgit_definitions=>ty_results_tt,
          ...
          lv_path     TYPE string,
          lt_items    TYPE yif_abapgit_definitions=>ty_items_tt.
    DATA: lo_folder_logic TYPE REF TO ycl_abapgit_folder_logic.

    FIELD-SYMBOLS: <ls_result> TYPE yif_abapgit_definitions=>ty_result,
    ...
    ycl_abapgit_objects_activation=>clear( ).
    ...
    lo_folder_logic = ycl_abapgit_folder_logic=>get_instance( ).
...
* Analyze XML in order to instantiate the proper serializer
  ...
  METHOD deserialize_checks.
    ...
    DATA: lt_results TYPE yif_abapgit_definitions=>ty_results_tt,
          li_package TYPE REF TO yif_abapgit_sap_package.
    ...
    IF lines( lt_results ) > 0.
      li_package = ycl_abapgit_factory=>get_sap_package( io_repo->get_package( ) ).
  ...
  METHOD deserialize_objects.
    ...
    DATA: lo_progress TYPE REF TO ycl_abapgit_progress.
    ...
    ycl_abapgit_objects_activation=>activate( iv_ddic ).
...
* ignore all errors and assume the object exists
  ...
  METHOD files_to_deserialize.
    ...
    rt_results = ycl_abapgit_file_status=>status( io_repo ).
    ...
    DELETE rt_results WHERE lstate = yif_abapgit_definitions=>c_state-added AND rstate IS INITIAL.
...
* handle namespaces
  ...
  METHOD has_changed_since.
    ...
    rv_changed = create_object(
      is_item     = is_item
      iv_language = yif_abapgit_definitions=>c_english )->has_changed_since( iv_timestamp ).
  ...
  METHOD jump.
    ...
    lv_adt_jump_enabled = ycl_abapgit_persist_settings=>get_instance( )->read( )->get_adt_jump_enabled( ).

    IF lv_adt_jump_enabled = abap_true.
      TRY.
          ycl_abapgit_objects_super=>jump_adt(
  ...
  METHOD map_tadir_to_items.
    ...
    DATA: ls_item LIKE LINE OF rt_items.
    FIELD-SYMBOLS: <ls_tadir> TYPE yif_abapgit_definitions=>ty_tadir.
...
* ENHS has to be handled before ENHO
  ...
  METHOD serialize.
    ...
    LOOP AT rt_files ASSIGNING <ls_file>.
      <ls_file>-sha1 = ycl_abapgit_hash=>sha1(
  ...
  METHOD supported_list.
    ...
    LOOP AT lt_objects ASSIGNING <ls_object> WHERE pgmid = 'R3TR'.
      ...
      lv_supported = ycl_abapgit_objects=>is_supported(
        is_item        = ls_item
  ...
  METHOD update_package_tree.
    ...
    DATA: lt_packages TYPE yif_abapgit_sap_package=>ty_devclass_tt,
          lv_package  LIKE LINE OF lt_packages,
          lv_tree     TYPE dirtree-tname.


    lt_packages = ycl_abapgit_factory=>get_sap_package( iv_package )->list_subpackages( ).
...
* make sure to get the current status, as something might have changed in the meanwhile
    ...
    LOOP AT lt_overwrite ASSIGNING <ls_overwrite>.
      ...
      IF sy-subrc <> 0 OR ls_overwrite-decision IS INITIAL.
        ycx_abapgit_exception=>raise( |Overwrite { <ls_overwrite>-obj_type } {
  ...
  METHOD warning_overwrite_find.
    ...
    LOOP AT it_results ASSIGNING <ls_result>
      ...
      IF <ls_result>-lstate IS NOT INITIAL
          AND <ls_result>-lstate <> yif_abapgit_definitions=>c_state-deleted
          AND NOT ( <ls_result>-lstate = yif_abapgit_definitions=>c_state-added
...
* make sure to get the current status, as something might have changed in the meanwhile
    ...
    LOOP AT lt_overwrite ASSIGNING <ls_overwrite>.
      READ TABLE it_overwrite INTO ls_overwrite WITH KEY
        ...
        obj_name = <ls_overwrite>-obj_name.
      IF sy-subrc <> 0 OR ls_overwrite-decision IS INITIAL.
        ycx_abapgit_exception=>raise( |Overwrite odd package { <ls_overwrite>-obj_type } {
          <ls_overwrite>-obj_name } undecided| ).
      ENDIF.
  ...
  METHOD warning_package_find.
    ...
    DATA: lv_package         TYPE devclass,
          lt_overwrite_uniqe TYPE HASHED TABLE OF yif_abapgit_definitions=>ty_overwrite
                                  WITH UNIQUE KEY obj_type obj_name devclass,
          ls_overwrite       LIKE LINE OF rt_overwrite,
          ls_tadir           TYPE yif_abapgit_definitions=>ty_tadir.
    ...
    LOOP AT it_results ASSIGNING <ls_result>.
      ...
      ls_tadir = ycl_abapgit_factory=>get_tadir( )->read_single(
