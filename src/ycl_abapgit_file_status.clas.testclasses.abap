...
CLASS ycl_abapgit_file_status DEFINITION LOCAL FRIENDS ltcl_run_checks.

CLASS ltcl_run_checks DEFINITION FOR TESTING RISK LEVEL HARMLESS
  DURATION SHORT FINAL.

  PRIVATE SECTION.
    DATA: mt_results TYPE yif_abapgit_definitions=>ty_results_tt,
          mo_dot     TYPE REF TO ycl_abapgit_dot_abapgit,
          mo_log     TYPE REF TO ycl_abapgit_log.

    METHODS:
      setup,
      positive FOR TESTING RAISING ycx_abapgit_exception,
      neg_diff_path_for_same_obj FOR TESTING RAISING ycx_abapgit_exception,
      neg_incorrect_path_vs_pack FOR TESTING RAISING ycx_abapgit_exception,
      neg_similar_filenames FOR TESTING RAISING ycx_abapgit_exception,
      neg_empty_filenames FOR TESTING RAISING ycx_abapgit_exception.
...
CLASS ltcl_run_checks IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    mo_dot = ycl_abapgit_dot_abapgit=>build_default( ).
...
*** 0 Positive
    ...
    ycl_abapgit_file_status=>run_checks(
      io_log     = mo_log
    ...
    METHODS:
      ...
      get_line
        IMPORTING
          iv_line        TYPE i
        RETURNING
          VALUE(rs_data) TYPE yif_abapgit_definitions=>ty_result,
      assert_lines
        IMPORTING
          iv_lines TYPE i.
...
CLASS ycl_abapgit_file_status DEFINITION LOCAL FRIENDS ltcl_status_helper.
...
CLASS ltcl_status_helper DEFINITION FOR TESTING.
  ...
  PRIVATE SECTION.
    ...
    DATA:
      ...
      mt_local  TYPE yif_abapgit_definitions=>ty_files_item_tt,
      mt_remote TYPE yif_abapgit_definitions=>ty_files_tt,
      mt_state  TYPE yif_abapgit_definitions=>ty_file_signatures_tt.
...
CLASS ltcl_status_helper IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    ycl_abapgit_injector=>set_tadir( me ).
  ...
  METHOD yif_abapgit_tadir~get_object_package.
  ...
  METHOD yif_abapgit_tadir~read.
    cl_abap_unit_assert=>fail( ).
  ENDMETHOD.

  METHOD yif_abapgit_tadir~read_single.
    cl_abap_unit_assert=>fail( ).
  ENDMETHOD.
...
* todo
  ...
  METHOD run.
    ...
    DATA: lt_results TYPE yif_abapgit_definitions=>ty_results_tt,
          lo_dot     TYPE REF TO ycl_abapgit_dot_abapgit.


    lo_dot = ycl_abapgit_dot_abapgit=>build_default( ).

    lt_results = ycl_abapgit_file_status=>calculate_status(
...
CLASS ycl_abapgit_file_status DEFINITION LOCAL FRIENDS ltcl_calculate_status.
...
CLASS ltcl_calculate_status DEFINITION FOR TESTING RISK LEVEL HARMLESS
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      setup,
      only_remote FOR TESTING RAISING ycx_abapgit_exception,
      only_local FOR TESTING RAISING ycx_abapgit_exception,
      match FOR TESTING RAISING ycx_abapgit_exception,
      diff FOR TESTING RAISING ycx_abapgit_exception,
      local_outside_main FOR TESTING RAISING ycx_abapgit_exception.
...
CLASS ltcl_calculate_status IMPLEMENTATION.
  ...
  METHOD only_remote.
    ...
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = yif_abapgit_definitions=>c_state-added ).
  ...
  METHOD diff.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = mo_result->get_line( 1 )-lstate
      exp = yif_abapgit_definitions=>c_state-modified ).
