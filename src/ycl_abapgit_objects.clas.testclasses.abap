...
CLASS ltcl_dangerous IMPLEMENTATION.
  ...
  METHOD run.
    ...
    DATA: lo_repo    TYPE REF TO ycl_abapgit_repo_online,
          lt_tadir   TYPE yif_abapgit_definitions=>ty_tadir_tt,
          lv_msg     TYPE string,
          lt_results TYPE yif_abapgit_definitions=>ty_results_tt,
          ls_checks  TYPE yif_abapgit_definitions=>ty_deserialize_checks,
          lt_types   TYPE ycl_abapgit_objects=>ty_types_tt.
    ...
    ycl_abapgit_factory=>get_sap_package( c_package )->create_local( ).

    lt_types = ycl_abapgit_objects=>supported_list( ).
    ...
    lt_tadir = ycl_abapgit_factory=>get_tadir( )->read( c_package ).
...
CLASS ltcl_object_types DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  ...
  PRIVATE SECTION.
    METHODS:
      not_exist FOR TESTING RAISING ycx_abapgit_exception.
...
CLASS ltcl_object_types IMPLEMENTATION.
  ...
  METHOD not_exist.
    ...
    LOOP AT lt_types ASSIGNING <lv_type>.
      CLEAR ls_item.
      ls_item-obj_name = 'YABAPGIT_FOOBAR'.
      ls_item-obj_type = <lv_type>.
      lv_exists = ycl_abapgit_objects=>exists( ls_item ).
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      check
        ...
        RAISING   ycx_abapgit_exception,
      serialize_tabl FOR TESTING RAISING ycx_abapgit_exception,
      serialize_shlp FOR TESTING RAISING ycx_abapgit_exception,
      serialize_view FOR TESTING RAISING ycx_abapgit_exception,
      serialize_auth FOR TESTING RAISING ycx_abapgit_exception,
      serialize_clas FOR TESTING RAISING ycx_abapgit_exception,
      serialize_doma FOR TESTING RAISING ycx_abapgit_exception,
      serialize_dtel FOR TESTING RAISING ycx_abapgit_exception,
      serialize_fugr FOR TESTING RAISING ycx_abapgit_exception,
      serialize_msag FOR TESTING RAISING ycx_abapgit_exception,
      serialize_prog FOR TESTING RAISING ycx_abapgit_exception,
      serialize_tran FOR TESTING RAISING ycx_abapgit_exception,
      serialize_ttyp FOR TESTING RAISING ycx_abapgit_exception.
...
CLASS ltcl_serialize IMPLEMENTATION.
  ...
  METHOD check.
    ...
    lt_files = ycl_abapgit_objects=>serialize( is_item     = is_item
...
CLASS ltcl_objcet_ddls_mock IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~changed_by. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_object~compare_to_remote_version. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_object~delete. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_object~deserialize. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_object~exists. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_object~get_metadata. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_object~has_changed_since. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_object~jump. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_object~serialize. "##needed
...
CLASS ltcl_check_objects_locked DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    DATA:
      mt_given_items    TYPE yif_abapgit_definitions=>ty_items_tt,
...
CLASS ycl_abapgit_objects DEFINITION LOCAL FRIENDS ltcl_check_objects_locked.
...
CLASS ltcl_check_objects_locked IMPLEMENTATION.
  ...
  METHOD when_check_objects_locked.
    ...
    DATA: lx_error TYPE REF TO ycx_abapgit_exception.

    TRY.
        ycl_abapgit_objects=>check_objects_locked( iv_language = 'E'
  ...
  METHOD given_object.
    ...
    DATA:
      ...
      ls_obj_serializer_map LIKE LINE OF ycl_abapgit_objects=>gt_obj_serializer_map.
    ...
    ls_obj_serializer_map-metadata-class = '\CLASS-POOL=YCL_ABAPGIT_OBJECTS\CLASS=LTCL_OBJCET_DDLS_MOCK'.
    INSERT ls_obj_serializer_map INTO TABLE ycl_abapgit_objects=>gt_obj_serializer_map.
