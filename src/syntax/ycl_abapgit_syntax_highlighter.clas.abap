CLASS ycl_abapgit_syntax_highlighter DEFINITION
  PUBLIC
  ABSTRACT
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create
      ...
      RETURNING
        VALUE(ro_instance) TYPE REF TO ycl_abapgit_syntax_highlighter .
...
CLASS ycl_abapgit_syntax_highlighter IMPLEMENTATION.
  ...
  METHOD create.
    ...
    IF iv_filename CP '*.abap'.
      CREATE OBJECT ro_instance TYPE ycl_abapgit_syntax_abap.
    ELSEIF iv_filename CP '*.xml'.
      CREATE OBJECT ro_instance TYPE ycl_abapgit_syntax_xml.
    ELSE.
      CLEAR ro_instance.
    ENDIF.
