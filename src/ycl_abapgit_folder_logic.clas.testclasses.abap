...
CLASS ltcl_folder_logic_helper IMPLEMENTATION.
  ...
  METHOD test.
    ...
    lv_package = ycl_abapgit_folder_logic=>get_instance( )->path_to_package(
      iv_top  = iv_top
      io_dot  = lo_dot
      iv_path = iv_path ).

    lv_path = ycl_abapgit_folder_logic=>get_instance( )->package_to_path(
      iv_top     = iv_top
      io_dot     = lo_dot
...
CLASS ltcl_folder_logic DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      setup,
      prefix1 FOR TESTING RAISING ycx_abapgit_exception,
      prefix2 FOR TESTING RAISING ycx_abapgit_exception,
      prefix_error1 FOR TESTING RAISING ycx_abapgit_exception,
      full1 FOR TESTING RAISING ycx_abapgit_exception,
      full2 FOR TESTING RAISING ycx_abapgit_exception.
...
CLASS ltcl_folder_logic IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    ycl_abapgit_injector=>set_sap_package( iv_package     = '$TOP'
                                           ii_sap_package = me ).

    ycl_abapgit_injector=>set_sap_package( iv_package     = '$TOP_FOO'
                                           ii_sap_package = me ).

    ycl_abapgit_injector=>set_sap_package( iv_package     = '$FOOBAR'
                                           ii_sap_package = me ).
  ...
  METHOD prefix1.
    ltcl_folder_logic_helper=>test(
      iv_starting = c_src
      iv_top      = c_top
      iv_logic    = yif_abapgit_dot_abapgit=>c_folder_logic-prefix
      iv_package  = c_top
      iv_path     = c_src ).
  ENDMETHOD.
...
* PREFIX mode, top package is $TOP, so all subpackages should be named $TOP_something
  ...
  METHOD full1.
    ltcl_folder_logic_helper=>test(
      iv_starting = c_src
      iv_top      = c_top
      iv_logic    = yif_abapgit_dot_abapgit=>c_folder_logic-full
      iv_package  = c_top
      iv_path     = c_src ).
  ENDMETHOD.
...
CLASS ltcl_folder_logic_namespaces IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    ycl_abapgit_injector=>set_sap_package( iv_package     = '/TEST/TOOLS'
                                           ii_sap_package = me ).

    ycl_abapgit_injector=>set_sap_package( iv_package     = '/TEST/T1'
                                           ii_sap_package = me ).

    ycl_abapgit_injector=>set_sap_package( iv_package     = '/TEST/TOOLS_T1'
                                           ii_sap_package = me ).
