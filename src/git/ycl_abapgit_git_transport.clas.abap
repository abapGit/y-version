CLASS ycl_abapgit_git_transport DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
* remote to local
    CLASS-METHODS upload_pack
      IMPORTING iv_url         TYPE string
                ...
                it_branches    TYPE yif_abapgit_definitions=>ty_git_branch_list_tt OPTIONAL
      EXPORTING et_objects     TYPE yif_abapgit_definitions=>ty_objects_tt
                ev_branch      TYPE yif_abapgit_definitions=>ty_sha1
                eo_branch_list TYPE REF TO ycl_abapgit_git_branch_list
      RAISING   ycx_abapgit_exception.

* local to remote
    CLASS-METHODS receive_pack
      IMPORTING iv_url         TYPE string
                iv_old         TYPE yif_abapgit_definitions=>ty_sha1
                iv_new         TYPE yif_abapgit_definitions=>ty_sha1
                iv_branch_name TYPE string
    ...
    CLASS-METHODS branches
      IMPORTING iv_url                TYPE string
      RETURNING VALUE(ro_branch_list) TYPE REF TO ycl_abapgit_git_branch_list
      RAISING   ycx_abapgit_exception.
  PRIVATE SECTION.
    ...
    CLASS-METHODS branch_list
      ...
      EXPORTING eo_client      TYPE REF TO ycl_abapgit_http_client
                eo_branch_list TYPE REF TO ycl_abapgit_git_branch_list
      RAISING   ycx_abapgit_exception.
...
CLASS ycl_abapgit_git_transport IMPLEMENTATION.


  METHOD branches.

    DATA: lo_client TYPE REF TO ycl_abapgit_http_client.
  ...
  METHOD branch_list.
    ...
    eo_client = ycl_abapgit_http=>create_by_url(
      iv_url     = iv_url
  ...
  METHOD parse.
    ...
    WHILE xstrlen( cv_data ) >= 4.
      lv_len = ycl_abapgit_git_utils=>length_utf8_hex( cv_data ).

      IF lv_len > xstrlen( cv_data ).
        ycx_abapgit_exception=>raise( 'parse, string length too large' ).
      ENDIF.
  ...
  METHOD receive_pack.
    ...
    lv_line = iv_old &&
              ...
              ` ` &&
              iv_branch_name &&
              ycl_abapgit_git_utils=>get_null( ) &&
              ` ` &&
              lv_cap_list &&
              yif_abapgit_definitions=>c_newline.           "#EC NOTEXT
    lv_cmd_pkt = ycl_abapgit_git_utils=>pkt_string( lv_line ).

    lv_buffer = lv_cmd_pkt && '0000'.
    lv_tmp = ycl_abapgit_convert=>string_to_xstring_utf8( lv_buffer ).
...
* parse the reply and look for the "ng" not good indicator
    ...
    IF NOT lv_string CP '*unpack ok*'.
      ycx_abapgit_exception=>raise( 'unpack not ok' ).
    ELSEIF lv_string CP '*pre-receive hook declined*'.
      ycx_abapgit_exception=>raise( 'pre-receive hook declined' ).
    ELSEIF lv_string CP '*push declined due to email privacy*'.
      ycx_abapgit_exception=>raise( 'push declined due to email privacy' ).
    ELSEIF lv_string CP '*funny refname*'.
      ycx_abapgit_exception=>raise( 'funny refname' ).
    ELSEIF lv_string CP '*failed to update ref*'.
      ycx_abapgit_exception=>raise( 'failed to update ref' ).
    ELSEIF lv_string CP '*missing necessary objects*'.
      ycx_abapgit_exception=>raise( 'missing necessary objects' ).
    ENDIF.
    ...
    LOOP AT lt_branches FROM 1 ASSIGNING <ls_branch>.
      IF sy-tabix = 1.
        ...
        lv_line = 'want' && ` ` && <ls_branch>-sha1
          && ` ` && lv_capa && yif_abapgit_definitions=>c_newline. "#EC NOTEXT
      ELSE.
        lv_line = 'want' && ` ` && <ls_branch>-sha1
          && yif_abapgit_definitions=>c_newline.            "#EC NOTEXT
      ENDIF.
      lv_buffer = lv_buffer && ycl_abapgit_git_utils=>pkt_string( lv_line ).
    ENDLOOP.

    IF iv_deepen = abap_true.
      lv_buffer = lv_buffer && ycl_abapgit_git_utils=>pkt_string( 'deepen 1'
        && yif_abapgit_definitions=>c_newline ).            "#EC NOTEXT
    ENDIF.

    lv_buffer = lv_buffer
             && '0000'
             && '0009done' && yif_abapgit_definitions=>c_newline.

    lv_xstring = lo_client->send_receive_close(
      ycl_abapgit_convert=>string_to_xstring_utf8( lv_buffer ) ).

    parse( IMPORTING ev_pack = lv_pack
           CHANGING cv_data = lv_xstring ).

    IF lv_pack IS INITIAL.
      ycx_abapgit_exception=>raise( 'empty pack' ).
    ENDIF.

    et_objects = ycl_abapgit_git_pack=>decode( lv_pack ).
