CLASS ycl_abapgit_git_pack DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_node,
        chmod TYPE yif_abapgit_definitions=>ty_chmod,
        name  TYPE string,
    ...
    TYPES:
      BEGIN OF ty_commit,
        tree      TYPE yif_abapgit_definitions=>ty_sha1,
        parent    TYPE yif_abapgit_definitions=>ty_sha1,
        parent2   TYPE yif_abapgit_definitions=>ty_sha1,
        author    TYPE string,
    ...
    CLASS-METHODS encode_tag
      IMPORTING
        !is_tag        TYPE ycl_abapgit_git_pack=>ty_tag
      RETURNING
        VALUE(rv_data) TYPE xstring
      RAISING
        ycx_abapgit_exception .
  PRIVATE SECTION.
    ...
    CLASS-METHODS decode_deltas
      CHANGING
        !ct_objects TYPE yif_abapgit_definitions=>ty_objects_tt
      RAISING
        ycx_abapgit_exception .
    CLASS-METHODS delta
      IMPORTING
        !is_object  TYPE yif_abapgit_definitions=>ty_object
      CHANGING
        !ct_objects TYPE yif_abapgit_definitions=>ty_objects_tt
      RAISING
        ycx_abapgit_exception .
    ...
    CLASS-METHODS get_type
      IMPORTING
        !iv_x          TYPE x
      RETURNING
        VALUE(rv_type) TYPE yif_abapgit_definitions=>ty_type
      RAISING
        ycx_abapgit_exception .
    ...
    CLASS-METHODS type_and_length
      IMPORTING
        !iv_type          TYPE yif_abapgit_definitions=>ty_type
        !iv_length        TYPE i
      RETURNING
...
CLASS YCL_ABAPGIT_GIT_PACK IMPLEMENTATION.
  ...
  METHOD decode.
    ...
    DATA: lv_x              TYPE x,
          ...
          lv_ref_delta      TYPE yif_abapgit_definitions=>ty_sha1,
...
* header
    IF NOT xstrlen( lv_data ) > 4 OR lv_data(4) <> c_pack_start.
      ycx_abapgit_exception=>raise( 'Unexpected pack header' ).
    ENDIF.
    lv_data = lv_data+4.

* version
    IF lv_data(4) <> c_version.
      ycx_abapgit_exception=>raise( 'Version not supported' ).
    ENDIF.
    lv_data = lv_data+4.

* number of objects
    lv_xstring = lv_data(4).
    lv_objects = ycl_abapgit_convert=>xstring_to_int( lv_xstring ).
    lv_data = lv_data+4.
    ...
    DO lv_objects TIMES.
      ...
      IF lv_type = yif_abapgit_definitions=>c_type-ref_d.
...
* strip header, '789C', CMF + FLG
      ...
      IF lv_zlib <> c_zlib AND lv_zlib <> c_zlib_hmm.
        ycx_abapgit_exception=>raise( 'Unexpected zlib header' ).
      ENDIF.
      lv_data = lv_data+2.
...
*******************************
      ...
      IF lv_zlib = c_zlib.
        ...
        IF lv_expected <> lv_decompress_len.
          ycx_abapgit_exception=>raise( 'Decompression falied' ).
        ENDIF.
...
* check SHA1 at end of pack
    ...
    lv_sha1 = ycl_abapgit_hash=>sha1_raw( lv_xstring ).
    IF to_upper( lv_sha1 ) <> lv_data.
      ycx_abapgit_exception=>raise( 'SHA1 at end of pack doesnt match' ).
    ENDIF.
  ...
  METHOD decode_commit.
    ...
    lv_string = ycl_abapgit_convert=>xstring_to_string_utf8( iv_data ).

    SPLIT lv_string AT yif_abapgit_definitions=>c_newline INTO TABLE lt_string.
    ...
    IF rs_commit-author IS INITIAL
      ...
      ycx_abapgit_exception=>raise( 'multiple parents? not supported' ).
    ENDIF.
  ...
  METHOD decode_deltas.
    ...
    LOOP AT ct_objects INTO ls_object
      USING KEY type
      WHERE type = yif_abapgit_definitions=>c_type-ref_d.
        ...
        WHEN OTHERS.
          ...
          ELSE.
            ...
            IF rs_tag-body IS NOT INITIAL.
              rs_tag-body = rs_tag-body && yif_abapgit_definitions=>c_newline.
            ENDIF.
  ...
  METHOD decode_tree.
    ...
    DATA: lv_xstring TYPE xstring,
          lv_chmod   TYPE yif_abapgit_definitions=>ty_chmod,
          lv_name    TYPE string,
    ...
    DO.
      ...
      lv_string = ycl_abapgit_convert=>xstring_to_string_utf8( lv_xstring ).
      SPLIT lv_string AT space INTO lv_chmod lv_name.

      CLEAR ls_node.
      ls_node-chmod = lv_chmod.
      IF ls_node-chmod <> yif_abapgit_definitions=>c_chmod-dir
          AND ls_node-chmod <> yif_abapgit_definitions=>c_chmod-file
          AND ls_node-chmod <> yif_abapgit_definitions=>c_chmod-executable.
        ycx_abapgit_exception=>raise( 'Unknown chmod' ).
      ENDIF.
...
* find base
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Base not found, { is_object-sha1 }| ).
    ELSEIF <ls_object>-type = yif_abapgit_definitions=>c_type-ref_d.
* sanity check
      ycx_abapgit_exception=>raise( 'Delta, base eq delta' ).
    ENDIF.
...
* insert from delta
    ...
    lv_sha1 = ycl_abapgit_hash=>sha1( iv_type = <ls_object>-type iv_data = lv_result ).
  ...
  METHOD delta_header.
    ...
    DATA: lv_bitbyte TYPE yif_abapgit_definitions=>ty_bitbyte,
          lv_bits    TYPE string,
          lv_x       TYPE x.
    ...
    DO.
      lv_x = cv_delta(1).
      cv_delta = cv_delta+1.
      lv_bitbyte = ycl_abapgit_convert=>x_to_bitbyte( lv_x ).
      CONCATENATE lv_bitbyte+1 lv_bits INTO lv_bits.
      IF lv_bitbyte(1) = '0'.
        EXIT. " current loop
      ENDIF.
    ENDDO.
    ev_header = ycl_abapgit_convert=>bitbyte_to_int( lv_bits ).
  ...
  METHOD encode.
    ...
    DATA: lv_sha1          TYPE x LENGTH 20,
          lv_adler32       TYPE yif_abapgit_definitions=>ty_adler32,
          lv_compressed    TYPE xstring,
          lv_xstring       TYPE xstring,
          lo_progress      TYPE REF TO ycl_abapgit_progress,
    ...
    lv_xstring = ycl_abapgit_convert=>int_to_xstring4( lines( it_objects ) ).
    ...
    LOOP AT it_objects ASSIGNING <ls_object>.
      ...
      ELSE.
        lv_adler32 = ycl_abapgit_hash=>adler32( <ls_object>-data ).
      ENDIF.
      CONCATENATE rv_data lv_adler32 INTO rv_data IN BYTE MODE.

    ENDLOOP.

    lv_sha1 = to_upper( ycl_abapgit_hash=>sha1_raw( rv_data ) ).
  ...
  METHOD encode_commit.
    ...
    CONCATENATE lv_string lv_tmp yif_abapgit_definitions=>c_newline INTO lv_string.
    ...
    CONCATENATE lv_string yif_abapgit_definitions=>c_newline is_commit-body INTO lv_string.

    rv_data = ycl_abapgit_convert=>string_to_xstring_utf8( lv_string ).
  ...
  METHOD encode_tag.
    ...
    DATA: lv_string TYPE string,
          lv_time   TYPE ycl_abapgit_time=>ty_unixtime.

    lv_time = ycl_abapgit_time=>get( ).

    lv_string = |object { is_tag-object }{ yif_abapgit_definitions=>c_newline }|
             && |type { is_tag-type }{ yif_abapgit_definitions=>c_newline }|
             && |tag { ycl_abapgit_tag=>remove_tag_prefix( is_tag-tag ) }{ yif_abapgit_definitions=>c_newline }|
  ...
  METHOD encode_tree.
    ...
    LOOP AT lt_nodes ASSIGNING <ls_node>.
      ...
      lv_xstring = ycl_abapgit_convert=>string_to_xstring_utf8( lv_string ).
  ...
  METHOD get_length.
    ...
    DATA: lv_x           TYPE x,
          ...
          lv_bitbyte     TYPE yif_abapgit_definitions=>ty_bitbyte.
    ...
    ev_length = ycl_abapgit_convert=>bitbyte_to_int( lv_length_bits ).
  ...
  METHOD get_type.
    ...
    CASE lv_xtype.
      WHEN 16.
        rv_type = yif_abapgit_definitions=>c_type-commit.
      WHEN 32.
        rv_type = yif_abapgit_definitions=>c_type-tree.
      WHEN 48.
        rv_type = yif_abapgit_definitions=>c_type-blob.
      WHEN 64.
        rv_type = yif_abapgit_definitions=>c_type-tag.
      WHEN 112.
        rv_type = yif_abapgit_definitions=>c_type-ref_d.
      WHEN OTHERS.
        ycx_abapgit_exception=>raise( 'Todo, unknown type' ).
    ENDCASE.
...
* see http://stefan.saasen.me/articles/git-clone-in-haskell-from-the-bottom-up/#pack_file_objects
    ...
    CASE iv_type.
      WHEN yif_abapgit_definitions=>c_type-commit.
        lv_type = 16.
      WHEN yif_abapgit_definitions=>c_type-tree.
        lv_type = 32.
      WHEN yif_abapgit_definitions=>c_type-blob.
        lv_type = 48.
      WHEN yif_abapgit_definitions=>c_type-tag.
        lv_type = 64.
      WHEN yif_abapgit_definitions=>c_type-ref_d.
        lv_type = 112.
      WHEN OTHERS.
        ycx_abapgit_exception=>raise( 'Unexpected object type while encoding pack' ).
    ENDCASE.
...
* last byte
  ...
  METHOD zlib_decompress.
    ...
    DATA: ls_data           TYPE ycl_abapgit_zlib=>ty_decompress,
          lv_compressed_len TYPE i,
          lv_adler32        TYPE yif_abapgit_definitions=>ty_adler32.


    ls_data = ycl_abapgit_zlib=>decompress( cv_data ).
    lv_compressed_len = ls_data-compressed_len.
    cv_decompressed = ls_data-raw.

    IF lv_compressed_len IS INITIAL.
      ycx_abapgit_exception=>raise( 'Decompression falied :o/' ).
    ENDIF.

    cv_data = cv_data+lv_compressed_len.

    lv_adler32 = ycl_abapgit_hash=>adler32( cv_decompressed ).
    ...
    ENDIF.
    IF cv_data(4) <> lv_adler32.
      ycx_abapgit_exception=>raise( 'Wrong Adler checksum' ).
    ENDIF.
  ENDMETHOD.
ENDCLASS.
