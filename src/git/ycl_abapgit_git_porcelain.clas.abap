CLASS ycl_abapgit_git_porcelain DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_pull_result,
        files   TYPE yif_abapgit_definitions=>ty_files_tt,
        objects TYPE yif_abapgit_definitions=>ty_objects_tt,
        branch  TYPE yif_abapgit_definitions=>ty_sha1,
      END OF ty_pull_result .
    TYPES:
      BEGIN OF ty_push_result,
        new_files     TYPE yif_abapgit_definitions=>ty_files_tt,
        branch        TYPE yif_abapgit_definitions=>ty_sha1,
        updated_files TYPE yif_abapgit_definitions=>ty_file_signatures_tt,
        new_objects   TYPE yif_abapgit_definitions=>ty_objects_tt,
    ...
    CLASS-METHODS push
      IMPORTING
        ...
        !it_old_objects  TYPE yif_abapgit_definitions=>ty_objects_tt
        !iv_parent       TYPE yif_abapgit_definitions=>ty_sha1
        !iv_url          TYPE string
        !iv_branch_name  TYPE string
      RETURNING
    ...
    CLASS-METHODS create_branch
      IMPORTING
        !iv_url  TYPE string
        !iv_name TYPE string
        !iv_from TYPE yif_abapgit_definitions=>ty_sha1
      RAISING
        ycx_abapgit_exception .
    CLASS-METHODS create_tag
      IMPORTING
        !iv_url TYPE string
        !is_tag TYPE yif_abapgit_definitions=>ty_git_tag
      RAISING
        ycx_abapgit_exception .
    CLASS-METHODS delete_branch
      IMPORTING
        !iv_url    TYPE string
        !is_branch TYPE yif_abapgit_definitions=>ty_git_branch
      RAISING
        ycx_abapgit_exception .
    ...
    CLASS-METHODS full_tree
      IMPORTING
        ...
        !iv_branch         TYPE yif_abapgit_definitions=>ty_sha1
      RETURNING
        VALUE(rt_expanded) TYPE yif_abapgit_definitions=>ty_expanded_tt
      RAISING
        ycx_abapgit_exception .
  PRIVATE SECTION.
    ...
    TYPES:
      BEGIN OF ty_tree,
        path TYPE string,
        data TYPE xstring,
        sha1 TYPE yif_abapgit_definitions=>ty_sha1,
      END OF ty_tree .
    TYPES:
    ...
    CONSTANTS c_zero TYPE yif_abapgit_definitions=>ty_sha1 VALUE '0000000000000000000000000000000000000000' ##NO_TEXT.

    CLASS-METHODS build_trees
      IMPORTING
        !it_expanded    TYPE yif_abapgit_definitions=>ty_expanded_tt
      RETURNING
    ...
    CLASS-METHODS walk
      IMPORTING
        !it_objects TYPE yif_abapgit_definitions=>ty_objects_tt
        !iv_sha1    TYPE yif_abapgit_definitions=>ty_sha1
        !iv_path    TYPE string
      CHANGING
        !ct_files   TYPE yif_abapgit_definitions=>ty_files_tt
      RAISING
        ycx_abapgit_exception .
    CLASS-METHODS walk_tree
      IMPORTING
        !it_objects        TYPE yif_abapgit_definitions=>ty_objects_tt
        !iv_tree           TYPE yif_abapgit_definitions=>ty_sha1
        !iv_base           TYPE string
      RETURNING
        VALUE(rt_expanded) TYPE yif_abapgit_definitions=>ty_expanded_tt
      RAISING
    ...
    CLASS-METHODS receive_pack_push
      IMPORTING
        ...
        !it_blobs       TYPE yif_abapgit_definitions=>ty_files_tt
        !iv_parent      TYPE yif_abapgit_definitions=>ty_sha1
        !iv_parent2     TYPE yif_abapgit_definitions=>ty_sha1 OPTIONAL
        !iv_url         TYPE string
        !iv_branch_name TYPE string
      EXPORTING
        !ev_new_commit  TYPE yif_abapgit_definitions=>ty_sha1
        !et_new_objects TYPE yif_abapgit_definitions=>ty_objects_tt
        !ev_new_tree    TYPE yif_abapgit_definitions=>ty_sha1
      RAISING
        ycx_abapgit_exception .
...
CLASS ycl_abapgit_git_porcelain IMPLEMENTATION.


  METHOD build_trees.

    DATA: lt_nodes   TYPE ycl_abapgit_git_pack=>ty_nodes_tt,
...
* folders
      LOOP AT lt_folders ASSIGNING <ls_sub> WHERE count = <ls_folder>-count + 1.
        ...
        IF strlen( <ls_sub>-path ) > lv_len AND <ls_sub>-path(lv_len) = <ls_folder>-path.
          APPEND INITIAL LINE TO lt_nodes ASSIGNING <ls_node>.
          <ls_node>-chmod = yif_abapgit_definitions=>c_chmod-dir.
...
* extract folder name, this can probably be done easier using regular expressions
      ...
      ls_tree-data = ycl_abapgit_git_pack=>encode_tree( lt_nodes ).
      ls_tree-sha1 = ycl_abapgit_hash=>sha1( iv_type = yif_abapgit_definitions=>c_type-tree iv_data = ls_tree-data ).
  ...
  METHOD create_branch.
    ...
    IF iv_name CS ` `.
      ycx_abapgit_exception=>raise( 'Branch name cannot contain blank spaces' ).
    ENDIF.

* "client MUST send an empty packfile"
* https://github.com/git/git/blob/master/Documentation/technical/pack-protocol.txt#L514
    lv_pack = ycl_abapgit_git_pack=>encode( lt_objects ).

    ycl_abapgit_git_transport=>receive_pack(
      iv_url         = iv_url
      iv_old         = c_zero
      iv_new         = iv_from
...
* https://github.com/git/git/blob/master/Documentation/technical/pack-protocol.txt#L514
  ...
  METHOD create_tag.
    ...
    IF is_tag-name CS ` `.
      ycx_abapgit_exception=>raise( 'Tag name cannot contain blank spaces' ).
    ENDIF.

    CASE is_tag-type.
      WHEN yif_abapgit_definitions=>c_git_branch_type-annotated_tag.

        create_annotated_tag(
          is_tag = is_tag
          iv_url = iv_url ).

      WHEN yif_abapgit_definitions=>c_git_branch_type-lightweight_tag.
      ...
      WHEN OTHERS.
        ...
        ycx_abapgit_exception=>raise( |Invalid tag type: { is_tag-type }| ).
...
* add empty folders
  ...
  METHOD full_tree.
    ...
    DATA: ls_object LIKE LINE OF it_objects,
          ls_commit TYPE ycl_abapgit_git_pack=>ty_commit.


    READ TABLE it_objects INTO ls_object
      WITH KEY type COMPONENTS
        type = yif_abapgit_definitions=>c_type-commit
        sha1 = iv_branch .
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'commit not found' ).
    ENDIF.
    ls_commit = ycl_abapgit_git_pack=>decode_commit( ls_object-data ).
  ...
  METHOD pull.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'Commit/branch not found' ).
    ENDIF.
    ls_commit = ycl_abapgit_git_pack=>decode_commit( ls_object-data ).
  ...
  METHOD push.
    ...
    DATA: lt_expanded TYPE yif_abapgit_definitions=>ty_expanded_tt,
          lt_blobs    TYPE yif_abapgit_definitions=>ty_files_tt,
          lv_sha1     TYPE yif_abapgit_definitions=>ty_sha1,
          lv_new_tree TYPE yif_abapgit_definitions=>ty_sha1,
          lt_trees    TYPE ty_trees_tt,
          lt_stage    TYPE ycl_abapgit_stage=>ty_stage_tt.
    ...
    LOOP AT lt_stage ASSIGNING <ls_stage>.
      ...
      CASE <ls_stage>-method.
        WHEN ycl_abapgit_stage=>c_method-add.
          ...
          IF sy-subrc <> 0. " new files
            ...
            <ls_exp>-chmod = yif_abapgit_definitions=>c_chmod-file.
          ENDIF.

          lv_sha1 = ycl_abapgit_hash=>sha1( iv_type = yif_abapgit_definitions=>c_type-blob
        ...
        WHEN ycl_abapgit_stage=>c_method-rm.
          DELETE lt_expanded
        ...
        WHEN OTHERS.
          ycx_abapgit_exception=>raise( 'stage method not supported, todo' ).
      ENDCASE.
    ENDLOOP.
  ...
  METHOD receive_pack_create_tag.
    ...
    DATA: lv_tag          TYPE xstring,
          lt_objects      TYPE yif_abapgit_definitions=>ty_objects_tt,
          lv_pack         TYPE xstring,
          ls_object       LIKE LINE OF lt_objects,
          ls_tag          TYPE ycl_abapgit_git_pack=>ty_tag,
          lv_new_tag_sha1 TYPE yif_abapgit_definitions=>ty_sha1.

* new tag
    ls_tag-object       = is_tag-sha1.
    ls_tag-type         = yif_abapgit_definitions=>c_type-commit.
    ...
    lv_tag = ycl_abapgit_git_pack=>encode_tag( ls_tag ).

    lv_new_tag_sha1 = ycl_abapgit_hash=>sha1(
      iv_type = yif_abapgit_definitions=>c_type-tag
      iv_data = lv_tag ).

    CLEAR ls_object.
    ls_object-sha1 = lv_new_tag_sha1.
    ls_object-type = yif_abapgit_definitions=>c_type-tag.
  ...
  METHOD receive_pack_push.
    ...
    DATA: lv_time   TYPE ycl_abapgit_time=>ty_unixtime,
...
* new commit
    ...
    lv_commit = ycl_abapgit_git_pack=>encode_commit( ls_commit ).

    CLEAR ls_object.
    ls_object-sha1 = ycl_abapgit_hash=>sha1( iv_type = yif_abapgit_definitions=>c_type-commit iv_data = lv_commit ).
    ls_object-type = yif_abapgit_definitions=>c_type-commit.
    ...
    LOOP AT it_trees ASSIGNING <ls_tree>.
      ...
      READ TABLE et_new_objects
        WITH KEY type COMPONENTS
          type = yif_abapgit_definitions=>c_type-tree
          sha1 = ls_object-sha1
...
* two identical trees added at the same time, only add one to the pack
      ...
      ls_object-type = yif_abapgit_definitions=>c_type-tree.
    ...
    LOOP AT it_blobs ASSIGNING <ls_blob>.
      CLEAR ls_object.
      ls_object-sha1 = ycl_abapgit_hash=>sha1(
        iv_type = yif_abapgit_definitions=>c_type-blob
        iv_data = <ls_blob>-data ).

      READ TABLE et_new_objects
        WITH KEY type COMPONENTS
          type = yif_abapgit_definitions=>c_type-blob
          sha1 = ls_object-sha1
        TRANSPORTING NO FIELDS.
      IF sy-subrc = 0.
* two identical files added at the same time, only add one blob to the pack
      ...
      ls_object-type = yif_abapgit_definitions=>c_type-blob.
*       ASSERT NOT <ls_blob>-data IS INITIAL. "#1857 allow empty files - some more checks needed?
    ...
    lv_pack = ycl_abapgit_git_pack=>encode( et_new_objects ).

    ev_new_commit = ycl_abapgit_hash=>sha1(
  ...
  METHOD walk.
    ...
    DATA: lv_path  TYPE string,
          ...
          lt_nodes TYPE ycl_abapgit_git_pack=>ty_nodes_tt.
    ...
    READ TABLE it_objects ASSIGNING <ls_tree>
      WITH KEY type COMPONENTS
        ...
        sha1 = iv_sha1.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'Walk, tree not found' ).
    ENDIF.

    lt_nodes = ycl_abapgit_git_pack=>decode_tree( <ls_tree>-data ).

    LOOP AT lt_nodes ASSIGNING <ls_node>.
      IF <ls_node>-chmod = yif_abapgit_definitions=>c_chmod-file.
        READ TABLE it_objects ASSIGNING <ls_blob>
          WITH KEY type COMPONENTS
            type = yif_abapgit_definitions=>c_type-blob
            sha1 = <ls_node>-sha1.
        IF sy-subrc <> 0.
          ycx_abapgit_exception=>raise( 'Walk, blob not found' ).
        ENDIF.
    ...
    LOOP AT lt_nodes ASSIGNING <ls_node> WHERE chmod = yif_abapgit_definitions=>c_chmod-dir.
  ...
  METHOD walk_tree.
    ...
    READ TABLE it_objects INTO ls_object
      WITH KEY type COMPONENTS
        ...
        sha1 = iv_tree.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'tree not found' ).
    ENDIF.
    lt_nodes = ycl_abapgit_git_pack=>decode_tree( ls_object-data ).

    LOOP AT lt_nodes ASSIGNING <ls_node>.
      CASE <ls_node>-chmod.
        WHEN yif_abapgit_definitions=>c_chmod-file
            OR yif_abapgit_definitions=>c_chmod-executable.
        ...
        WHEN yif_abapgit_definitions=>c_chmod-dir.
        ...
        WHEN OTHERS.
          ycx_abapgit_exception=>raise( 'walk_tree: unknown chmod' ).
      ENDCASE.
    ENDLOOP.
