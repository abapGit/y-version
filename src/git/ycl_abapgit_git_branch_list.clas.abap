CLASS ycl_abapgit_git_branch_list DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS find_by_name
      ...
      RETURNING
        VALUE(rs_branch) TYPE yif_abapgit_definitions=>ty_git_branch
      RAISING
        ycx_abapgit_exception .
    ...
    METHODS get_branches_only
      RETURNING
        VALUE(rt_branches) TYPE yif_abapgit_definitions=>ty_git_branch_list_tt
      RAISING
        ycx_abapgit_exception .
    METHODS get_tags_only     " For potential future use
      RETURNING
        VALUE(rt_tags) TYPE yif_abapgit_definitions=>ty_git_branch_list_tt
      RAISING
        ycx_abapgit_exception .
    ...
    CLASS-METHODS get_type
      ...
      RETURNING
        VALUE(rv_type)        TYPE yif_abapgit_definitions=>ty_git_branch_type .
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS parse_branch_list
      IMPORTING
        !iv_data        TYPE string
      EXPORTING
        !et_list        TYPE yif_abapgit_definitions=>ty_git_branch_list_tt
...
CLASS ycl_abapgit_git_branch_list IMPLEMENTATION.
  ...
  METHOD find_by_name.
    ...
    IF iv_branch_name IS INITIAL.
      ycx_abapgit_exception=>raise( 'Branch name empty' ) ##NO_TEXT.
    ENDIF.
    ...
    ELSE.
      ...
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( |Branch not found: { iv_branch_name }| ).
      ENDIF.
  ...
  METHOD find_tag_by_name.
    ...
    IF sy-subrc <> 0.
      ...
      READ TABLE mt_branches INTO rs_branch
        WITH KEY name = iv_branch_name.
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'Branch not found' ) ##NO_TEXT.
      ENDIF.
  ...
  METHOD get_branches_only.
    ...
    LOOP AT mt_branches ASSIGNING <ls_branch>.
      IF <ls_branch>-type = yif_abapgit_definitions=>c_git_branch_type-branch.
  ...
  METHOD get_head.
    ...
    ELSE.
      rs_branch = find_by_name( yif_abapgit_definitions=>c_head_name ).
    ENDIF.
  ...
  METHOD get_tags_only.
    ...
    LOOP AT mt_branches ASSIGNING <ls_branch>
                        WHERE type = yif_abapgit_definitions=>c_git_branch_type-lightweight_tag
                           OR type = yif_abapgit_definitions=>c_git_branch_type-annotated_tag.
  ...
  METHOD get_type.
    ...
    rv_type = yif_abapgit_definitions=>c_git_branch_type-other.

    IF iv_branch_name CP 'refs/heads/*' OR iv_branch_name = yif_abapgit_definitions=>c_head_name.
      rv_type = yif_abapgit_definitions=>c_git_branch_type-branch.
      RETURN.
    ENDIF.
    ...
    IF iv_branch_name CP 'refs/tags/*'.
      ...
      IF sy-subrc = 0 AND <lv_result> CP '*' && lv_annotated_tag_with_suffix.
        rv_type = yif_abapgit_definitions=>c_git_branch_type-annotated_tag.
      ELSE.
        rv_type = yif_abapgit_definitions=>c_git_branch_type-lightweight_tag.
      ENDIF.
  ...
  METHOD parse_branch_list.
    ...
    DATA: lt_result            TYPE TABLE OF string,
          lv_hash              TYPE yif_abapgit_definitions=>ty_sha1,
          lv_name              TYPE string,
    ...
    SPLIT lv_data AT yif_abapgit_definitions=>c_newline INTO TABLE lt_result.
    ...
    LOOP AT lt_result INTO lv_data.
      ...
      IF sy-tabix = 1 AND strlen( lv_data ) > 49.
        lv_hash = lv_data+8.
        lv_name = lv_data+49.
        lv_char = ycl_abapgit_git_utils=>get_null( ).
      ...
      ELSEIF sy-tabix = 1 AND strlen( lv_data ) = 8 AND lv_data(8) = '00000000'.
        ycx_abapgit_exception=>raise( 'No branches, create branch manually by adding file' ) ##NO_TEXT.
      ELSE.
        CONTINUE.
      ENDIF.
      ...
      IF <ls_branch>-name = yif_abapgit_definitions=>c_head_name OR <ls_branch>-name = ev_head_symref.
