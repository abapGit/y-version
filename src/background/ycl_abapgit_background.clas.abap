CLASS ycl_abapgit_background DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS YCL_ABAPGIT_BACKGROUND IMPLEMENTATION.
...
* in order to handle local classes in the compiled report
    ls_method-class = 'YCL_ABAPGIT_BACKGROUND_PULL'.
    INSERT ls_method INTO TABLE rt_methods.
    ls_method-class = 'YCL_ABAPGIT_BACKGROUND_PUSH_AU'.
    INSERT ls_method INTO TABLE rt_methods.
    ls_method-class = 'YCL_ABAPGIT_BACKGROUND_PUSH_FI'.
    INSERT ls_method INTO TABLE rt_methods.

    ls_key-clsname = 'YIF_ABAPGIT_BACKGROUND'.
    ...
    LOOP AT rt_methods ASSIGNING <ls_method>.
      CALL METHOD (<ls_method>-class)=>yif_abapgit_background~get_description
        RECEIVING
  ...
  METHOD run.
    ...
    DATA: lo_per        TYPE REF TO ycl_abapgit_persist_background,
          ...
          lt_list       TYPE ycl_abapgit_persist_background=>tt_background,
          li_background TYPE REF TO yif_abapgit_background,
    ...
    LOOP AT lt_list ASSIGNING <ls_list>.
      lo_repo ?= ycl_abapgit_repo_srv=>get_instance( )->get( <ls_list>-key ).
      lv_repo_name = lo_repo->get_name( ).
      WRITE: / <ls_list>-method, lv_repo_name.

      ycl_abapgit_login_manager=>set(
    ...
    CALL FUNCTION 'DEQUEUE_EYABAPGIT'
      EXPORTING
        type = lc_enq_type.
