CLASS ycl_abapgit_repo_online DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    INTERFACES yif_abapgit_git_operations .

    ALIASES create_branch
      FOR yif_abapgit_git_operations~create_branch .
    ALIASES push
      FOR yif_abapgit_git_operations~push .
    ...
    METHODS get_url
      RETURNING
        VALUE(rv_url) TYPE yif_abapgit_persistence=>ty_repo-url .
    METHODS get_branch_name
      RETURNING
        VALUE(rv_name) TYPE yif_abapgit_persistence=>ty_repo-branch_name .
    METHODS set_url
      IMPORTING
        !iv_url TYPE yif_abapgit_persistence=>ty_repo-url
      RAISING
        ycx_abapgit_exception .
    METHODS set_branch_name
      IMPORTING
        !iv_branch_name TYPE yif_abapgit_persistence=>ty_repo-branch_name
      RAISING
        ycx_abapgit_exception .
    METHODS get_sha1_remote
      RETURNING
        VALUE(rv_sha1) TYPE yif_abapgit_definitions=>ty_sha1
      RAISING
        ycx_abapgit_exception .
    METHODS get_objects
      RETURNING
    ...
    METHODS get_unnecessary_local_objs
      RETURNING
        VALUE(rt_unnecessary_local_objects) TYPE yif_abapgit_definitions=>ty_tadir_tt
      RAISING
        ycx_abapgit_exception .
  ...
  PRIVATE SECTION.
    ...
    DATA mt_objects TYPE yif_abapgit_definitions=>ty_objects_tt .
    DATA mv_branch TYPE yif_abapgit_definitions=>ty_sha1 .
    DATA mv_initialized TYPE abap_bool .
    DATA mt_status TYPE yif_abapgit_definitions=>ty_results_tt .
...
CLASS YCL_ABAPGIT_REPO_ONLINE IMPLEMENTATION.
  ...
  METHOD get_unnecessary_local_objs.
    ...
    DATA: lt_tadir        TYPE yif_abapgit_definitions=>ty_tadir_tt,
          lt_tadir_unique TYPE HASHED TABLE OF yif_abapgit_definitions=>ty_tadir
          ...
          lv_package      TYPE yif_abapgit_persistence=>ty_repo-package.

    FIELD-SYMBOLS: <ls_status> TYPE yif_abapgit_definitions=>ty_result,
    ...
    LOOP AT lt_status ASSIGNING <ls_status>
                      WHERE lstate = yif_abapgit_definitions=>c_state-added.
...
* skip objects that does not exist locally
  ...
  METHOD handle_stage_ignore.
    ...
    DATA: lv_add         TYPE abap_bool,
          lo_dot_abapgit TYPE REF TO ycl_abapgit_dot_abapgit,
    ...
    LOOP AT lt_stage ASSIGNING <ls_stage> WHERE method = ycl_abapgit_stage=>c_method-ignore.
    ...
    IF lv_add = abap_true.
      io_stage->add(
        iv_path     = yif_abapgit_definitions=>c_root_dir
        iv_filename = yif_abapgit_definitions=>c_dot_abapgit
  ...
  METHOD refresh.
    ...
    DATA: lo_progress  TYPE REF TO ycl_abapgit_progress,
          ls_pull      TYPE ycl_abapgit_git_porcelain=>ty_pull_result,
          lx_exception TYPE REF TO ycx_abapgit_exception.
    ...
    ls_pull = ycl_abapgit_git_porcelain=>pull(
      iv_url         = get_url( )
  ...
  METHOD set_branch_name.
    ...
    IF ms_data-local_settings-write_protected = abap_true.
      ycx_abapgit_exception=>raise( 'Cannot switch branch. Local code is write-protected by repo config' ).
    ENDIF.
  ...
  METHOD set_url.
    ...
    IF ms_data-local_settings-write_protected = abap_true.
      ycx_abapgit_exception=>raise( 'Cannot change URL. Local code is write-protected by repo config' ).
    ENDIF.
  ...
  METHOD status.
    ...
    IF lines( mt_status ) = 0.
      mt_status = ycl_abapgit_file_status=>status( io_repo = me
                                                   io_log  = io_log ).
    ENDIF.
  ...
  METHOD yif_abapgit_git_operations~create_branch.
    ...
    ycl_abapgit_git_porcelain=>create_branch(
      iv_url  = get_url( )
      iv_name = iv_name
      iv_from = lv_sha1 ).
  ...
  METHOD yif_abapgit_git_operations~push.

* assumption: PUSH is done on top of the currently selected branch

    DATA: ls_push TYPE ycl_abapgit_git_porcelain=>ty_push_result,
          lv_text TYPE string.
    ...
    IF ms_data-local_settings-block_commit = abap_true
      ...
      ycx_abapgit_exception=>raise( |A successful code inspection is required| ).
    ENDIF.

    handle_stage_ignore( io_stage ).

    ls_push = ycl_abapgit_git_porcelain=>push(
