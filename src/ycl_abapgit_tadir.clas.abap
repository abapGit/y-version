CLASS ycl_abapgit_tadir DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE
  ...
  PRIVATE SECTION.
    ...
    METHODS build
      IMPORTING
        ...
        !io_folder_logic       TYPE REF TO ycl_abapgit_folder_logic OPTIONAL
      RETURNING
        VALUE(rt_tadir)        TYPE yif_abapgit_definitions=>ty_tadir_tt
      RAISING
        ycx_abapgit_exception .
...
CLASS YCL_ABAPGIT_TADIR IMPLEMENTATION.
  ...
  METHOD build.
    ...
    DATA: lt_tadir        TYPE yif_abapgit_definitions=>ty_tadir_tt,
          ...
          lv_path         TYPE string,
          lo_skip_objects TYPE REF TO ycl_abapgit_skip_objects,
...
* replace the internal GUID with a hash of the path
          TRY.
              CALL METHOD ('YCL_ABAPGIT_OBJECT_SICF')=>read_sicf_url
                EXPORTING
...
* transportable objects until the transport is released
  ...
  METHOD exists.
    ...
    IF ycl_abapgit_objects=>is_supported( is_item ) = abap_false.
      rv_exists = abap_true.
      RETURN.
    ENDIF.

    rv_exists = ycl_abapgit_objects=>exists( is_item ).
  ...
  METHOD yif_abapgit_tadir~get_object_package.
    ...
    ls_tadir = yif_abapgit_tadir~read_single(
      iv_pgmid    = iv_pgmid
      iv_object   = iv_object
  ...
  METHOD yif_abapgit_tadir~read.
    ...
    DATA: li_exit TYPE REF TO yif_abapgit_exit.
...
* hmm, some problems here, should TADIR also build path?
  ...
  METHOD yif_abapgit_tadir~read_single.
    ...
    IF iv_object = 'SICF'.
      TRY.
          CALL METHOD ('YCL_ABAPGIT_OBJECT_SICF')=>read_tadir
            EXPORTING
              iv_pgmid    = iv_pgmid
              iv_obj_name = iv_obj_name
            RECEIVING
