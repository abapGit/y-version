...
CLASS ltd_sap_package IMPLEMENTATION.
  ...
  METHOD yif_abapgit_sap_package~are_changes_recorded_in_tr_req. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_sap_package~create. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_sap_package~create_child. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_sap_package~create_local. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_sap_package~exists. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_sap_package~get_transport_type. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_sap_package~list_superpackages. "##needed

  ENDMETHOD.

  METHOD yif_abapgit_sap_package~read_parent. "##needed
...
CLASS ltcl_resolve_packages DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    DATA:
      mt_tadir        TYPE yif_abapgit_definitions=>ty_tadir_tt,
...
CLASS ycl_abapgit_dependencies DEFINITION LOCAL FRIENDS ltcl_resolve_packages.
...
CLASS ltcl_resolve_packages IMPLEMENTATION.
  ...
  METHOD when_packages_are_resolved.
    ...
    ycl_abapgit_injector=>set_sap_package( iv_package     = 'Z_MAIN'
                                           ii_sap_package = lo_mock_sap_package ).

    ycl_abapgit_dependencies=>resolve_packages(
      CHANGING
        ct_tadir = mt_tadir ).
