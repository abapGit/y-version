CLASS ycl_abapgit_merge DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS get_conflicts
      RETURNING
        VALUE(rt_conflicts) TYPE yif_abapgit_definitions=>tt_merge_conflict .
    METHODS get_result
      RETURNING
        VALUE(rs_merge) TYPE yif_abapgit_definitions=>ty_merge .
    ...
    METHODS resolve_conflict
      IMPORTING
        !is_conflict TYPE yif_abapgit_definitions=>ty_merge_conflict
      RAISING
        ycx_abapgit_exception .
    METHODS run
      RAISING
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      ty_ancestor_tt TYPE STANDARD TABLE OF yif_abapgit_definitions=>ty_ancestor WITH DEFAULT KEY .

    DATA mo_repo TYPE REF TO ycl_abapgit_repo_online .
    DATA ms_merge TYPE yif_abapgit_definitions=>ty_merge .
    ...
    METHODS all_files
      RETURNING
        VALUE(rt_files) TYPE yif_abapgit_definitions=>ty_expanded_tt .
    ...
    METHODS find_ancestors
      IMPORTING
        !iv_commit          TYPE yif_abapgit_definitions=>ty_sha1
      RETURNING
    ...
    METHODS find_first_common
      ...
      RETURNING
        VALUE(rs_common) TYPE yif_abapgit_definitions=>ty_ancestor
      RAISING
        ycx_abapgit_exception .
ENDCLASS.
...
CLASS YCL_ABAPGIT_MERGE IMPLEMENTATION.
  ...
  METHOD calculate_result.
    ...
    DATA: lt_files        TYPE yif_abapgit_definitions=>ty_expanded_tt,
...
* conflict must be resolved before merge
  ...
  METHOD constructor.
    ...
    IF iv_source_branch EQ io_repo->get_branch_name( ).
      ycx_abapgit_exception=>raise( 'source = target' ).
    ENDIF.
  ...
  METHOD fetch_git.
    ...
    DATA: lo_branch_list TYPE REF TO ycl_abapgit_git_branch_list,
          lt_upload      TYPE yif_abapgit_definitions=>ty_git_branch_list_tt.

    lo_branch_list = ycl_abapgit_git_transport=>branches( ms_merge-repo->get_url( ) ).

    ms_merge-source = lo_branch_list->find_by_name(
      ycl_abapgit_git_branch_list=>complete_heads_branch_name( mv_source_branch ) ).

    ms_merge-target = lo_branch_list->find_by_name(
      ycl_abapgit_git_branch_list=>complete_heads_branch_name( mo_repo->get_branch_name( ) ) ).
  ...
  METHOD find_ancestors.
    ...
    DATA: ls_commit TYPE ycl_abapgit_git_pack=>ty_commit,
          lt_visit  TYPE STANDARD TABLE OF yif_abapgit_definitions=>ty_sha1,
  ...
  METHOD find_first_common.
    ...
    ycx_abapgit_exception=>raise( 'error finding common ancestor' ).
  ...
  METHOD run.
    ...
    ms_merge-stree = ycl_abapgit_git_porcelain=>full_tree(
      it_objects = mt_objects
      iv_branch  = ms_merge-source-sha1 ).
    ms_merge-ttree = ycl_abapgit_git_porcelain=>full_tree(
      it_objects = mt_objects
      iv_branch  = ms_merge-target-sha1 ).
    ms_merge-ctree = ycl_abapgit_git_porcelain=>full_tree(
      it_objects = mt_objects
