CLASS ycl_abapgit_dot_abapgit DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        !is_data TYPE yif_abapgit_dot_abapgit=>ty_dot_abapgit .
    METHODS serialize
      RETURNING
    ...
    METHODS get_data
      RETURNING
        VALUE(rs_data) TYPE yif_abapgit_dot_abapgit=>ty_dot_abapgit .
    METHODS add_ignore
      IMPORTING
        !iv_path     TYPE string
...
*        IMPORTING iv_language TYPE spras,
    METHODS get_signature
      RETURNING
        VALUE(rs_signature) TYPE yif_abapgit_definitions=>ty_file_signature
      RAISING
        ycx_abapgit_exception .
    METHODS get_requirements
      RETURNING
        VALUE(rt_requirements) TYPE yif_abapgit_dot_abapgit=>ty_requirement_tt.
    METHODS set_requirements
      IMPORTING
        it_requirements TYPE yif_abapgit_dot_abapgit=>ty_requirement_tt.
  PRIVATE SECTION.
    DATA: ms_data TYPE yif_abapgit_dot_abapgit=>ty_dot_abapgit.

    CLASS-METHODS:
      to_xml
        IMPORTING is_data       TYPE yif_abapgit_dot_abapgit=>ty_dot_abapgit
        RETURNING VALUE(rv_xml) TYPE string
        RAISING   ycx_abapgit_exception,
      from_xml
        IMPORTING iv_xml         TYPE string
        RETURNING VALUE(rs_data) TYPE yif_abapgit_dot_abapgit=>ty_dot_abapgit.

ENDCLASS.



CLASS ycl_abapgit_dot_abapgit IMPLEMENTATION.
  ...
  METHOD build_default.
    ...
    DATA: ls_data TYPE yif_abapgit_dot_abapgit=>ty_dot_abapgit.


    ls_data-master_language = sy-langu.
    ls_data-starting_folder = '/src/'.
    ls_data-folder_logic    = yif_abapgit_dot_abapgit=>c_folder_logic-prefix.
  ...
  METHOD deserialize.
    ...
    DATA: lv_xml  TYPE string,
          ls_data TYPE yif_abapgit_dot_abapgit=>ty_dot_abapgit.


    lv_xml = ycl_abapgit_convert=>xstring_to_string_utf8( iv_xstr ).
...
* downward compatibility
    IF rs_data-folder_logic IS INITIAL.
      rs_data-folder_logic = yif_abapgit_dot_abapgit=>c_folder_logic-prefix.
    ENDIF.
  ...
  METHOD get_signature.
    ...
    rs_signature-path     = yif_abapgit_definitions=>c_root_dir.
    rs_signature-filename = yif_abapgit_definitions=>c_dot_abapgit.
    rs_signature-sha1     = ycl_abapgit_hash=>sha1( iv_type = yif_abapgit_definitions=>c_type-blob
                                                    iv_data = serialize( ) ).
  ...
  METHOD is_ignored.
    ...
    CONCATENATE '/' yif_abapgit_definitions=>c_dot_abapgit INTO lv_dot.
  ...
  METHOD serialize.
    ...
    rv_xstr = ycl_abapgit_convert=>string_to_xstring_utf8( lv_xml ).
  ...
  METHOD to_xml.
    ...
    rv_xml = ycl_abapgit_xml_pretty=>print( rv_xml ).
