CLASS ycl_abapgit_gui_page_settings DEFINITION
  PUBLIC
  FINAL
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor.
    METHODS yif_abapgit_gui_page~on_event REDEFINITION.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_default_hotkeys
      RETURNING
        VALUE(rt_default_hotkeys) TYPE yif_abapgit_definitions=>tty_hotkey
      RAISING
        ycx_abapgit_exception.
...
CLASS YCL_ABAPGIT_GUI_PAGE_SETTINGS IMPLEMENTATION.
  ...
  METHOD build_settings.
    ...
    DATA: lv_i_param_value TYPE i,
          ...
          lt_key_bindings  TYPE yif_abapgit_definitions=>tty_hotkey.

    FIELD-SYMBOLS: <ls_post_field>  TYPE ihttpnvp,
                   <ls_key_binding> TYPE yif_abapgit_definitions=>ty_hotkey.
    ...
    IF sy-subrc = 0.
      lv_i_param_value = <ls_post_field>-value.
      IF lv_i_param_value < ycl_abapgit_settings=>c_commitmsg_comment_length_dft.
        lv_i_param_value = ycl_abapgit_settings=>c_commitmsg_comment_length_dft.
      ENDIF.
      mo_settings->set_commitmsg_comment_length( lv_i_param_value ).
    ELSE.
      mo_settings->set_commitmsg_comment_length( ycl_abapgit_settings=>c_commitmsg_comment_length_dft ).
    ENDIF.

    READ TABLE mt_post_fields ASSIGNING <ls_post_field> WITH KEY name = 'body_size'.
    IF sy-subrc = 0.
      lv_i_param_value = <ls_post_field>-value.
      IF lv_i_param_value < ycl_abapgit_settings=>c_commitmsg_body_size_dft.
        lv_i_param_value = ycl_abapgit_settings=>c_commitmsg_body_size_dft.
      ENDIF.
      mo_settings->set_commitmsg_body_size( lv_i_param_value ).
    ELSE.
      mo_settings->set_commitmsg_body_size( ycl_abapgit_settings=>c_commitmsg_body_size_dft ).
    ENDIF.
  ...
  METHOD get_default_hotkeys.
    ...
    DATA: lt_actions TYPE yif_abapgit_gui_page_hotkey=>tty_hotkey_action,
  ...
  METHOD get_possible_hotkey_actions.
    ...
    rt_hotkey_actions = ycl_abapgit_hotkeys=>get_default_hotkeys_from_pages( ).
  ...
  METHOD persist_settings.
    ...
    DATA lo_settings_persistence TYPE REF TO ycl_abapgit_persist_settings.

    lo_settings_persistence = ycl_abapgit_persist_settings=>get_instance( ).
