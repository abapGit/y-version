CLASS ycl_abapgit_services_git DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    TYPES:
      BEGIN OF ty_commit_fields,
        repo_key        TYPE yif_abapgit_persistence=>ty_repo-key,
...
CLASS YCL_ABAPGIT_SERVICES_GIT IMPLEMENTATION.


  METHOD commit.

    DATA: ls_comment TYPE yif_abapgit_definitions=>ty_comment,
          li_user    TYPE REF TO yif_abapgit_persist_user.
    ...
    IF is_commit-committer_name IS INITIAL.
      ycx_abapgit_exception=>raise( 'Commit: Committer name empty' ).
    ELSEIF is_commit-committer_email IS INITIAL.
      ycx_abapgit_exception=>raise( 'Commit: Committer email empty' ).
    ELSEIF is_commit-author_email IS NOT INITIAL AND is_commit-author_name IS INITIAL.
      ycx_abapgit_exception=>raise( 'Commit: Author name empty' ). " Opposite should be OK ?
    ELSEIF is_commit-comment IS INITIAL.
      ycx_abapgit_exception=>raise( 'Commit: empty comment' ).
    ENDIF.
    ...
    IF NOT is_commit-body IS INITIAL.
      CONCATENATE ls_comment-comment '' is_commit-body
        INTO ls_comment-comment SEPARATED BY yif_abapgit_definitions=>c_newline.
    ENDIF.
  ...
  METHOD delete_branch.
    ...
    ls_branch = ycl_abapgit_ui_factory=>get_popups( )->branch_list_popup( lo_repo->get_url( ) ).
    IF ls_branch IS INITIAL.
      RAISE EXCEPTION TYPE ycx_abapgit_cancel.
    ENDIF.

    IF ls_branch-name = 'HEAD'.
      ycx_abapgit_exception=>raise( 'Cannot delete HEAD' ).
    ELSEIF ls_branch-name = lo_repo->get_branch_name( ).
      ycx_abapgit_exception=>raise( 'Switch branch before deleting current' ).
    ENDIF.

    ycl_abapgit_git_porcelain=>delete_branch(
  ...
  METHOD delete_tag.
    ...
    ls_tag = ycl_abapgit_ui_factory=>get_tag_popups( )->tag_select_popup( lo_repo ).
    IF ls_tag IS INITIAL.
      RAISE EXCEPTION TYPE ycx_abapgit_cancel.
    ENDIF.

    ycl_abapgit_git_porcelain=>delete_tag(
      iv_url = lo_repo->get_url( )
      is_tag = ls_tag ).

    lv_text = |Tag { ycl_abapgit_tag=>remove_tag_prefix( ls_tag-name ) } deleted| ##NO_TEXT.
  ...
  METHOD reset.
    ...
    DATA: lo_repo                   TYPE REF TO ycl_abapgit_repo_online,
          ...
          lt_unnecessary_local_objs TYPE yif_abapgit_definitions=>ty_tadir_tt,
    ...
    IF lo_repo->get_local_settings( )-write_protected = abap_true.
      ycx_abapgit_exception=>raise( 'Cannot reset. Local code is write-protected by repo config' ).
    ENDIF.
...
* todo, separate UI and logic
    ...
    IF lines( lt_unnecessary_local_objs ) > 0.
      ...
      IF lines( lt_selected ) > 0.
        ...
        ycl_abapgit_objects=>delete( it_tadir  = lt_selected
                                     is_checks = ls_checks ).
* update repo cache
        lo_repo->refresh( ).
      ENDIF.
  ...
  METHOD switch_branch.
    ...
    ls_branch = ycl_abapgit_ui_factory=>get_popups( )->branch_list_popup(
      iv_url             = lo_repo->get_url( )
    ...
    IF ls_branch-name = ycl_abapgit_ui_factory=>get_popups( )->c_new_branch_label.
  ...
  METHOD tag_overview.
    ...
    ycl_abapgit_ui_factory=>get_tag_popups( )->tag_list_popup( lo_repo ).
