CLASS ycl_abapgit_gui_page_db DEFINITION
  PUBLIC
  ...
  PRIVATE SECTION.
    ...
    METHODS explain_content
      IMPORTING
        !is_data       TYPE yif_abapgit_persistence=>ty_content
      RETURNING
...
CLASS YCL_ABAPGIT_GUI_PAGE_DB IMPLEMENTATION.
    ...
    CASE is_data-type.
      WHEN 'REPO'.
        ...
        IF lv_cnt > 0.
          rv_text = |<strong>On-line</strong>, Name: <strong>{
                    ycl_abapgit_url=>name( rv_text ) }</strong>|.
        ELSE.
      ...
      WHEN 'BACKGROUND'.
        ...
        rv_text = |Method: { is_data-data_str+ls_match-offset(ls_match-length) }, |
               && |Repository: { ycl_abapgit_repo_srv=>get_instance( )->get( is_data-value )->get_name( ) }|.
  ...
  METHOD render_content.
    ...
    DATA: lt_data    TYPE yif_abapgit_persistence=>tt_content,
    ...
    lt_data = ycl_abapgit_persistence_db=>get_instance( )->list( ).
    ...
    LOOP AT lt_data ASSIGNING <ls_data>.
      ...
      lv_action  = ycl_abapgit_html_action_utils=>dbkey_encode( <ls_data> ).

      CREATE OBJECT lo_toolbar.
      lo_toolbar->add( iv_txt = 'Display' iv_act = |{ yif_abapgit_definitions=>c_action-db_display }?{ lv_action }| ).
      lo_toolbar->add( iv_txt = 'Edit'    iv_act = |{ yif_abapgit_definitions=>c_action-db_edit }?{ lv_action }| ).
  ...
  METHOD yif_abapgit_gui_page~on_event.
    ...
    CASE iv_action.
      WHEN c_action-delete.
        ls_db = ycl_abapgit_html_action_utils=>dbkey_decode( iv_getdata ).
        delete( ls_db ).
