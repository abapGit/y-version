CLASS ycl_abapgit_gui_view_repo DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    ALIASES render
      FOR yif_abapgit_gui_page~render .
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      ...
      build_head_menu
        IMPORTING iv_lstate         TYPE char1
                  iv_rstate         TYPE char1
        RETURNING VALUE(ro_toolbar) TYPE REF TO ycl_abapgit_html_toolbar
        RAISING   ycx_abapgit_exception,
      build_grid_menu
        RETURNING VALUE(ro_toolbar) TYPE REF TO ycl_abapgit_html_toolbar
        RAISING   ycx_abapgit_exception,
      render_item
        IMPORTING is_item        TYPE yif_abapgit_definitions=>ty_repo_item
        RETURNING VALUE(ro_html) TYPE REF TO ycl_abapgit_html
        RAISING   ycx_abapgit_exception,
      render_item_files
        IMPORTING is_item        TYPE yif_abapgit_definitions=>ty_repo_item
...
CLASS ycl_abapgit_gui_view_repo IMPLEMENTATION.
  ...
  METHOD build_dir_jump_link.
    ...
    lv_encode = ycl_abapgit_html_action_utils=>dir_encode( lv_path ).

    rv_html = ycl_abapgit_html=>a( iv_txt = lv_path
  ...
  METHOD build_head_menu.
    ...
    DATA: lo_tb_advanced TYPE REF TO ycl_abapgit_html_toolbar,
          lo_tb_branch   TYPE REF TO ycl_abapgit_html_toolbar,
          lo_tb_tag      TYPE REF TO ycl_abapgit_html_toolbar,
          lv_key         TYPE yif_abapgit_persistence=>ty_value,
          lv_wp_opt      LIKE yif_abapgit_definitions=>c_html_opt-crossout,
          lv_crossout    LIKE yif_abapgit_definitions=>c_html_opt-crossout,
          lv_pull_opt    LIKE yif_abapgit_definitions=>c_html_opt-crossout.
    ...
    IF mo_repo->get_local_settings( )-write_protected = abap_true.
      lv_wp_opt   = yif_abapgit_definitions=>c_html_opt-crossout.
      lv_pull_opt = yif_abapgit_definitions=>c_html_opt-crossout.
    ELSE.
      lv_pull_opt = yif_abapgit_definitions=>c_html_opt-strong.
    ENDIF.

    " Build branch drop-down ========================
    IF mo_repo->is_offline( ) = abap_false. " Online ?
      lo_tb_branch->add( iv_txt = 'Overview'
                         iv_act = |{ yif_abapgit_definitions=>c_action-go_branch_overview }?{ lv_key }| ).
      lo_tb_branch->add( iv_txt = 'Switch'
                         iv_act = |{ yif_abapgit_definitions=>c_action-git_branch_switch }?{ lv_key }|
                         iv_opt = lv_wp_opt ).
      lo_tb_branch->add( iv_txt = 'Create'
                         iv_act = |{ yif_abapgit_definitions=>c_action-git_branch_create }?{ lv_key }| ).
      lo_tb_branch->add( iv_txt = 'Delete'
                         iv_act = |{ yif_abapgit_definitions=>c_action-git_branch_delete }?{ lv_key }| ).

      lo_tb_tag->add( iv_txt = 'Overview'
                      iv_act = |{ yif_abapgit_definitions=>c_action-go_tag_overview }?{ lv_key }| ).
      lo_tb_tag->add( iv_txt = 'Switch'
                      iv_act = |{ yif_abapgit_definitions=>c_action-git_tag_switch }?{ lv_key }|
                      iv_opt = lv_wp_opt ).
      lo_tb_tag->add( iv_txt = 'Create'
                      iv_act = |{ yif_abapgit_definitions=>c_action-git_tag_create }?{ lv_key }| ).
      lo_tb_tag->add( iv_txt = 'Delete'
                      iv_act = |{ yif_abapgit_definitions=>c_action-git_tag_delete }?{ lv_key }| ).
    ...
    IF mo_repo->is_offline( ) = abap_false. " Online ?
      lo_tb_advanced->add( iv_txt = 'Reset local'
                           iv_act = |{ yif_abapgit_definitions=>c_action-git_reset }?{ lv_key }|
                           iv_opt = lv_wp_opt ).
      lo_tb_advanced->add( iv_txt = 'Background mode'
                           iv_act = |{ yif_abapgit_definitions=>c_action-go_background }?{ lv_key }| ).
      lo_tb_advanced->add( iv_txt = 'Change remote'
                           iv_act = |{ yif_abapgit_definitions=>c_action-repo_remote_change }?{ lv_key }| ).
      lo_tb_advanced->add( iv_txt = 'Make off-line'
                           iv_act = |{ yif_abapgit_definitions=>c_action-repo_remote_detach }?{ lv_key }| ).
      lo_tb_advanced->add( iv_txt = 'Force stage'
                           iv_act = |{ yif_abapgit_definitions=>c_action-go_stage }?{ lv_key }| ).

      CLEAR lv_crossout.
      IF ycl_abapgit_auth=>is_allowed( yif_abapgit_auth=>gc_authorization-transport_to_branch ) = abap_false.
        lv_crossout = yif_abapgit_definitions=>c_html_opt-crossout.
      ENDIF.
      lo_tb_advanced->add( iv_txt = 'Transport to Branch'
                           iv_act = |{ yif_abapgit_definitions=>c_action-repo_transport_to_branch }?{ lv_key }|
                           iv_opt = lv_crossout ).

    ELSE.
      lo_tb_advanced->add( iv_txt = 'Make on-line'
                           iv_act = |{ yif_abapgit_definitions=>c_action-repo_remote_attach }?{ lv_key }| ).
    ENDIF.
    lo_tb_advanced->add( iv_txt = 'Syntax Check'
                         iv_act = |{ yif_abapgit_definitions=>c_action-repo_syntax_check }?{ lv_key }| ).
    lo_tb_advanced->add( iv_txt = 'Run Code Inspector'
                         iv_act = |{ yif_abapgit_definitions=>c_action-repo_code_inspector }?{ lv_key }| ).
    lo_tb_advanced->add( iv_txt = 'Repo settings'
                         iv_act = |{ yif_abapgit_definitions=>c_action-repo_settings }?{ lv_key }| ).
    ...
    lo_tb_advanced->add( iv_txt = 'Update local checksums'
                         iv_act = |{ yif_abapgit_definitions=>c_action-repo_refresh_checksums }?{ lv_key }|
                         iv_opt = lv_crossout ).

    lo_tb_advanced->add( iv_txt = 'Remove'
                         iv_act = |{ yif_abapgit_definitions=>c_action-repo_remove }?{ lv_key }| ).

    CLEAR lv_crossout.
    IF mo_repo->get_local_settings( )-write_protected = abap_true
        OR ycl_abapgit_auth=>is_allowed( yif_abapgit_auth=>gc_authorization-uninstall ) = abap_false.
      lv_crossout = yif_abapgit_definitions=>c_html_opt-crossout.
    ENDIF.
    lo_tb_advanced->add( iv_txt = 'Uninstall'
                         iv_act = |{ yif_abapgit_definitions=>c_action-repo_purge }?{ lv_key }|
                         iv_opt = lv_crossout ).
    ...
    ELSE.
      ro_toolbar->add( iv_txt = 'Import ZIP'
                       iv_act = |{ yif_abapgit_definitions=>c_action-zip_import }?{ lv_key }|
                       iv_opt = yif_abapgit_definitions=>c_html_opt-strong ).
      ro_toolbar->add( iv_txt = 'Export ZIP'
                       iv_act = |{ yif_abapgit_definitions=>c_action-zip_export }?{ lv_key }|
                       iv_opt = yif_abapgit_definitions=>c_html_opt-strong ).
    ENDIF.
    ...
    ro_toolbar->add( iv_txt = 'Refresh'
                     iv_act = |{ yif_abapgit_definitions=>c_action-repo_refresh }?{ lv_key }| ).
    ro_toolbar->add( iv_txt = ycl_abapgit_html=>icon( iv_name = 'settings/grey70' )
  ...
  METHOD build_obj_jump_link.
    ...
    lv_encode = ycl_abapgit_html_action_utils=>jump_encode( iv_obj_type = is_item-obj_type
                                                    iv_obj_name = is_item-obj_name ).

    rv_html = ycl_abapgit_html=>a( iv_txt = |{ is_item-obj_name }|
                                   iv_act = |{ yif_abapgit_definitions=>c_action-jump }?{ lv_encode }| ).
  ...
  METHOD constructor.
    ...
    mo_repo         = ycl_abapgit_repo_srv=>get_instance( )->get( iv_key ).
    mv_cur_dir      = '/'. " Root
    mv_hide_files   = ycl_abapgit_persistence_user=>get_instance( )->get_hide_files( ).
    mv_changes_only = ycl_abapgit_persistence_user=>get_instance( )->get_changes_only( ).
  ...
  METHOD get_item_icon.
    ...
    CASE is_item-obj_type.
      WHEN 'PROG' OR 'CLAS' OR 'FUGR'.
        rv_html = ycl_abapgit_html=>icon( 'file-code/darkgrey' ).
      WHEN 'W3MI' OR 'W3HT'.
        rv_html = ycl_abapgit_html=>icon( 'file-binary/darkgrey' ).
      WHEN ''.
        rv_html = space. " no icon
      WHEN OTHERS.
        rv_html = ycl_abapgit_html=>icon( 'file/darkgrey' ).
    ENDCASE.

    IF is_item-is_dir = abap_true.
      rv_html = ycl_abapgit_html=>icon( 'file-directory/darkgrey' ).
    ENDIF.
  ...
  METHOD render_head_line.
    ...
    DATA lo_toolbar TYPE REF TO ycl_abapgit_html_toolbar.
  ...
  METHOD render_item_command.
    ...
    IF is_item-is_dir = abap_true. " Directory
      ...
      ro_html->add( ycl_abapgit_gui_chunk_lib=>render_item_state( iv_lstate = is_item-lstate
    ...
    ELSEIF is_item-changes > 0.
      ...
      IF mv_hide_files = abap_true AND is_item-obj_name IS NOT INITIAL.
        ...
        lv_difflink = ycl_abapgit_html_action_utils=>obj_encode(
          iv_key    = mo_repo->get_key( )
          ig_object = is_item ).

        ro_html->add( '<div>' ).
        ro_html->add_a( iv_txt = |view diff ({ is_item-changes })|
                        iv_act = |{ yif_abapgit_definitions=>c_action-go_diff }?{ lv_difflink }| ).
        ro_html->add( ycl_abapgit_gui_chunk_lib=>render_item_state( iv_lstate = is_item-lstate
                                                            iv_rstate = is_item-rstate ) ).
        ro_html->add( '</div>' ).
      ...
      ELSE.
        LOOP AT is_item-files INTO ls_file.
          ...
          IF ls_file-is_changed = abap_true.
            lv_difflink = ycl_abapgit_html_action_utils=>file_encode(
              iv_key  = mo_repo->get_key( )
              ig_file = ls_file ).
            ro_html->add_a( iv_txt = 'view diff'
                            iv_act = |{ yif_abapgit_definitions=>c_action-go_diff }?{ lv_difflink }| ).
            ro_html->add( ycl_abapgit_gui_chunk_lib=>render_item_state( iv_lstate = ls_file-lstate
  ...
  METHOD render_parent_dir.
    ...
    ro_html->add( |<td class="icon">{ ycl_abapgit_html=>icon( 'dir' ) }</td>| ).
  ...
  METHOD yif_abapgit_gui_page~on_event.
    ...
    CASE iv_action.
      WHEN c_actions-toggle_hide_files. " Toggle file diplay
        mv_hide_files   = ycl_abapgit_persistence_user=>get_instance( )->toggle_hide_files( ).
        ev_state        = yif_abapgit_definitions=>c_event_state-re_render.
      WHEN c_actions-change_dir.        " Change dir
        lv_path         = ycl_abapgit_html_action_utils=>dir_decode( iv_getdata ).
        mv_cur_dir      = ycl_abapgit_path=>change_dir( iv_cur_dir = mv_cur_dir iv_cd = lv_path ).
        ev_state        = yif_abapgit_definitions=>c_event_state-re_render.
      ...
      WHEN c_actions-toggle_changes.    " Toggle changes only view
        mv_changes_only = ycl_abapgit_persistence_user=>get_instance( )->toggle_changes_only( ).
        ev_state        = yif_abapgit_definitions=>c_event_state-re_render.
  ...
  METHOD yif_abapgit_gui_page~render.
    ...
    DATA: lt_repo_items TYPE yif_abapgit_definitions=>tt_repo_items,
          lo_browser    TYPE REF TO ycl_abapgit_repo_content_list,
          ...
          lo_log        TYPE REF TO ycl_abapgit_log.

    FIELD-SYMBOLS <ls_item> LIKE LINE OF lt_repo_items.

    " Reinit, for the case of type change
    mo_repo = ycl_abapgit_repo_srv=>get_instance( )->get( mo_repo->get_key( ) ).
    ...
    TRY.
        ...
        LOOP AT lt_repo_items ASSIGNING <ls_item>.
          ycl_abapgit_state=>reduce( EXPORTING iv_cur = <ls_item>-lstate
                                     CHANGING cv_prev = lv_lstate ).
          ycl_abapgit_state=>reduce( EXPORTING iv_cur = <ls_item>-rstate
        ...
        IF ycl_abapgit_path=>is_root( mv_cur_dir ) = abap_false.
        ...
        IF lv_max = abap_true.
          ...
          ro_html->add( |Only { lv_max_str } shown in list. Display {
            ycl_abapgit_html=>a( iv_txt = lv_add_str iv_act = c_actions-display_more )
            } more. (Set in Advanced > {
            ycl_abapgit_html=>a( iv_txt = 'Settings' iv_act = yif_abapgit_definitions=>c_action-go_settings )
            } )| ).
          ro_html->add( '</div>' ).
        ENDIF.
