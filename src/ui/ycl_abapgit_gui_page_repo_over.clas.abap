CLASS ycl_abapgit_gui_page_repo_over DEFINITION
  PUBLIC
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      ...
      apply_order_by
        CHANGING
          ct_overview TYPE ycl_abapgit_gui_page_repo_over=>tty_overview,
      ...
      map_repo_list_to_overview
        IMPORTING
          it_repo_list       TYPE yif_abapgit_persistence=>tt_repo
        RETURNING
          VALUE(rt_overview) TYPE ycl_abapgit_gui_page_repo_over=>tty_overview
        RAISING
      ...
      render_table
        IMPORTING
          io_html     TYPE REF TO ycl_abapgit_html
          it_overview TYPE ycl_abapgit_gui_page_repo_over=>tty_overview,
...
CLASS ycl_abapgit_gui_page_repo_over IMPLEMENTATION.
  ...
  METHOD map_repo_list_to_overview.
    ...
    DATA: ls_overview LIKE LINE OF rt_overview,
          lo_repo_srv TYPE REF TO ycl_abapgit_repo,
          lv_date     TYPE d,
          lv_time     TYPE t.
    ...
    LOOP AT it_repo_list ASSIGNING <ls_repo>.
      ...
      CLEAR: ls_overview.
      lo_repo_srv = ycl_abapgit_repo_srv=>get_instance( )->get( <ls_repo>-key ).

      ls_overview-favorite   = ycl_abapgit_persistence_user=>get_instance(
      ...
      ls_overview-branch     = ycl_abapgit_git_branch_list=>get_display_name( <ls_repo>-branch_name ).
  ...
  METHOD render_content.
    ...
    lt_overview = map_repo_list_to_overview(
      ycl_abapgit_persist_factory=>get_repo( )->list( ) ).
  ...
  METHOD render_table_body.
    ...
    LOOP AT it_overview ASSIGNING <ls_overview>.
      ...
      io_html->add( |<td>| ).
      io_html->add_a( iv_act = |{ yif_abapgit_definitions=>c_action-repo_toggle_fav }?{ <ls_overview>-key }|
                      iv_txt = ycl_abapgit_html=>icon( iv_name  = lv_favorite_icon
                                                       iv_class = 'pad-sides'
                                                       iv_hint  = 'Click to toggle favorite' ) ).
      io_html->add( |</td>| ).
      io_html->add( |<td>{ ycl_abapgit_html=>icon( lv_type_icon )  }</td>| ).

      io_html->add( |<td>{ <ls_overview>-key }</td>| ).
      io_html->add( |<td>{ ycl_abapgit_html=>a( iv_txt = <ls_overview>-name
  ...
  METHOD yif_abapgit_gui_page~on_event.
    ...
    DATA: lv_key  TYPE yif_abapgit_persistence=>ty_value.
