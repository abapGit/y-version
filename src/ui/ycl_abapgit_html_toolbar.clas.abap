CLASS ycl_abapgit_html_toolbar DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    METHODS:
      ...
      add
        IMPORTING
          iv_txt TYPE string
          io_sub TYPE REF TO ycl_abapgit_html_toolbar OPTIONAL
          iv_typ TYPE c         DEFAULT yif_abapgit_definitions=>c_action_type-sapevent
  ...
  PRIVATE SECTION.
    TYPES:
      BEGIN OF ty_item,
        ...
        act TYPE string,
        ico TYPE string,
        sub TYPE REF TO ycl_abapgit_html_toolbar,
        opt TYPE char1,
        typ TYPE char1,
        cur TYPE abap_bool,
        chk TYPE abap_bool,
...
CLASS ycl_abapgit_html_toolbar IMPLEMENTATION.


  METHOD add.
    DATA ls_item TYPE ty_item.

    ASSERT iv_typ = yif_abapgit_definitions=>c_action_type-separator  " sep doesn't have action
      OR iv_typ = yif_abapgit_definitions=>c_action_type-onclick      " click may have no action (assigned in JS)
      OR iv_typ = yif_abapgit_definitions=>c_action_type-dummy        " dummy may have no action
  ...
  METHOD render_items.
    ...
    LOOP AT mt_items ASSIGNING <ls_item>.
      ...
      IF <ls_item>-typ = yif_abapgit_definitions=>c_action_type-separator.
      ...
      IF lv_has_icons = abap_true.
        IF <ls_item>-chk = abap_true.
          lv_icon  = ycl_abapgit_html=>icon( 'check/blue' ).
          lv_check = ' data-check="X"'.
        ELSEIF <ls_item>-chk = abap_false.
          lv_icon = ycl_abapgit_html=>icon( 'check/grey' ).
          lv_check = ' data-check=""'.
        ELSE. " abap_undefined -> not a check box
          lv_icon = ycl_abapgit_html=>icon( <ls_item>-ico ).
        ENDIF.
      ENDIF.
