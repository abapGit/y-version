CLASS ycl_abapgit_gui_chunk_lib DEFINITION PUBLIC FINAL CREATE PUBLIC.

  PUBLIC SECTION.

    CLASS-METHODS render_error
      IMPORTING ix_error       TYPE REF TO ycx_abapgit_exception OPTIONAL
    ...
    CLASS-METHODS render_repo_top
      IMPORTING io_repo               TYPE REF TO ycl_abapgit_repo
                ...
                io_news               TYPE REF TO ycl_abapgit_news OPTIONAL
      RETURNING VALUE(ro_html)        TYPE REF TO ycl_abapgit_html
    ...
    CLASS-METHODS render_news
      IMPORTING
                io_news        TYPE REF TO ycl_abapgit_news
      RETURNING VALUE(ro_html) TYPE REF TO ycl_abapgit_html
      RAISING   ycx_abapgit_exception.
...
CLASS ycl_abapgit_gui_chunk_lib IMPLEMENTATION.
  ...
  METHOD render_branch_span.
    ...
    lv_text = ycl_abapgit_git_branch_list=>get_display_name( iv_branch ).

    IF ycl_abapgit_git_branch_list=>get_type( iv_branch ) = yif_abapgit_definitions=>c_git_branch_type-branch.
    ...
    IF iv_interactive = abap_true.
      ro_html->add_a( iv_act = |{ yif_abapgit_definitions=>c_action-git_branch_switch }?{ io_repo->get_key( ) }|
                      iv_txt = lv_text ).
    ELSE.
      ro_html->add( lv_text ).
    ENDIF.
  ...
  METHOD render_error.
    ...
    ro_html->add( |{ ycl_abapgit_html=>icon( 'alert/red' ) } Error: { lv_error }| ).
    ro_html->add( '</div>' ).
  ...
  METHOD render_hotkey_overview.
    ...
    DATA: lv_display  TYPE string,
          lt_hotkeys  TYPE yif_abapgit_definitions=>tty_hotkey,
          lt_actions  TYPE yif_abapgit_gui_page_hotkey=>tty_hotkey_action,
    ...
    lt_actions = ycl_abapgit_hotkeys=>get_default_hotkeys_from_pages( ).
    ...
    READ TABLE lt_hotkeys ASSIGNING <ls_hotkey>
                          WITH KEY action = ycl_abapgit_gui_page=>c_global_page_action-showhotkeys.
    IF sy-subrc = 0.
  ...
  METHOD render_item_state.
    ...
    DO 2 TIMES.
      ...
      CASE <lv_state>.
        WHEN yif_abapgit_definitions=>c_state-unchanged.  "None or unchanged
          IF iv_lstate = yif_abapgit_definitions=>c_state-added OR iv_rstate = yif_abapgit_definitions=>c_state-added.
          ...
          ENDIF.
        WHEN yif_abapgit_definitions=>c_state-modified.   "Changed
          rv_html = rv_html && |<span class="changed" title="{ lv_system } Modified">M</span>|.
        WHEN yif_abapgit_definitions=>c_state-added.      "Added new
          rv_html = rv_html && |<span class="added" title="{ lv_system } Added new">A</span>|.
        WHEN yif_abapgit_definitions=>c_state-mixed.      "Multiple changes (multifile)
          rv_html = rv_html && |<span class="mixed" title="{ lv_system } Multiple changes">&#x25A0;</span>|.
        WHEN yif_abapgit_definitions=>c_state-deleted.    "Deleted
  ...
  METHOD render_js_error_banner.
    ...
    ro_html->add( |{ ycl_abapgit_html=>icon( 'alert/red' ) }| &&
  ...
  METHOD render_news.
    ...
    DATA: lv_text    TYPE string,
          ...
          lt_log     TYPE ycl_abapgit_news=>tt_log.
    ...
    IF io_news->has_important( ) = abap_true.
      ro_html->add( '<div class="headbar important">'
        && ycl_abapgit_html=>icon( iv_name = 'alert' iv_class = 'pad-right' )
  ...
  METHOD render_repo_top.
    ...
    DATA: lo_repo_online TYPE REF TO ycl_abapgit_repo_online,
          lo_pback       TYPE REF TO ycl_abapgit_persist_background,
          lv_hint        TYPE string,
          lv_icon        TYPE string.
    ...
    IF io_repo->is_offline( ) = abap_false.
      ...
      ro_html->add_a( iv_txt   = lo_repo_online->get_url( )
                      iv_act   = |{ yif_abapgit_definitions=>c_action-url }?|
    ...
    IF io_news IS BOUND AND io_news->has_news( ) = abap_true.
      ...
      ro_html->add_a( iv_act = 'displayNews()'
                      iv_typ = yif_abapgit_definitions=>c_action_type-onclick
                      iv_txt = ycl_abapgit_html=>icon( iv_name  = lv_icon
                                                       iv_class = 'pad-sides'
    ...
    " Fav
    IF abap_true = ycl_abapgit_persistence_user=>get_instance( )->is_favorite_repo( io_repo->get_key( ) ).
      lv_icon = 'star/blue' ##NO_TEXT.
    ELSE.
      lv_icon = 'star/grey' ##NO_TEXT.
    ENDIF.
    ro_html->add_a( iv_act = |{ yif_abapgit_definitions=>c_action-repo_toggle_fav }?{ io_repo->get_key( ) }|
                    iv_txt = ycl_abapgit_html=>icon( iv_name  = lv_icon
                                                     iv_class = 'pad-sides'
    ...
    IF iv_show_package = abap_true.
      ...
      ro_html->add_a( iv_txt = io_repo->get_package( )
                      iv_act = |{ yif_abapgit_definitions=>c_action-jump_pkg }?{ io_repo->get_package( ) }| ).
      ro_html->add( '</span>' ).
    ENDIF.
