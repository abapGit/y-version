CLASS ycl_abapgit_gui_asset_manager DEFINITION PUBLIC FINAL CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS get_images
      RETURNING VALUE(rt_images) TYPE yif_abapgit_definitions=>tt_web_assets.
...
CLASS YCL_ABAPGIT_GUI_ASSET_MANAGER IMPLEMENTATION.
  ...
  METHOD get_asset.
    ...
    CASE lv_asset_name.
      WHEN 'CSS_COMMON'.
        lv_mime_name = 'YABAPGIT_CSS_COMMON'.
      WHEN 'JS_COMMON'.
        lv_mime_name = 'YABAPGIT_JS_COMMON'.
      WHEN OTHERS.
        ycx_abapgit_exception=>raise( |Improper resource name: { iv_asset_name }| ).
    ENDCASE.
    ...
    IF rv_data IS INITIAL.
      ycx_abapgit_exception=>raise( |Failed to get GUI resource: { iv_asset_name }| ).
    ENDIF.
...
* used by abapmerge
    ...
    DATA: lt_data TYPE yif_abapgit_definitions=>ty_string_tt,
          lv_str  TYPE string.

    CASE iv_asset_name.
      WHEN 'CSS_COMMON'.
        " @@abapmerge include yabapgit_css_common.w3mi.data.css > _inline '$$'.
      WHEN 'JS_COMMON'.
        " @@abapmerge include yabapgit_js_common.w3mi.data.js > _inline '$$'.
      WHEN OTHERS.
        ycx_abapgit_exception=>raise( |No inline resource: { iv_asset_name }| ).
    ENDCASE.

    CONCATENATE LINES OF lt_data INTO lv_str SEPARATED BY yif_abapgit_definitions=>c_newline.
  ...
  METHOD get_inline_images.
    ...
    DATA ls_image TYPE yif_abapgit_definitions=>ty_web_asset.
