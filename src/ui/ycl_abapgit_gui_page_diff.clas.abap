CLASS ycl_abapgit_gui_page_diff DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_file_diff,
        ...
        o_diff     TYPE REF TO ycl_abapgit_diff,
    ...
    METHODS constructor
      IMPORTING
        ...
        !is_file       TYPE yif_abapgit_definitions=>ty_file OPTIONAL
        !is_object     TYPE yif_abapgit_definitions=>ty_item OPTIONAL
        !io_stage      TYPE REF TO ycl_abapgit_stage OPTIONAL
  ...
  PRIVATE SECTION.
    ...
    DATA: mt_diff_files    TYPE tt_file_diff,
          mt_delayed_lines TYPE yif_abapgit_definitions=>ty_diffs_tt,
          mv_unified       TYPE abap_bool VALUE abap_true,
          mv_repo_key      TYPE yif_abapgit_persistence=>ty_repo-key,
          mv_seed          TYPE string, " Unique page id to bind JS sessionStorage
          mv_patch_mode    TYPE abap_bool,
          mo_stage         TYPE REF TO ycl_abapgit_stage.
    ...
    METHODS render_beacon
      IMPORTING is_diff_line   TYPE yif_abapgit_definitions=>ty_diff
    ...
    METHODS render_line_unified
      IMPORTING is_diff_line   TYPE yif_abapgit_definitions=>ty_diff OPTIONAL
      RETURNING VALUE(ro_html) TYPE REF TO ycl_abapgit_html.
    METHODS append_diff
      IMPORTING it_remote TYPE yif_abapgit_definitions=>ty_files_tt
                it_local  TYPE yif_abapgit_definitions=>ty_files_item_tt
                is_status TYPE yif_abapgit_definitions=>ty_result
    ...
    METHODS render_patch
      IMPORTING
        ...
        iv_filename            TYPE string
        is_diff_line           TYPE yif_abapgit_definitions=>ty_diff
        iv_index               TYPE sytabix.
    ...
    METHODS render_patch_head
      IMPORTING
        io_html TYPE REF TO ycl_abapgit_html
        is_diff TYPE ycl_abapgit_gui_page_diff=>ty_file_diff.
    ...
    METHODS get_diff_object
      IMPORTING
        iv_filename    TYPE string
      RETURNING
        VALUE(ro_diff) TYPE REF TO ycl_abapgit_diff
      RAISING
        ycx_abapgit_exception.
    METHODS get_diff_line
      IMPORTING
        io_diff        TYPE REF TO ycl_abapgit_diff
        iv_line_index  TYPE string
      RETURNING
        VALUE(rs_diff) TYPE yif_abapgit_definitions=>ty_diff
      RAISING
        ycx_abapgit_exception.
...
CLASS ycl_abapgit_gui_page_diff IMPLEMENTATION.
  ...
  METHOD add_to_stage.
    ...
    DATA: lo_repo              TYPE REF TO ycl_abapgit_repo_online,
          ...
          lt_diff              TYPE yif_abapgit_definitions=>ty_diffs_tt,
          lv_something_patched TYPE abap_bool,
          lv_patch             TYPE xstring,
          lo_git_add_patch     TYPE REF TO ycl_abapgit_git_add_patch.

    FIELD-SYMBOLS: <ls_diff_file> TYPE ycl_abapgit_gui_page_diff=>ty_file_diff.

    lo_repo  ?= ycl_abapgit_repo_srv=>get_instance( )->get( mv_repo_key ).
    ...
    IF lv_something_patched = abap_false.
      ycx_abapgit_exception=>raise( |Nothing added| ).
    ENDIF.
  ...
  METHOD append_diff.
    ...
    IF <ls_local> IS INITIAL AND <ls_remote> IS INITIAL.
      ycx_abapgit_exception=>raise( |DIFF: file not found { is_status-filename }| ).
    ENDIF.
    ...
    IF <ls_local>-item-obj_type IS NOT INITIAL.
      <ls_diff>-changed_by = to_lower( ycl_abapgit_objects=>changed_by( <ls_local>-item ) ).
    ENDIF.
  ...
  METHOD apply_patch_for.
    ...
    DATA: lo_diff      TYPE REF TO ycl_abapgit_diff,
          ls_diff_line TYPE yif_abapgit_definitions=>ty_diff,
          lv_line      TYPE i.
    ...
    CASE ls_diff_line-result.
      WHEN yif_abapgit_definitions=>c_diff-update
        OR yif_abapgit_definitions=>c_diff-insert.
  ...
  METHOD build_menu.
    ...
    DATA: lo_sub   TYPE REF TO ycl_abapgit_html_toolbar,
    ...
    IF lines( lt_types ) > 1 OR lines( lt_users ) > 1.
      ...
      " File types
      IF lines( lt_types ) > 1.
        lo_sub->add( iv_txt = 'TYPE' iv_typ = yif_abapgit_definitions=>c_action_type-separator ).
        LOOP AT lt_types ASSIGNING <lv_i>.
          lo_sub->add( iv_txt = <lv_i>
                       iv_typ = yif_abapgit_definitions=>c_action_type-onclick
                       iv_aux = 'type'
                       iv_chk = abap_true ).
        ENDLOOP.
      ENDIF.
      ...
      " Changed by
      IF lines( lt_users ) > 1.
        lo_sub->add( iv_txt = 'CHANGED BY' iv_typ = yif_abapgit_definitions=>c_action_type-separator ).
    ...
    IF mv_patch_mode = abap_true.
      ro_menu->add( iv_txt = 'Stage'
                    ...
                    iv_id  = 'stage'
                    iv_typ = yif_abapgit_definitions=>c_action_type-dummy
                     ) ##NO_TEXT.
    ENDIF.
  ...
  METHOD constructor.
    ...
    DATA: lt_remote TYPE yif_abapgit_definitions=>ty_files_tt,
          lt_local  TYPE yif_abapgit_definitions=>ty_files_item_tt,
          lt_status TYPE yif_abapgit_definitions=>ty_results_tt,
    ...
    mv_unified            = ycl_abapgit_persistence_user=>get_instance( )->get_diff_unified( ).
    mv_repo_key           = iv_key.
    ...
    IF lines( mt_diff_files ) = 0.
      ycx_abapgit_exception=>raise( 'PAGE_DIFF ERROR: No diff files found' ).
    ENDIF.
  ...
  METHOD get_patch_data.
    ...
    AND iv_action <> c_patch_action-remove.
      ycx_abapgit_exception=>raise( |Invalid action { iv_action }| ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Invalid patch| ).
    ENDIF.
  ...
  METHOD render_content.
    ...
    ro_html->add( ycl_abapgit_gui_chunk_lib=>render_js_error_banner( ) ).
  ...
  METHOD render_diff_head.
    ...
    DATA: ls_stats TYPE yif_abapgit_definitions=>ty_count.
    ...
    ro_html->add( ycl_abapgit_gui_chunk_lib=>render_item_state(
  ...
  METHOD render_lines.
    ...
    DATA: lo_highlighter TYPE REF TO ycl_abapgit_syntax_highlighter,
          lt_diffs       TYPE yif_abapgit_definitions=>ty_diffs_tt,
          lv_insert_nav  TYPE abap_bool,
          lv_tabix       TYPE syst-tabix.

    FIELD-SYMBOLS <ls_diff>  LIKE LINE OF lt_diffs.

    lo_highlighter = ycl_abapgit_syntax_highlighter=>create( is_diff-filename ).
    CREATE OBJECT ro_html.
  ...
  METHOD render_line_split.
    ...
    " New line
    lv_mark = ` `.
    IF iv_fstate = c_fstate-both OR is_diff_line-result = yif_abapgit_definitions=>c_diff-update.
      lv_bg = ' diff_upd'.
      lv_mark = `~`.
    ELSEIF is_diff_line-result = yif_abapgit_definitions=>c_diff-insert.
      lv_bg = ' diff_ins'.
      lv_mark = `+`.
    ENDIF.
    ...
    IF iv_fstate = c_fstate-both OR is_diff_line-result = yif_abapgit_definitions=>c_diff-update.
      lv_bg = ' diff_upd'.
      lv_mark = `~`.
    ELSEIF is_diff_line-result = yif_abapgit_definitions=>c_diff-delete.
      lv_bg = ' diff_del'.
      lv_mark = `-`.
    ENDIF.
  ...
  METHOD render_line_unified.
    ...
    IF is_diff_line-result <> yif_abapgit_definitions=>c_diff-update.
  ...
  METHOD start_staging.
    ...
    lt_fields = ycl_abapgit_html_action_utils=>parse_fields( lv_string ).

    ycl_abapgit_html_action_utils=>get_field( EXPORTING iv_name  = c_patch_action-add
                                                        it_field = lt_fields
                                              CHANGING  cg_field = lv_add ).

    ycl_abapgit_html_action_utils=>get_field( EXPORTING iv_name  = c_patch_action-remove
                                                        it_field = lt_fields
  ...
  METHOD yif_abapgit_gui_page~on_event.
    ...
    CASE iv_action.
      WHEN c_actions-toggle_unified. " Toggle file diplay
        ...
        mv_unified = ycl_abapgit_persistence_user=>get_instance( )->toggle_diff_unified( ).
      ...
      WHEN c_actions-stage.
        ...
        lo_repo  ?= ycl_abapgit_repo_srv=>get_instance( )->get( mv_repo_key ).
        CREATE OBJECT ei_page TYPE ycl_abapgit_gui_page_commit
          EXPORTING
            io_repo  = lo_repo
            io_stage = mo_stage.
  ...
  METHOD get_diff_object.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Invalid filename { iv_filename }| ).
    ENDIF.
  ...
  METHOD get_diff_line.
    ...
    DATA: lt_diff       TYPE yif_abapgit_definitions=>ty_diffs_tt,
    ...
    READ TABLE lt_diff INTO rs_diff
                       INDEX lv_line_index.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Invalid line index { lv_line_index }| ).
    ENDIF.
