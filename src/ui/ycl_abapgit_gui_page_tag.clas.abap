CLASS ycl_abapgit_gui_page_tag DEFINITION PUBLIC FINAL
  ...
  PRIVATE SECTION.
    ...
    DATA: mo_repo_online   TYPE REF TO ycl_abapgit_repo_online,
...
CLASS ycl_abapgit_gui_page_tag IMPLEMENTATION.
  ...
  METHOD create_tag.
    ...
    DATA:
      ls_tag   TYPE yif_abapgit_definitions=>ty_git_tag,
    ...
    IF ls_tag-name IS INITIAL.
      ycx_abapgit_exception=>raise( |Please supply a tag name| ).
    ENDIF.

    ls_tag-name = ycl_abapgit_tag=>add_tag_prefix( ls_tag-name ).
    ASSERT ls_tag-name CP 'refs/tags/+*'.

    CASE mv_selected_type.
      WHEN c_tag_type-lightweight.

        ls_tag-type = yif_abapgit_definitions=>c_git_branch_type-lightweight_tag.
      ...
      WHEN OTHERS.
        ...
        ycx_abapgit_exception=>raise( |Invalid tag type: { mv_selected_type }| ).

    ENDCASE.

    TRY.
        ycl_abapgit_git_porcelain=>create_tag( iv_url = mo_repo_online->get_url( )
                                               is_tag = ls_tag ).

      CATCH ycx_abapgit_exception INTO lx_error.
        ycx_abapgit_exception=>raise( |Cannot create tag { ls_tag-name }. Error: '{ lx_error->get_text( ) }'| ).
    ENDTRY.

    IF ls_tag-type = yif_abapgit_definitions=>c_git_branch_type-lightweight_tag.
      lv_text = |Lightweight tag { ycl_abapgit_tag=>remove_tag_prefix( ls_tag-name ) } created| ##NO_TEXT.
    ELSEIF ls_tag-type = yif_abapgit_definitions=>c_git_branch_type-annotated_tag.
      lv_text = |Annotated tag { ycl_abapgit_tag=>remove_tag_prefix( ls_tag-name ) } created| ##NO_TEXT.
    ENDIF.
  ...
  METHOD parse_tag_request.
    ...
    REPLACE ALL OCCURRENCES OF yif_abapgit_definitions=>c_crlf    IN lv_string WITH lc_replace.
    REPLACE ALL OCCURRENCES OF yif_abapgit_definitions=>c_newline IN lv_string WITH lc_replace.
    lt_fields = ycl_abapgit_html_action_utils=>parse_fields_upper_case_name( lv_string ).

    ycl_abapgit_html_action_utils=>get_field( EXPORTING iv_name = 'SHA1'
                                                        it_field = lt_fields
                                              CHANGING cg_field = eg_fields ).
    ycl_abapgit_html_action_utils=>get_field( EXPORTING iv_name = 'NAME'
                                                        it_field = lt_fields
                                              CHANGING cg_field = eg_fields ).
    ycl_abapgit_html_action_utils=>get_field( EXPORTING iv_name = 'TAGGER_NAME'
                                                        it_field = lt_fields
                                              CHANGING cg_field = eg_fields ).
    ycl_abapgit_html_action_utils=>get_field( EXPORTING iv_name = 'TAGGER_EMAIL'
                                                        it_field = lt_fields
                                              CHANGING cg_field = eg_fields ).
    ycl_abapgit_html_action_utils=>get_field( EXPORTING iv_name = 'MESSAGE'
                                                        it_field = lt_fields
                                              CHANGING cg_field = eg_fields ).
    ycl_abapgit_html_action_utils=>get_field( EXPORTING iv_name = 'BODY'
                                                        it_field = lt_fields
                                              CHANGING cg_field = eg_fields ).

    ASSIGN COMPONENT 'BODY' OF STRUCTURE eg_fields TO <lv_body>.
    ASSERT <lv_body> IS ASSIGNED.
    REPLACE ALL OCCURRENCES OF lc_replace IN <lv_body> WITH yif_abapgit_definitions=>c_newline.
  ...
  METHOD render_form.
    ...
    DATA: li_user      TYPE REF TO yif_abapgit_persist_user,
          lv_user      TYPE string,
          lv_email     TYPE string,
          lv_s_param   TYPE string,
          lo_settings  TYPE REF TO ycl_abapgit_settings,
          lv_body_size TYPE i,
    ...
    li_user = ycl_abapgit_persistence_user=>get_instance( ).
    ...
    IF lv_user IS INITIAL.
      ...
      lv_user = ycl_abapgit_user_master_record=>get_instance( sy-uname )->get_name( ).
    ENDIF.
    ...
    IF lv_email IS INITIAL.
      " get default from user master record
      lv_email = ycl_abapgit_user_master_record=>get_instance( sy-uname )->get_email( ).
    ENDIF.
  ...
  METHOD render_menu.
    ...
    lo_toolbar->add( iv_act = 'submitFormById(''commit_form'');'
                     ...
                     iv_opt = yif_abapgit_definitions=>c_html_opt-strong ) ##NO_TEXT.

    lo_toolbar->add( iv_act = c_action-commit_cancel
                     iv_txt = 'Cancel'
                     iv_opt = yif_abapgit_definitions=>c_html_opt-cancel ) ##NO_TEXT.
