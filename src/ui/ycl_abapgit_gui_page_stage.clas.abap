CLASS ycl_abapgit_gui_page_stage DEFINITION
  PUBLIC
  FINAL
  ...
  PRIVATE SECTION.
    ...
    DATA ms_files TYPE yif_abapgit_definitions=>ty_stage_files .
    ...
    METHODS render_file
      IMPORTING
        ...
        !is_file       TYPE yif_abapgit_definitions=>ty_file
        !is_item       TYPE yif_abapgit_definitions=>ty_item OPTIONAL
...
CLASS YCL_ABAPGIT_GUI_PAGE_STAGE IMPLEMENTATION.
  ...
  METHOD build_menu.
    ...
    IF lines( ms_files-local ) > 0.
      ro_menu->add( iv_txt = |All diffs|
                    iv_act = |{ yif_abapgit_definitions=>c_action-go_diff }?key={ mo_repo->get_key( ) }| ).
    ENDIF.
  ...
  METHOD constructor.
    ...
    mo_repo               = io_repo.
    ms_files              = ycl_abapgit_factory=>get_stage_logic( )->get( mo_repo ).
    mv_seed               = iv_seed.
  ...
  METHOD find_changed_by.
    ...
    LOOP AT rt_changed_by ASSIGNING <ls_changed_by>.
      TRY.
          <ls_changed_by>-name = to_lower( ycl_abapgit_objects=>changed_by( <ls_changed_by>-item ) ).
  ...
  METHOD get_page_patch.
    ...
    DATA: lo_page   TYPE REF TO ycl_abapgit_gui_page_diff,
          lv_key    TYPE yif_abapgit_persistence=>ty_repo-key,
          ls_file   TYPE yif_abapgit_definitions=>ty_file,
  ...
  METHOD process_stage_list.
    ...
    IF lines( lt_fields ) = 0.
      ycx_abapgit_exception=>raise( 'process_stage_list: empty list' ).
    ENDIF.

    LOOP AT lt_fields ASSIGNING <ls_item>.

      ycl_abapgit_path=>split_file_location(
        EXPORTING
      ...
      CASE <ls_item>-value.
        ...
        WHEN ycl_abapgit_stage=>c_method-ignore.
          io_stage->ignore( iv_path     = ls_file-path
                            iv_filename = ls_file-filename ).
        WHEN ycl_abapgit_stage=>c_method-rm.
          io_stage->rm(     iv_path     = ls_file-path
                            iv_filename = ls_file-filename ).
        WHEN ycl_abapgit_stage=>c_method-skip.
          " Do nothing
        WHEN OTHERS.
          ycx_abapgit_exception=>raise( |process_stage_list: unknown method { <ls_item>-value }| ).
      ENDCASE.
    ENDLOOP.
  ...
  METHOD render_actions.
    ...
    lv_param = ycl_abapgit_html_action_utils=>file_encode( iv_key  = mo_repo->get_key( )
                                                           ig_file = ls_file ).
    ...
    ro_html->add_a(
      iv_txt = |Patch|
      iv_act = |{ yif_abapgit_definitions=>c_action-go_patch }?{ lv_param }| ).
    ro_html->add( '</td>' ).
  ...
  METHOD render_content.
    ...
    ro_html->add( ycl_abapgit_gui_chunk_lib=>render_repo_top( mo_repo ) ).
...
* make sure whitespace is preserved in the DOM
    ...
    CASE iv_context.
      WHEN 'local'.
        lv_param = ycl_abapgit_html_action_utils=>file_encode(
          iv_key  = mo_repo->get_key( )
          ig_file = is_file ).

        lv_filename = ycl_abapgit_html=>a(
          iv_txt = lv_filename
          iv_act = |{ yif_abapgit_definitions=>c_action-go_diff }?{ lv_param }| ).
  ...
  METHOD yif_abapgit_gui_page_hotkey~get_hotkey_actions.
    ...
    ls_hotkey_action-action         = yif_abapgit_definitions=>c_action-go_patch.
  ...
  METHOD yif_abapgit_gui_page~on_event.
    ...
    DATA: lo_stage  TYPE REF TO ycl_abapgit_stage,
    ...
    CASE iv_action.
      ...
      WHEN c_action-stage_filter.
        ...
        ycl_abapgit_html_action_utils=>get_field( EXPORTING iv_name  = 'filterValue'
                                                            it_field = lt_fields
                                                  CHANGING  cg_field = mv_filter_value ).

        ev_state = yif_abapgit_definitions=>c_event_state-no_more_act.

      WHEN yif_abapgit_definitions=>c_action-go_patch.                         " Go Patch page
