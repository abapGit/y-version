CLASS ycl_abapgit_gui_page_debuginfo DEFINITION
  PUBLIC
...
CLASS YCL_ABAPGIT_GUI_PAGE_DEBUGINFO IMPLEMENTATION.
  ...
  METHOD render_debug_info.
    ...
    ro_html->add( |<p>abapGit version: { yif_abapgit_version=>gc_abap_version }</p>| ).
    ro_html->add( |<p>XML version:     { yif_abapgit_version=>gc_xml_version }</p>| ).
    ro_html->add( |<p>GUI version:     { lv_gui_version }</p>| ).
    ro_html->add( |<p>LCL_TIME:        { ycl_abapgit_time=>get( ) }</p>| ).
