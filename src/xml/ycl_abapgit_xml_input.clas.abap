CLASS ycl_abapgit_xml_input DEFINITION
  PUBLIC
...
* todo, add read_xml to match add_xml in lcl_xml_output
    METHODS get_metadata
      RETURNING
        VALUE(rs_metadata) TYPE yif_abapgit_definitions=>ty_metadata .
  PRIVATE SECTION.
    METHODS: fix_xml.
...
CLASS YCL_ABAPGIT_XML_INPUT IMPLEMENTATION.
  ...
  METHOD read.
    ...
    TRY.
      ...
      CATCH cx_transformation_error INTO lx_error.
        ycx_abapgit_exception=>raise( lx_error->if_message~get_text( ) ).
    ENDTRY.
