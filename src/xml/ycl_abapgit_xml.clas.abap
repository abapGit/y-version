CLASS ycl_abapgit_xml DEFINITION
  PUBLIC
  ABSTRACT
  CREATE PUBLIC.
  ...
  PROTECTED SECTION.
    DATA: mi_ixml     TYPE REF TO if_ixml,
          ...
          ms_metadata TYPE yif_abapgit_definitions=>ty_metadata.
...
CLASS YCL_ABAPGIT_XML IMPLEMENTATION.
  ...
  METHOD display_xml_error.
    ...
    lv_version = |abapGit version: { yif_abapgit_version=>gc_abap_version }|.
    ...
    ycx_abapgit_exception=>raise( 'XML error' ).
  ...
  METHOD error.
    ...
    ycx_abapgit_exception=>raise( 'Error while parsing XML' ).
  ENDMETHOD.
  ...
  METHOD parse.
    ...
    IF li_version->get_value( ) <> yif_abapgit_version=>gc_xml_version.
      display_xml_error( ).
    ENDIF.
