CLASS ycl_abapgit_repo DEFINITION
  PUBLIC
  ABSTRACT
  CREATE PUBLIC

  GLOBAL FRIENDS ycl_abapgit_repo_srv .

  PUBLIC SECTION.

    METHODS deserialize_checks
      RETURNING
        VALUE(rs_checks) TYPE yif_abapgit_definitions=>ty_deserialize_checks
      RAISING
        ycx_abapgit_exception .
    METHODS delete_checks
      RETURNING
        VALUE(rs_checks) TYPE yif_abapgit_definitions=>ty_delete_checks
      RAISING
        ycx_abapgit_exception .
    METHODS constructor
      IMPORTING
        !is_data TYPE yif_abapgit_persistence=>ty_repo .
    METHODS get_key
      RETURNING
        VALUE(rv_key) TYPE yif_abapgit_persistence=>ty_value .
    METHODS get_name
      RETURNING
    ...
    METHODS get_files_local
      IMPORTING
        ...
        !it_filter      TYPE yif_abapgit_definitions=>ty_tadir_tt OPTIONAL
      RETURNING
        VALUE(rt_files) TYPE yif_abapgit_definitions=>ty_files_item_tt
      RAISING
        ycx_abapgit_exception .
    METHODS get_local_checksums
      RETURNING
        VALUE(rt_checksums) TYPE yif_abapgit_persistence=>ty_local_checksum_tt .
    METHODS get_local_checksums_per_file
      RETURNING
        VALUE(rt_checksums) TYPE yif_abapgit_definitions=>ty_file_signatures_tt .
    METHODS get_files_remote
      RETURNING
        VALUE(rt_files) TYPE yif_abapgit_definitions=>ty_files_tt
      RAISING
        ycx_abapgit_exception .
    METHODS get_package
      RETURNING
        VALUE(rv_package) TYPE yif_abapgit_persistence=>ty_repo-package .
    METHODS delete
      RAISING
        ycx_abapgit_exception .
    METHODS get_dot_abapgit
      RETURNING
        VALUE(ro_dot_abapgit) TYPE REF TO ycl_abapgit_dot_abapgit .
    METHODS set_dot_abapgit
      IMPORTING
        !io_dot_abapgit TYPE REF TO ycl_abapgit_dot_abapgit
      RAISING
        ycx_abapgit_exception .
    METHODS deserialize
      IMPORTING
        !is_checks TYPE yif_abapgit_definitions=>ty_deserialize_checks
      RAISING
        ycx_abapgit_exception .
    METHODS refresh
      IMPORTING
    ...
    METHODS update_local_checksums
      IMPORTING
        !it_files TYPE yif_abapgit_definitions=>ty_file_signatures_tt
      RAISING
        ycx_abapgit_exception .
    ...
    METHODS find_remote_dot_abapgit
      RETURNING
        VALUE(ro_dot) TYPE REF TO ycl_abapgit_dot_abapgit
      RAISING
        ycx_abapgit_exception .
    METHODS is_offline
      RETURNING
    ...
    METHODS set_files_remote
      IMPORTING
        !it_files TYPE yif_abapgit_definitions=>ty_files_tt
      RAISING
        ycx_abapgit_exception .
    METHODS get_local_settings
      RETURNING
        VALUE(rs_settings) TYPE yif_abapgit_persistence=>ty_repo-local_settings .
    METHODS set_local_settings
      IMPORTING
        !is_settings TYPE yif_abapgit_persistence=>ty_repo-local_settings
      RAISING
        ycx_abapgit_exception .
    METHODS run_code_inspector
      ...
      RAISING
        ycx_abapgit_exception .
  PROTECTED SECTION.
    DATA mt_local TYPE yif_abapgit_definitions=>ty_files_item_tt .
    DATA mt_remote TYPE yif_abapgit_definitions=>ty_files_tt .
    DATA mv_do_local_refresh TYPE abap_bool .
    DATA mv_last_serialization TYPE timestamp .
    DATA ms_data TYPE yif_abapgit_persistence=>ty_repo .
    DATA mv_code_inspector_successful TYPE abap_bool .

    METHODS set
      IMPORTING
        !it_checksums       TYPE yif_abapgit_persistence=>ty_local_checksum_tt OPTIONAL
        !iv_url             TYPE yif_abapgit_persistence=>ty_repo-url OPTIONAL
        !iv_branch_name     TYPE yif_abapgit_persistence=>ty_repo-branch_name OPTIONAL
        !iv_head_branch     TYPE yif_abapgit_persistence=>ty_repo-head_branch OPTIONAL
        !iv_offline         TYPE yif_abapgit_persistence=>ty_repo-offline OPTIONAL
        !is_dot_abapgit     TYPE yif_abapgit_persistence=>ty_repo-dot_abapgit OPTIONAL
        !is_local_settings  TYPE yif_abapgit_persistence=>ty_repo-local_settings OPTIONAL
        !iv_deserialized_at TYPE yif_abapgit_persistence=>ty_repo-deserialized_at OPTIONAL
        !iv_deserialized_by TYPE yif_abapgit_persistence=>ty_repo-deserialized_by OPTIONAL
      RAISING
        ycx_abapgit_exception .
...
CLASS YCL_ABAPGIT_REPO IMPLEMENTATION.
  ...
  METHOD delete.
    ...
    ycl_abapgit_persist_factory=>get_repo( )->delete( ms_data-key ).
  ...
  METHOD delete_checks.
    ...
    DATA: li_package TYPE REF TO yif_abapgit_sap_package.

    li_package = ycl_abapgit_factory=>get_sap_package( get_package( ) ).
  ...
  METHOD deserialize.
    ...
    DATA: lt_updated_files TYPE yif_abapgit_definitions=>ty_file_signatures_tt,
    ...
    IF is_checks-requirements-met = 'N' AND is_checks-requirements-decision IS INITIAL.
      ycx_abapgit_exception=>raise( 'Requirements not met and undecided ').
    ENDIF.
    ...
    TRY.
        lt_updated_files = ycl_abapgit_objects=>deserialize(
            io_repo   = me
            is_checks = is_checks ).
      CATCH ycx_abapgit_exception INTO lx_error.
* ensure to reset default transport request task
        ycl_abapgit_default_transport=>get_instance( )->reset( ).
  ...
  METHOD deserialize_checks.
    ...
    DATA: lt_requirements TYPE yif_abapgit_dot_abapgit=>ty_requirement_tt.


    find_remote_dot_abapgit( ).

    IF get_local_settings( )-write_protected = abap_true.
      ycx_abapgit_exception=>raise( 'Cannot deserialize. Local code is write-protected by repo config' ).
    ELSEIF get_dot_abapgit( )->get_master_language( ) <> sy-langu.
      ycx_abapgit_exception=>raise( 'Current login language does not match master language' ).
    ENDIF.

    rs_checks = ycl_abapgit_objects=>deserialize_checks( me ).

    lt_requirements = get_dot_abapgit( )->get_data( )-requirements.
    rs_checks-requirements-met = ycl_abapgit_requirement_helper=>is_requirements_met(
      lt_requirements ).
  ...
  METHOD find_remote_dot_abapgit.
    ...
    READ TABLE mt_remote ASSIGNING <ls_remote>
      WITH KEY path = yif_abapgit_definitions=>c_root_dir
      filename = yif_abapgit_definitions=>c_dot_abapgit.
    IF sy-subrc = 0.
      ro_dot = ycl_abapgit_dot_abapgit=>deserialize( <ls_remote>-data ).
  ...
  METHOD get_files_local.
    ...
    DATA: lt_tadir    TYPE yif_abapgit_definitions=>ty_tadir_tt,
          ...
          lt_files    TYPE yif_abapgit_definitions=>ty_files_tt,
          lo_progress TYPE REF TO ycl_abapgit_progress,
          lt_cache    TYPE SORTED TABLE OF yif_abapgit_definitions=>ty_file_item
                   WITH NON-UNIQUE KEY item.

    DATA: lt_filter       TYPE SORTED TABLE OF yif_abapgit_definitions=>ty_tadir
    ...
    <ls_return>-file-path     = yif_abapgit_definitions=>c_root_dir.
    <ls_return>-file-filename = yif_abapgit_definitions=>c_dot_abapgit.
    <ls_return>-file-data     = get_dot_abapgit( )->serialize( ).
    <ls_return>-file-sha1     = ycl_abapgit_hash=>sha1( iv_type = yif_abapgit_definitions=>c_type-blob
                                                        iv_data = <ls_return>-file-data ).

    lt_cache = mt_local.
    lt_tadir = ycl_abapgit_factory=>get_tadir( )->read(
    ...
    LOOP AT lt_tadir ASSIGNING <ls_tadir>.
      ...
      IF mv_last_serialization IS NOT INITIAL. " Try to fetch from cache
        ...
        IF sy-subrc = 0
            AND abap_false = ycl_abapgit_objects=>has_changed_since(
            is_item      = ls_item
  ...
  METHOD get_name.
    ...
    IF ms_data-offline = abap_true.
      rv_name = ms_data-url.
    ELSE.
      rv_name = ycl_abapgit_url=>name( ms_data-url ).
  ...
  METHOD rebuild_local_checksums. "LOCAL (BASE)
    ...
    DATA: lt_local     TYPE yif_abapgit_definitions=>ty_files_item_tt,
          ls_last_item TYPE yif_abapgit_definitions=>ty_item,
          lt_checksums TYPE yif_abapgit_persistence=>ty_local_checksum_tt.
    ...
    DELETE lt_local " Remove non-code related files except .abapgit
      ...
      AND NOT ( file-path     = yif_abapgit_definitions=>c_root_dir
      AND       file-filename = yif_abapgit_definitions=>c_dot_abapgit ).
  ...
  METHOD run_code_inspector.
    ...
    DATA: li_code_inspector TYPE REF TO yif_abapgit_code_inspector,
    ...
    IF lv_check_variant IS INITIAL.
      ycx_abapgit_exception=>raise( |No check variant maintained in repo settings.| ).
    ENDIF.

    li_code_inspector = ycl_abapgit_factory=>get_code_inspector(
                                  iv_package            = get_package( )
...
* TODO: refactor
    ...
    DATA: li_persistence TYPE REF TO yif_abapgit_persist_repo.
    ...
    li_persistence = ycl_abapgit_persist_factory=>get_repo( ).
  ...
  METHOD update_last_deserialize.
    ...
    DATA: lv_deserialized_at TYPE yif_abapgit_persistence=>ty_repo-deserialized_at,
          lv_deserialized_by TYPE yif_abapgit_persistence=>ty_repo-deserialized_by.
  ...
  METHOD update_local_checksums.
    ...
    DATA: lt_checksums TYPE yif_abapgit_persistence=>ty_local_checksum_tt,
          lt_files_idx TYPE yif_abapgit_definitions=>ty_file_signatures_tt,
          lt_local     TYPE yif_abapgit_definitions=>ty_files_item_tt,
          lv_chks_row  TYPE i,
          lv_file_row  TYPE i.
