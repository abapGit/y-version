...
CLASS ltcl_convert DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  ...
  PRIVATE SECTION.
    METHODS convert_int FOR TESTING RAISING ycx_abapgit_exception.
    METHODS split_string FOR TESTING.
    METHODS convert_bitbyte FOR TESTING RAISING ycx_abapgit_exception.
...
CLASS ltcl_convert IMPLEMENTATION.
  ...
  METHOD convert_bitbyte.
    ...
    DO 1000 TIMES.
      ...
      lv_xstring = ycl_abapgit_convert=>int_to_xstring4( lv_input ).
      DO 4 TIMES.
        lv_offset = sy-index - 1.
        lv_byte = lv_xstring+lv_offset(1).
        lv_bitbyte = ycl_abapgit_convert=>x_to_bitbyte( lv_byte ).
        lv_byteint = ycl_abapgit_convert=>bitbyte_to_int( lv_bitbyte ).
        lv_xbyteint = lv_byteint.
        CONCATENATE lv_xresult lv_xbyteint INTO lv_xresult
          IN BYTE MODE.
      ENDDO.
      lv_result = ycl_abapgit_convert=>xstring_to_int( lv_xresult ).
  ...
  METHOD convert_int.
    ...
    DO 1000 TIMES.
      lv_input = sy-index.
      lv_xstring = ycl_abapgit_convert=>int_to_xstring4( lv_input ).
      lv_result = ycl_abapgit_convert=>xstring_to_int( lv_xstring ).
  ...
  METHOD split_string.
    ...
    lt_act = ycl_abapgit_convert=>split_string( 'ABC' && cl_abap_char_utilities=>cr_lf && '123' ).
    ...
    lt_act = ycl_abapgit_convert=>split_string( 'ABC' && cl_abap_char_utilities=>newline && '123' ).
