CLASS ycl_abapgit_requirement_helper DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS is_requirements_met
      IMPORTING
        !it_requirements TYPE yif_abapgit_dot_abapgit=>ty_requirement_tt
      RETURNING
        VALUE(rv_status) TYPE yif_abapgit_definitions=>ty_yes_no
      RAISING
        ycx_abapgit_exception .
  PRIVATE SECTION.
...
CLASS YCL_ABAPGIT_REQUIREMENT_HELPER IMPLEMENTATION.
  ...
  METHOD get_requirement_met_status.
    ...
    CALL FUNCTION 'DELIVERY_GET_INSTALLED_COMPS'
      ...
      EXCEPTIONS
        ...
        OTHERS           = 2.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error from DELIVERY_GET_INSTALLED_COMPS { sy-subrc }| ) ##no_text.
    ENDIF.
  ...
  METHOD requirements_popup.
    ...
    CALL FUNCTION 'POPUP_TO_CONFIRM'
      ...
      IMPORTING
        answer        = lv_answer.
    IF lv_answer <> '1'.
      ycx_abapgit_exception=>raise( 'Cancelling because of unmet requirements.' ).
    ENDIF.
...
*        lo_column->set_fixed_header_text( 'S' ).
      ...
      CATCH cx_salv_msg cx_salv_not_found cx_salv_data_error INTO lx_ex.
        ycx_abapgit_exception=>raise( lx_ex->get_text( ) ).
    ENDTRY.
