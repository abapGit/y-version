CLASS ycl_abapgit_persistence_repo DEFINITION
  PUBLIC
  CREATE PROTECTED

  GLOBAL FRIENDS ycl_abapgit_persist_factory .

  PUBLIC SECTION.

    INTERFACES yif_abapgit_persist_repo .

    METHODS constructor .
  PROTECTED SECTION.

    ALIASES list
      FOR yif_abapgit_persist_repo~list .
    ALIASES read
      FOR yif_abapgit_persist_repo~read .
  PRIVATE SECTION.

    DATA mo_db TYPE REF TO ycl_abapgit_persistence_db .

    METHODS from_xml
      IMPORTING
        !iv_repo_xml_string TYPE string
      RETURNING
        VALUE(rs_repo)      TYPE yif_abapgit_persistence=>ty_repo_xml
      RAISING
        ycx_abapgit_exception .
    METHODS to_xml
      IMPORTING
        !is_repo                  TYPE yif_abapgit_persistence=>ty_repo
      RETURNING
        VALUE(rv_repo_xml_string) TYPE string .
    METHODS get_next_id
      RETURNING
        VALUE(rv_next_repo_id) TYPE yif_abapgit_persistence=>ty_content-value
      RAISING
        ycx_abapgit_exception .
ENDCLASS.
...
CLASS YCL_ABAPGIT_PERSISTENCE_REPO IMPLEMENTATION.


  METHOD constructor.
    mo_db = ycl_abapgit_persistence_db=>get_instance( ).
  ENDMETHOD.
...
* automatic migration of old fields
    ...
    IF rs_repo IS INITIAL.
      ycx_abapgit_exception=>raise( 'Inconsistent repo metadata' ).
    ENDIF.
...
* however the current approach will most likely work in almost all cases
    ...
    DATA: lt_content TYPE yif_abapgit_persistence=>tt_content.
    ...
    lt_content = mo_db->list_by_type( ycl_abapgit_persistence_db=>c_type_repo ).
  ...
  METHOD to_xml.
    ...
    DATA: ls_xml TYPE yif_abapgit_persistence=>ty_repo_xml.
  ...
  METHOD yif_abapgit_persist_repo~add.
    ...
    mo_db->add( iv_type  = ycl_abapgit_persistence_db=>c_type_repo
                iv_value = rv_key
  ...
  METHOD yif_abapgit_persist_repo~delete.

    DATA: lo_background TYPE REF TO ycl_abapgit_persist_background.

    CREATE OBJECT lo_background.
    lo_background->delete( iv_key ).

    mo_db->delete( iv_type  = ycl_abapgit_persistence_db=>c_type_repo
                   iv_value = iv_key ).

  ENDMETHOD.


  METHOD yif_abapgit_persist_repo~list.
  ...
  METHOD yif_abapgit_persist_repo~lock.

    mo_db->lock( iv_mode  = iv_mode
                 iv_type  = ycl_abapgit_persistence_db=>c_type_repo
                 iv_value = iv_key ).

  ENDMETHOD.


  METHOD yif_abapgit_persist_repo~read.

    DATA lt_repo TYPE yif_abapgit_persistence=>tt_repo.
  ...
  METHOD yif_abapgit_persist_repo~update_branch_name.

    DATA: lt_content TYPE yif_abapgit_persistence=>tt_content,
          ls_content LIKE LINE OF lt_content,
          ls_repo    TYPE yif_abapgit_persistence=>ty_repo.
    ...
    TRY.
      ...
      CATCH ycx_abapgit_not_found.
        ycx_abapgit_exception=>raise( 'key not found' ).
    ENDTRY.

    ls_repo-branch_name = iv_branch_name.
    ls_content-data_str = to_xml( ls_repo ).

    mo_db->update( iv_type  = ycl_abapgit_persistence_db=>c_type_repo
                   iv_value = iv_key
  ...
  METHOD yif_abapgit_persist_repo~update_deserialized.
  ...
  METHOD yif_abapgit_persist_repo~update_dot_abapgit.
  ...
  METHOD yif_abapgit_persist_repo~update_head_branch.
  ...
  METHOD yif_abapgit_persist_repo~update_local_checksums.
  ...
  METHOD yif_abapgit_persist_repo~update_local_settings.
  ...
  METHOD yif_abapgit_persist_repo~update_offline.
  ...
  METHOD yif_abapgit_persist_repo~update_url.
    ...
    IF iv_url IS INITIAL.
      ycx_abapgit_exception=>raise( 'update, url empty' ).
    ENDIF.
