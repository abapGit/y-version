CLASS ycl_abapgit_persistence_db DEFINITION
  PUBLIC
  CREATE PRIVATE .

  PUBLIC SECTION.
    CONSTANTS c_tabname TYPE tabname VALUE 'YABAPGIT' ##NO_TEXT.
    CONSTANTS c_lock TYPE viewname VALUE 'EYABAPGIT' ##NO_TEXT.

    CONSTANTS:
      c_type_settings   TYPE yif_abapgit_persistence=>ty_type VALUE 'SETTINGS' ##NO_TEXT,
      c_type_repo       TYPE yif_abapgit_persistence=>ty_type VALUE 'REPO' ##NO_TEXT,
      c_type_background TYPE yif_abapgit_persistence=>ty_type VALUE 'BACKGROUND' ##NO_TEXT,
      c_type_user       TYPE yif_abapgit_persistence=>ty_type VALUE 'USER' ##NO_TEXT.

    CLASS-METHODS get_instance
      RETURNING
        VALUE(ro_db) TYPE REF TO ycl_abapgit_persistence_db .
    METHODS add
      IMPORTING
        !iv_type  TYPE yif_abapgit_persistence=>ty_type
        !iv_value TYPE yif_abapgit_persistence=>ty_content-value
        !iv_data  TYPE yif_abapgit_persistence=>ty_content-data_str
      RAISING
    ...
    METHODS read
      IMPORTING
        ...
        !iv_value      TYPE yif_abapgit_persistence=>ty_content-value
      RETURNING
        VALUE(rv_data) TYPE yif_abapgit_persistence=>ty_content-data_str
      RAISING
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA go_db TYPE REF TO ycl_abapgit_persistence_db .
...
CLASS ycl_abapgit_persistence_db IMPLEMENTATION.


  METHOD add.

    DATA ls_table TYPE yif_abapgit_persistence=>ty_content.
  ...
  METHOD delete.
    ...
    DELETE FROM (c_tabname)
      ...
      AND value = iv_value.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'DB Delete failed' ).
    ENDIF.
  ...
  METHOD lock.
    ...
    CALL FUNCTION 'ENQUEUE_EYABAPGIT'
      EXPORTING
        mode_yabapgit  = iv_mode
        type           = iv_type
        value          = iv_value
      EXCEPTIONS
        foreign_lock   = 1
        ...
        OTHERS         = 3.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Could not aquire lock { iv_type } { iv_value }| ).
    ENDIF.
...
* trigger dummy update task to automatically release locks at commit
  ...
  METHOD modify.
    ...
    DATA: ls_content TYPE yif_abapgit_persistence=>ty_content.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'DB modify failed' ).
    ENDIF.
  ...
  METHOD update.
    ...
    UPDATE (c_tabname) SET data_str = lv_data
      ...
      AND   value = iv_value.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'DB update failed' ).
    ENDIF.
  ...
  METHOD validate_and_unprettify_xml.
    ...
    rv_xml = ycl_abapgit_xml_pretty=>print(
      iv_xml           = iv_xml
