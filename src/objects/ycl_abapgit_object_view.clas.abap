CLASS ycl_abapgit_object_view DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS YCL_ABAPGIT_OBJECT_VIEW IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from RS_DD_DELETE_OBJ, VIEW' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    CALL FUNCTION 'DDIF_VIEW_PUT'
      ...
      EXCEPTIONS
        ...
        OTHERS            = 6.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DDIF_VIEW_PUT' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    CALL FUNCTION 'DDIF_VIEW_GET'
      ...
      EXCEPTIONS
        ...
        OTHERS        = 2.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DDIF_VIEW_GET' ).
    ENDIF.
