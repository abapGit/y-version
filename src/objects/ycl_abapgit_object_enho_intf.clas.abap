CLASS ycl_abapgit_object_enho_intf DEFINITION PUBLIC.
  ...
  PRIVATE SECTION.
    DATA: ms_item  TYPE yif_abapgit_definitions=>ty_item,
          mo_files TYPE REF TO ycl_abapgit_objects_files.

ENDCLASS.

CLASS ycl_abapgit_object_enho_intf IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object_enho~serialize.
    ...
    ycl_abapgit_object_enho_clif=>serialize(
      io_xml  = io_xml
      io_files = mo_files
      io_clif = lo_enh_intf ).
  ...
  METHOD yif_abapgit_object_enho~deserialize.
    ...
    TRY.
        ...
        ycl_abapgit_object_enho_clif=>deserialize(
          io_xml  = io_xml
          io_clif = lo_enh_intf ).
      ...
      CATCH cx_enh_root.
        ycx_abapgit_exception=>raise( 'error deserializing ENHO interface' ).
    ENDTRY.
