CLASS ycl_abapgit_object_sxci DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS YCL_ABAPGIT_OBJECT_SXCI IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SXO_IMPL_DELETE' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SXO_BADI_READ' ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SXO_IMPL_SAVE' ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SXO_IMPL_ACTIVE' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SXV_EXIT_FOR_IMP' ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SXO_IMPL_FOR_BADI_READ' ).
    ENDIF.
