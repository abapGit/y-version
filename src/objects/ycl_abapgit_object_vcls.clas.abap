CLASS ycl_abapgit_object_vcls DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS YCL_ABAPGIT_OBJECT_VCLS IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    CALL FUNCTION 'OBJ_GENERATE'
      ...
      EXCEPTIONS
        ...
        OTHERS                = 6.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error in OBJ_GENERATE for VCLS' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    CALL FUNCTION 'VIEWCLUSTER_GET_DEFINITION'
      ...
      EXCEPTIONS
        ...
        OTHERS                 = 3.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error in VIEWCLUSTER_GET_DEFINITION' ).
    ENDIF.
