CLASS ycl_abapgit_oo_base DEFINITION PUBLIC ABSTRACT.
  PUBLIC SECTION.
    INTERFACES: yif_abapgit_oo_object_fnc.

  PROTECTED SECTION.
    CLASS-METHODS:
      convert_attrib_to_vseoattrib
        IMPORTING iv_clsname           TYPE seoclsname
                  it_attributes        TYPE yif_abapgit_definitions=>ty_obj_attribute_tt
...
CLASS ycl_abapgit_oo_base IMPLEMENTATION.
  ...
  METHOD deserialize_abap_source_new.
    ...
    TRY.
      ...
      CATCH cx_oo_access_permission.
        ycx_abapgit_exception=>raise( 'source_new, access permission exception' ).
    ENDTRY.
  ...
  METHOD deserialize_abap_source_old.
    ...
    TRY.
      ...
      CATCH cx_oo_access_permission.
        ycx_abapgit_exception=>raise( 'permission error' ).
      CATCH cx_oo_source_save_failure.
        ycx_abapgit_exception=>raise( 'save failure' ).
    ENDTRY.

  ENDMETHOD.


  METHOD yif_abapgit_oo_object_fnc~add_to_activation_list.
    ycl_abapgit_objects_activation=>add_item( is_item ).
  ENDMETHOD.
  ...
  METHOD yif_abapgit_oo_object_fnc~create_documentation.
    CALL FUNCTION 'DOCU_UPD'
      ...
      EXCEPTIONS
        ...
        OTHERS   = 2.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DOCU_UPD' ).
    ENDIF.
  ENDMETHOD.


  METHOD yif_abapgit_oo_object_fnc~create_sotr.
    ASSERT 0 = 1. "Subclass responsibility
  ENDMETHOD.
  ...
  METHOD yif_abapgit_oo_object_fnc~deserialize_source.
    TRY.
  ...
  METHOD yif_abapgit_oo_object_fnc~exists.
  ...
  METHOD yif_abapgit_oo_object_fnc~generate_locals.
    ASSERT 0 = 1. "Subclass responsibility
  ENDMETHOD.


  METHOD yif_abapgit_oo_object_fnc~get_class_properties.
    ASSERT 0 = 1. "Subclass responsibility
  ENDMETHOD.
  ...
  METHOD yif_abapgit_oo_object_fnc~get_skip_test_classes.
    rv_skip = mv_skip_test_classes.
  ENDMETHOD.


  METHOD yif_abapgit_oo_object_fnc~insert_text_pool.
    ASSERT 0 = 1. "Subclass responsibility
  ENDMETHOD.


  METHOD yif_abapgit_oo_object_fnc~read_descriptions.
  ...
  METHOD yif_abapgit_oo_object_fnc~read_documentation.
  ...
  METHOD yif_abapgit_oo_object_fnc~read_sotr.
    ASSERT 0 = 1. "Subclass responsibility
  ENDMETHOD.


  METHOD yif_abapgit_oo_object_fnc~read_superclass.
    SELECT SINGLE refclsname FROM vseoextend INTO rv_superclass
      WHERE clsname = iv_classname.
  ENDMETHOD.


  METHOD yif_abapgit_oo_object_fnc~read_text_pool.
    ASSERT 0 = 1. "Subclass responsibility
  ENDMETHOD.


  METHOD yif_abapgit_oo_object_fnc~serialize_abap.
    DATA lo_oo_serializer TYPE REF TO ycl_abapgit_oo_serializer.
