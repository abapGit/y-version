CLASS ycl_abapgit_object_tobj DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_tobj IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    IF sy-subrc = 1.
      RETURN.
    ELSEIF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from CTO_OBJECT_GET' ).
    ENDIF.
...
* activated - fixed with late deserialization
      ycx_abapgit_exception=>raise( 'error from OBJ_GENERATE' ).
    ENDIF.
    ...
    CALL FUNCTION 'OBJ_SET_IMPORTABLE'
      ...
      EXCEPTIONS
        ...
        OTHERS                = 5.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from OBJ_SET_IMPORTABLE' ).
    ENDIF.
...
* locking has been done in OBJ_SET_IMPORTABLE plus recording of transport
  ...
  METHOD yif_abapgit_object~jump.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from ABAP4_CALL_TRANSACTION, TOBJ' ).
    ENDIF.
