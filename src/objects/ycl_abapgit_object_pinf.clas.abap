CLASS ycl_abapgit_object_pinf DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_pinf IMPLEMENTATION.
    ...
    IF yif_abapgit_object~exists( ) = abap_false.
      cl_package_interface=>create_new_package_interface(
        ...
        EXCEPTIONS
          ...
          OTHERS                  = 7 ).
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error creating new package interface' ).
      ENDIF.
    ELSE.
      cl_package_interface=>load_package_interface(
        EXPORTING
        ...
        EXCEPTIONS
          ...
          OTHERS                     = 6 ).
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error loading package interface' ).
      ENDIF.
    ENDIF.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    cl_package_interface=>load_package_interface(
      ...
      EXCEPTIONS
        ...
        OTHERS                     = 6 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error loading package interface, delete' ).
    ENDIF.
