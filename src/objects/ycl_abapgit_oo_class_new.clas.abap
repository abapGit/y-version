CLASS ycl_abapgit_oo_class_new DEFINITION PUBLIC INHERITING FROM ycl_abapgit_oo_class.

  PUBLIC SECTION.
    METHODS:
      yif_abapgit_oo_object_fnc~create REDEFINITION,
      yif_abapgit_oo_object_fnc~generate_locals REDEFINITION,
      yif_abapgit_oo_object_fnc~deserialize_source REDEFINITION.
...
CLASS ycl_abapgit_oo_class_new IMPLEMENTATION.
  ...
  METHOD determine_method_include.
    ...
    CALL FUNCTION 'SEO_METHOD_GENERATE_INCLUDE'
      ...
      EXCEPTIONS
        ...
        OTHERS                         = 12.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SEO_METHOD_GENERATE_INCLUDE' ).
    ENDIF.
  ...
  METHOD generate_classpool.
    ...
    CALL FUNCTION 'SEO_CLASS_GENERATE_CLASSPOOL'
      ...
      EXCEPTIONS
        ...
        OTHERS                        = 20.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SEO_CLASS_GENERATE_CLASSPOOL' ).
    ENDIF.
  ...
  METHOD init_scanner.
    ...
    TRY.
      ...
      CATCH cx_clif_scan_error.
        ycx_abapgit_exception=>raise( 'error initializing CLAS scanner' ).
    ENDTRY.
...
* this will READ REPORT instead of using it_source, which should be okay
    ...
    ENDTRY.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error instantiating CL_OO_CLASS_SECTION_SOURCE' ).
    ENDIF.
...
* AMDP not supported in this system, ignore error
    ...
    IF sy-subrc <> 0 OR lv_scan_error = abap_true.
      ycx_abapgit_exception=>raise( 'CLAS, error while scanning source' ).
    ENDIF.
...
* this will update the SEO* database tables
  ...
  METHOD update_report.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Fatal error. Include { iv_program } should have been created previously!| ).
    ENDIF.
    ...
    LOOP AT lt_methods INTO lv_method.
      TRY.
        ...
        CATCH cx_oo_clif_component.
          ycx_abapgit_exception=>raise( 'error from GET_METHOD_IMPL_SOURCE' ).
      ENDTRY.
