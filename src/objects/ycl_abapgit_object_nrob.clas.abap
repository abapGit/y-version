CLASS ycl_abapgit_object_nrob DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_nrob IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    IF sy-subrc = 1.
      RETURN.
    ELSEIF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from NUMBER_RANGE_OBJECT_READ' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    CALL FUNCTION 'NUMBER_RANGE_OBJECT_UPDATE'
      ...
      EXCEPTIONS
        ...
        OTHERS                    = 6.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from NUMBER_RANGE_OBJECT_UPDATE' ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from NUMBER_RANGE_OBJECT_CLOSE' ).
    ENDIF.
  ...
  METHOD delete_intervals.
    ...
    CALL FUNCTION 'NUMBER_RANGE_INTERVAL_LIST'
      ...
      EXCEPTIONS
        ...
        OTHERS                     = 9.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from NUMBER_RANGE_INTERVAL_LIST' ).
    ENDIF.
    ...
    IF sy-subrc <> 0 OR lv_error = abap_true.
      ycx_abapgit_exception=>raise( 'error from NUMBER_RANGE_INTERVAL_UPDATE' ).
    ENDIF.
    ...
    CALL FUNCTION 'NUMBER_RANGE_UPDATE_CLOSE'
      ...
      EXCEPTIONS
        ...
        OTHERS                 = 3.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from NUMBER_RANGE_UPDATE_CLOSE' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    CALL FUNCTION 'NUMBER_RANGE_OBJECT_DELETE'
      ...
      EXCEPTIONS
        ...
        OTHERS             = 4.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from NUMBER_RANGE_OBJECT_DELETE' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~jump.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from ABAP4_CALL_TRANSACTION, NROB' ).
    ENDIF.
