CLASS ycl_abapgit_object_smim DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_url_for_io
      ...
      RAISING   ycx_abapgit_not_found
                ycx_abapgit_exception.

ENDCLASS.

CLASS ycl_abapgit_object_smim IMPLEMENTATION.
  ...
  METHOD find_content.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'SMIM, file not found' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    IF lv_folder = abap_false.
      ...
      IF sy-subrc <> 0 AND sy-subrc <> 2 AND sy-subrc <> 3.
        ycx_abapgit_exception=>raise( 'error from mime api->get:' && sy-msgv1 ).
      ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    IF lv_folder = abap_true.
      ...
      IF sy-subrc <> 5 AND sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error frrom SMIM create_folder' ).
      ENDIF.
    ELSE.
      lv_filename = get_filename( lv_url ).
      ...
      li_api->put(
        ...
        EXCEPTIONS
          ...
          OTHERS                  = 8 ).
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from SMIM put' ).
      ENDIF.
    ENDIF.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    li_api->delete(
      ...
      EXCEPTIONS
        ...
        OTHERS             = 6 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from delete' ).
    ENDIF.
