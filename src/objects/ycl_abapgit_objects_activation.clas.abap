CLASS ycl_abapgit_objects_activation DEFINITION PUBLIC CREATE PUBLIC.
...
CLASS YCL_ABAPGIT_OBJECTS_ACTIVATION IMPLEMENTATION.
  ...
  METHOD activate_ddic.
    ...
    LOOP AT gt_objects ASSIGNING <ls_object>.
      ...
      IF sy-subrc <> 0.
        ...
        ycx_abapgit_exception=>raise( lv_errmsg ).
      ENDIF.
    ...
    IF lt_gentab IS NOT INITIAL.
      ...
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from DD_MASS_ACT_C3' ).
      ENDIF.
    ...
    IF gt_objects IS NOT INITIAL.
      ...
      CALL FUNCTION 'RS_WORKING_OBJECTS_ACTIVATE'
        ...
        EXCEPTIONS
          ...
          OTHERS                 = 4.
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from RS_WORKING_OBJECTS_ACTIVATE' ).
      ENDIF.
...
* todo, move this to the object type include instead
        CALL FUNCTION 'RS_INACTIVE_OBJECTS_IN_OBJECT'
          ...
          EXCEPTIONS
            ...
            OTHERS           = 2.
        IF sy-subrc <> 0.
          ycx_abapgit_exception=>raise( 'Error from RS_INACTIVE_OBJECTS_IN_OBJECT' ).
        ENDIF.
...
* TODO, this class is obsolete with new CLAS deserialization logic
  ...
  METHOD show_activation_errors.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from TR_READ_LOG' ).
    ENDIF.
