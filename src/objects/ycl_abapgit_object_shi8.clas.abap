CLASS ycl_abapgit_object_shi8 DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_shi8 IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~jump.
    ycx_abapgit_exception=>raise( |TODO: Jump SHI8| ).
  ENDMETHOD.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error serializing { ms_item-obj_type } { ms_item-obj_name  }| ).
    ENDIF.
