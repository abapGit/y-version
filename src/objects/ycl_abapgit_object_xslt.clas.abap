CLASS ycl_abapgit_object_xslt DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_xslt IMPLEMENTATION.
  ...
  METHOD get.
    ...
    cl_o2_api_xsltdesc=>load(
      ...
      EXCEPTIONS
        ...
        OTHERS             = 3 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from cl_o2_api_xsltdesc=>load' ).
    ENDIF.
...
* workaround: somewhere additional linefeeds are added
    ...
    cl_o2_api_xsltdesc=>create_new_from_string(
      ...
      EXCEPTIONS
        ...
        OTHERS                  = 6 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |error from cl_o2_api_xsltdesc=>create_new_from_string, { sy-subrc }| ).
    ENDIF.
