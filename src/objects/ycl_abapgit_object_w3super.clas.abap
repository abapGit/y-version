CLASS ycl_abapgit_object_w3super DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super ABSTRACT.
...
CLASS ycl_abapgit_object_w3super IMPLEMENTATION.
  ...
  METHOD find_param.
    ...
    IF sy-subrc > 0.
      ycx_abapgit_exception=>raise( |W3xx: Cannot find { iv_name } for { ms_key-objid }| ).
    ENDIF.
  ...
  METHOD strip_params.
    ...
    <ls_param>-value = ycl_abapgit_path=>get_filename_from_syspath( |{ <ls_param>-value }| ).
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF sy-subrc IS NOT INITIAL.
      ycx_abapgit_exception=>raise( 'Cannot delete W3xx data' ).
    ENDIF.
    ...
    IF sy-subrc IS NOT INITIAL.
      ycx_abapgit_exception=>raise( 'Cannot delete W3xx params' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    CASE io_xml->get_metadata( )-version.
      ...
      WHEN 'v2.0.0'.
        lv_xstring = yif_abapgit_object~mo_files->read_raw( iv_extra = 'data'
                                                    iv_ext   = get_ext( lt_w3params ) ).
      WHEN OTHERS.
        ycx_abapgit_exception=>raise( 'W3xx: Unknown serializer version' ).
    ENDCASE.
      ...
      WHEN 'HT'.
        ...
        CALL FUNCTION 'SCMS_BINARY_TO_TEXT'
          ...
          EXCEPTIONS
            failed        = 1.
        IF sy-subrc IS NOT INITIAL.
          ycx_abapgit_exception=>raise( 'Cannot update W3xx params' ).
        ENDIF.

        CLEAR lt_w3mime.
      WHEN OTHERS.
        ycx_abapgit_exception=>raise( 'Wrong W3xx type' ).
    ENDCASE.
    ...
    IF sy-subrc IS NOT INITIAL.
      ycx_abapgit_exception=>raise( 'Cannot upload W3xx data' ).
    ENDIF.
    ...
    IF sy-subrc IS NOT INITIAL.
      ycx_abapgit_exception=>raise( 'Cannot update TADIR for W3xx' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    IF sy-subrc IS NOT INITIAL.
      ycx_abapgit_exception=>raise( 'Cannot read W3xx data' ).
    ENDIF.
    ...
    IF sy-subrc IS NOT INITIAL.
      ycx_abapgit_exception=>raise( 'Cannot convert W3xx to xstring' ).
    ENDIF.
    ...
    yif_abapgit_object~mo_files->add_raw( iv_data  = lv_xstring
                                  iv_extra = 'data'
                                  iv_ext   = get_ext( lt_w3params ) ).
