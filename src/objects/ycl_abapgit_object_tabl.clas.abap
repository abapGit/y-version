CLASS ycl_abapgit_object_tabl DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_tabl IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~compare_to_remote_version.
    DATA: lo_table_validation     TYPE REF TO ycl_abapgit_object_tabl_valid,
          lo_local_version_output TYPE REF TO ycl_abapgit_xml_output,
          lo_local_version_input  TYPE REF TO ycl_abapgit_xml_input,
          lv_validation_text      TYPE string.

    CREATE OBJECT lo_local_version_output.
    me->yif_abapgit_object~serialize( lo_local_version_output ).
    ...
    IF lv_validation_text IS NOT INITIAL.
      ...
      CREATE OBJECT ro_comparison_result TYPE ycl_abapgit_object_tabl_dialog
        EXPORTING
...
* it cannot delete table with table wihtout asking
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from RS_DD_DELETE_OBJ, TABL' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    CALL FUNCTION 'DDIF_TABL_PUT'
      ...
      EXCEPTIONS
        ...
        OTHERS            = 6.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DDIF_TABL_PUT' ).
    ENDIF.
...
* todo, call corr_insert?
      ...
      CALL FUNCTION 'DDIF_INDX_PUT'
        ...
        EXCEPTIONS
          ...
          OTHERS            = 6.
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from DDIF_INDX_PUT' ).
      ENDIF.
      ...
      ycl_abapgit_objects_activation=>add( iv_type = 'INDX'
                                           iv_name = lv_tname ).
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    CALL FUNCTION 'DDIF_TABL_GET'
      ...
      EXCEPTIONS
        ...
        OTHERS        = 2.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DDIF_TABL_GET' ).
    ENDIF.
