CLASS ycl_abapgit_object_doma DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_doma IMPLEMENTATION.
  ...
  METHOD deserialize_texts.
    ...
    LOOP AT lt_i18n_langs ASSIGNING <lv_lang>.
      ...
      IF sy-subrc > 0.
        ycx_abapgit_exception=>raise( |DD01_TEXTS cannot find lang { <lv_lang> } in XML| ).
      ENDIF.
      ...
      CALL FUNCTION 'DDIF_DOMA_PUT'
        ...
        EXCEPTIONS
          ...
          OTHERS            = 6.
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from DDIF_DOMA_PUT @TEXTS' ).
      ENDIF.
    ENDLOOP.
...
* see class CL_WB_DDIC
    ...
    TRY.
        ...
        IF sy-subrc <> 0.
          ycx_abapgit_exception=>raise( 'error from RS_DD_DELETE_OBJ, DOMA' ).
        ENDIF.
...
* fm RS_CORR_INSERT ?
    ...
    CALL FUNCTION 'DDIF_DOMA_PUT'
      ...
      EXCEPTIONS
        ...
        OTHERS            = 6.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DDIF_DOMA_PUT' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    IF sy-subrc <> 0 OR ls_dd01v IS INITIAL.
      ycx_abapgit_exception=>raise( 'error from DDIF_DOMA_GET' ).
    ENDIF.
