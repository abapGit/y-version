CLASS ycl_abapgit_object_char DEFINITION
  PUBLIC
...
CLASS YCL_ABAPGIT_OBJECT_CHAR IMPLEMENTATION.
  ...
  METHOD instantiate_char.
    ...
    TRY.
      ...
      CATCH cx_pak_invalid_data
        ...
        ycx_abapgit_exception=>raise( 'Error while instantiating CL_CLS_ATTRIBUTE' ).
    ENDTRY.
    ...
    IF lv_new = abap_false.
      TRY.
        ...
        CATCH cx_pak_invalid_data
            ...
            cx_pak_invalid_state
            cx_pak_wb_object_locked.
          ycx_abapgit_exception=>raise( |Could not aquire lock, CHAR { lv_name }| ).
      ENDTRY.
    ENDIF.
  ...
  METHOD yif_abapgit_object~jump.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error from RS_TOOL_ACCESS, CHAR| ).
    ENDIF.
