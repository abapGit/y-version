CLASS ycl_abapgit_objects_saxx_super DEFINITION PUBLIC ABSTRACT
...
CLASS ycl_abapgit_objects_saxx_super IMPLEMENTATION.
  ...
  METHOD create_channel_objects.
    ...
    TRY.
      ...
      CATCH cx_root.
        ycx_abapgit_exception=>raise( |{ ms_item-obj_type } not supported| ).
    ENDTRY.
  ...
  METHOD lock.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error occured while locking { ms_item-obj_type } | && lv_objname ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~changed_by.
    ...
    TRY.
      ...
      CATCH cx_root.
        ycx_abapgit_exception=>raise( |{ ms_item-obj_name } not supported| ).
    ENDTRY.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    TRY.
      ...
      CATCH cx_swb_exception.
        ycx_abapgit_exception=>raise( |Error occured while deleting { ms_item-obj_type }| ).
    ENDTRY.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    TRY.
        ...
        IF sy-subrc <> 0.
          ycx_abapgit_exception=>raise( |Error occured while creating { ms_item-obj_type }| ).
        ENDIF.
