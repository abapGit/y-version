CLASS ycl_abapgit_object_ssfo DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_ssfo IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF sy-subrc <> 0 AND sy-subrc <> 2.
      ycx_abapgit_exception=>raise( 'Error from FB_DELETE_FORM' ).
    ENDIF.
...
* set "created by" and "changed by" to current user
    ...
    TRY.
      ...
      CATCH cx_ssf_fb INTO lx_error.
        ...
        ycx_abapgit_exception=>raise( |{ ms_item-obj_type } { ms_item-obj_name }: { lv_text } | ).
    ENDTRY.
