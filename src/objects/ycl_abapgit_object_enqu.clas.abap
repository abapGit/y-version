CLASS ycl_abapgit_object_enqu DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS YCL_ABAPGIT_OBJECT_ENQU IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from RS_DD_DELETE_OBJ, ENQU' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    CALL FUNCTION 'DDIF_ENQU_PUT'
      ...
      EXCEPTIONS
        ...
        OTHERS            = 6.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DDIF_ENQU_PUT' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    CALL FUNCTION 'DDIF_ENQU_GET'
      ...
      EXCEPTIONS
        ...
        OTHERS        = 2.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DDIF_ENQU_GET' ).
    ENDIF.
