CLASS ycl_abapgit_object_webi DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS YCL_ABAPGIT_OBJECT_WEBI IMPLEMENTATION.
...
* set_data, and looking at the cluster data LS_ENDPOINT-CLUSTD
      ycx_abapgit_exception=>raise( 'todo, WEBI BAPI' ).
    ENDIF.

    IF lines( is_webi-pvepfunction ) <> 1.
      ycx_abapgit_exception=>raise( 'todo, WEBI, function name' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    TRY.
      ...
      CATCH cx_ws_md_exception.
        ycx_abapgit_exception=>raise( 'error deleting WEBI' ).
    ENDTRY.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    TRY.
      ...
      CATCH cx_ws_md_exception INTO lx_root.
        ...
        ycx_abapgit_exception=>raise( 'error deserializing WEBI' ).
    ENDTRY.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    CALL FUNCTION 'WEBI_GET_OBJECT'
      ...
      EXCEPTIONS
        ...
        OTHERS            = 3.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from WEBI_GET_OBJECT' ).
    ENDIF.
    ...
    TRY.
      ...
      CATCH cx_ws_md_exception.
        ycx_abapgit_exception=>raise( 'error serializing WEBI' ).
    ENDTRY.
