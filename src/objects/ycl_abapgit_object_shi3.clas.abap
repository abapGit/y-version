CLASS ycl_abapgit_object_shi3 DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_shi3 IMPLEMENTATION.
  ...
  METHOD jump_se43.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from ABAP4_CALL_TRANSACTION, SHI3' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~jump.
    ...
    CASE ls_head-type.
      WHEN 'BMENU'.
        jump_se43( ).
      WHEN OTHERS.
        ycx_abapgit_exception=>raise( |Jump for type { ls_head-type } not implemented| ).
    ENDCASE.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    CALL FUNCTION 'BMENU_DELETE_TREE'
      ...
      EXCEPTIONS
        ...
        OTHERS             = 4.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from BMENU_DELETE_TREE, SHI3' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    CALL FUNCTION 'STREE_HIERARCHY_SAVE'
      ...
      EXCEPTIONS
        ...
        OTHERS                   = 2.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'Error from STREE_HIERARCHY_SAVE, SHI3' ).
    ENDIF.
