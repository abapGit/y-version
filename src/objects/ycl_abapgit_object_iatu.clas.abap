CLASS ycl_abapgit_object_iatu DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_iatu IMPLEMENTATION.
  ...
  METHOD read.
    ...
    cl_w3_api_template=>if_w3_api_template~load(
      ...
      EXCEPTIONS
        ...
        OTHERS              = 4 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from w3api_template~load' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    cl_w3_api_template=>if_w3_api_template~load(
      ...
      EXCEPTIONS
        ...
        OTHERS              = 4 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from if_w3_api_template~load' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~exists.
    ...
    IF sy-subrc = 1.
      rv_bool = abap_false.
    ELSEIF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from w3_api_template~load' ).
    ELSE.
      rv_bool = abap_true.
    ENDIF.
