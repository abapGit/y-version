CLASS ycl_abapgit_object_cus2 DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_cus2 IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF ls_message-msgty <> 'S'.
      ycx_abapgit_exception=>raise( |error from delete CUS2 { mv_img_attribute } S_CUS_ATTRIBUTES_DELETE| ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    IF ls_message-msgty <> 'S'.
      ycx_abapgit_exception=>raise( |error from deserialize CUS2 { mv_img_attribute } S_CUS_ATTRIBUTES_SAVE| ).
    ENDIF.
