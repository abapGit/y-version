CLASS ycl_abapgit_object_acid DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_acid IMPLEMENTATION.
  ...
  METHOD create_object.
    ...
    CREATE OBJECT ro_aab
      ...
      EXCEPTIONS
        ...
        OTHERS           = 2.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error creating CL_AAB_ID object' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    lo_aab->delete(
      EXCEPTIONS
        ...
        OTHERS           = 10 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error deleting ACID object' ).
    ENDIF.
    lo_aab->dequeue( ).
