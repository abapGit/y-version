CLASS ycl_abapgit_object_sicf DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    ALIASES mo_files
      FOR yif_abapgit_object~mo_files .
...
CLASS YCL_ABAPGIT_OBJECT_SICF IMPLEMENTATION.
...
* call to SAP standard code raise an exception
    ...
    cl_icf_tree=>if_icf_tree~change_node(
      ...
      EXCEPTIONS
        ...
        OTHERS                    = 27 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'SICF - error from change_node' ).
    ENDIF.
  ...
  METHOD find_parent.
    ...
    cl_icf_tree=>if_icf_tree~service_from_url(
      ...
      EXCEPTIONS
        ...
        OTHERS                = 6 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'SICF - error from service_from_url' ).
    ENDIF.
...
* nice, it seems that the structure should be mistreated
    ...
    cl_icf_tree=>if_icf_tree~insert_node(
      ...
      EXCEPTIONS
        ...
        OTHERS                    = 27 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |SICF - error from insert_node: { sy-subrc }| ).
    ENDIF.
  ...
  METHOD read.
    ...
    cl_icf_tree=>if_icf_tree~get_info_from_serv(
      ...
      EXCEPTIONS
        ...
        OTHERS            = 5 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'SICF - error from get_info_from_serv' ).
    ENDIF.
...
* note: this method is called dynamically from some places
    ...
    cl_icf_tree=>if_icf_tree~get_info_from_serv(
      ...
      EXCEPTIONS
        ...
        OTHERS            = 5 ).
    IF sy-subrc = 0.
      rv_hash = ycl_abapgit_hash=>sha1_raw( ycl_abapgit_convert=>string_to_xstring_utf8( lv_url ) ).
    ENDIF.
...
* convert to sorted table
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF ls_icfservice-icfparguid CO '0'.
      ...
      ycx_abapgit_exception=>raise( 'SICF - cannot delete root node, delete node manually' ).
    ENDIF.
    ...
    cl_icf_tree=>if_icf_tree~delete_node(
      ...
      EXCEPTIONS
        ...
        OTHERS                      = 12 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'SICF - error from delete_node' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    lv_exists = yif_abapgit_object~exists( ).
  ...
  METHOD yif_abapgit_object~jump.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from ABAP4_CALL_TRANSACTION, SICF' ).
    ENDIF.
