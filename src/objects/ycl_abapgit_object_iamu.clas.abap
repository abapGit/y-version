CLASS ycl_abapgit_object_iamu DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_iamu IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error from if_w3_api_mime~set_changeable| ).
    ENDIF.
    ...
    mo_mime_api->if_w3_api_object~delete(
      EXCEPTIONS
        ...
        OTHERS                = 5 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error from if_w3_api_mime~delete| ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error from if_w3_api_mime~save| ).
    ENDIF.
  ...
  METHOD load_mime_api.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from if_w3_api_mime~load' ).
    ENDIF.
  ...
  METHOD read.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error from if_w3_api_mime~get_attributes| ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error from if_w3_api_mime~get_source| ).
    ENDIF.
  ...
  METHOD save.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error from if_w3_api_mime~create_ne| ).
    ENDIF.
