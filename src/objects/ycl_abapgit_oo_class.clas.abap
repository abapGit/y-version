CLASS ycl_abapgit_oo_class DEFINITION PUBLIC INHERITING FROM ycl_abapgit_oo_base.

  PUBLIC SECTION.
    METHODS:
      yif_abapgit_oo_object_fnc~create REDEFINITION,
      yif_abapgit_oo_object_fnc~generate_locals REDEFINITION,
      yif_abapgit_oo_object_fnc~insert_text_pool REDEFINITION,
      yif_abapgit_oo_object_fnc~create_sotr REDEFINITION,
      yif_abapgit_oo_object_fnc~get_includes REDEFINITION,
      yif_abapgit_oo_object_fnc~get_class_properties REDEFINITION,
      yif_abapgit_oo_object_fnc~read_text_pool REDEFINITION,
      yif_abapgit_oo_object_fnc~read_sotr REDEFINITION,
      yif_abapgit_oo_object_fnc~delete REDEFINITION.

ENDCLASS.

CLASS ycl_abapgit_oo_class IMPLEMENTATION.
  METHOD yif_abapgit_oo_object_fnc~create.
    ...
    CALL FUNCTION 'SEO_CLASS_CREATE_COMPLETE'
      ...
      EXCEPTIONS
        ...
        OTHERS          = 7.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SEO_CLASS_CREATE_COMPLETE' ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD yif_abapgit_oo_object_fnc~generate_locals.
    CALL FUNCTION 'SEO_CLASS_GENERATE_LOCALS'
      ...
      EXCEPTIONS
        ...
        OTHERS                 = 5.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from generate_locals' ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD yif_abapgit_oo_object_fnc~insert_text_pool.
    ...
    ycl_abapgit_objects_activation=>add( iv_type = 'REPT'
                                         iv_name = lv_cp ).
  ENDMETHOD.

  METHOD yif_abapgit_oo_object_fnc~create_sotr.
    DATA: lt_sotr    TYPE yif_abapgit_definitions=>ty_sotr_tt,
    ...
    LOOP AT it_sotr ASSIGNING <ls_sotr>.
      CALL FUNCTION 'SOTR_OBJECT_GET_OBJECTS'
        ...
        EXCEPTIONS
          ...
          OTHERS           = 2.
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from SOTR_OBJECT_GET_OBJECTS' ).
      ENDIF.
      ...
      CALL FUNCTION 'SOTR_CREATE_CONCEPT'
        ...
        EXCEPTIONS
          ...
          OTHERS                        = 19.
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from SOTR_CREATE_CONCEPT' ).
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
...
*    APPEND cl_oo_classname_service=>get_cs_name( lv_class_name ) TO rt_includes.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Class { lv_class_name } not existing| ).
    ENDIF.
...
* this can happen when the SXCI object is deleted before the implementing CLAS
      RETURN.
    ELSEIF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error from SEO_CLASS_DELETE_COMPLETE: { sy-subrc }| ).
    ENDIF.
  ENDMETHOD.
