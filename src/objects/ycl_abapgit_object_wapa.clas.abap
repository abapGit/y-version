CLASS ycl_abapgit_object_wapa DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
    ...
    METHODS:
      ...
      delete_superfluous_pages
        IMPORTING
          ...
          it_remote_pages TYPE ycl_abapgit_object_wapa=>ty_pages_tt
        RAISING
          ycx_abapgit_exception.
...
CLASS ycl_abapgit_object_wapa IMPLEMENTATION.
  ...
  METHOD get_page_content.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |WAPA - error from get_page_content| ).
    ENDIF.

    CONCATENATE LINES OF lt_content INTO lv_string SEPARATED BY yif_abapgit_definitions=>c_newline RESPECTING BLANKS.

    rv_content = ycl_abapgit_convert=>string_to_xstring_utf8( lv_string ).
  ...
  METHOD to_page_content.
    ...
    lv_string = ycl_abapgit_convert=>xstring_to_string_utf8( iv_content ).

    SPLIT lv_string AT yif_abapgit_definitions=>c_newline INTO TABLE rt_content.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |WAPA - error from delete: { sy-subrc }| ).
    ENDIF.
...
* release lock
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    DATA: lo_bsp            TYPE REF TO cl_o2_api_application,
          ...
          ls_pagekey        TYPE o2pagkey,
          ls_local_page     TYPE ycl_abapgit_object_wapa=>ty_page,
    ...
    CASE sy-subrc.
      ...
      WHEN OTHERS.
        ...
        ycx_abapgit_exception=>raise( |Error { sy-subrc } from CL_O2_API_APPLICATION=>LOAD| ).
    ...
    LOOP AT lt_pages_info ASSIGNING <ls_remote_page>.
      ...
      CASE sy-subrc.
        ...
        WHEN OTHERS.
          ...
          ycx_abapgit_exception=>raise( |Error { sy-subrc } from CL_O2_API_PAGES=>LOAD| ).
      ...
      ycl_abapgit_objects_activation=>add( iv_type = 'WAPP'
                                           iv_name = lv_obj_name ).
  ...
  METHOD create_new_application.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |WAPA - error from create_new: { sy-subrc }| ).
    ENDIF.
    ...
    ycl_abapgit_objects_activation=>add_item( ls_item ).
  ...
  METHOD create_new_page.
    ...
    cl_o2_api_pages=>create_new_page(
      ...
      EXCEPTIONS
        ...
        OTHERS                = 5 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error { sy-subrc } from CL_O2_API_PAGES=>CREATE_NEW_PAGE| ).
    ENDIF.
  ...
  METHOD delete_superfluous_pages.
    ...
    LOOP AT it_local_pages ASSIGNING <ls_local_page>.
      ...
      IF sy-subrc <> 0.
        ...
        IF sy-subrc <> 0.
          ycx_abapgit_exception=>raise( |Error { sy-subrc } from CL_O2_PAGE=>DELETE_PAGE_FOR_APPLICATION| ).
        ENDIF.
