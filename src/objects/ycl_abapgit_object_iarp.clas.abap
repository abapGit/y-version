CLASS ycl_abapgit_object_iarp DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_iarp IMPLEMENTATION.
  ...
  METHOD read.
    ...
    cl_w3_api_resource=>if_w3_api_resource~load(
      ...
      EXCEPTIONS
        ...
        OTHERS              = 4 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from w3api_resource~load' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    cl_w3_api_resource=>if_w3_api_resource~load(
      ...
      EXCEPTIONS
        ...
        OTHERS              = 4 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from if_w3_api_resource~load' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~exists.
    ...
    IF sy-subrc = 1.
      rv_bool = abap_false.
    ELSEIF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from w3_api_resource~load' ).
    ELSE.
      rv_bool = abap_true.
    ENDIF.
