CLASS ycl_abapgit_object_wdyn DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS YCL_ABAPGIT_OBJECT_WDYN IMPLEMENTATION.
    ...
    IF lv_found = abap_false.
      TRY.
        ...
        CATCH cx_wdy_md_exception.
          ycx_abapgit_exception=>raise( 'error creating dummy controller' ).
      ENDTRY.
    ENDIF.
...
*   use them dynamically for downward compatibility
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from SVRS_MAKE_OBJECT_DELTA' ).
    ENDIF.
  ...
  METHOD delta_definition.
    ...
    IF lv_found = abap_false.
      TRY.
          ...
          li_component->unlock( ).
        CATCH cx_wdy_md_exception.
          ycx_abapgit_exception=>raise( 'error creating dummy component' ).
      ENDTRY.
    ENDIF.
  ...
  METHOD delta_view.
    ...
    IF lv_found = abap_false.
      TRY.
          ...
          li_view->unlock( ).
        CATCH cx_wdy_md_exception.
          ycx_abapgit_exception=>raise( 'error creating dummy view' ).
      ENDTRY.
    ENDIF.
...
*   FM exceptions
    ...
    CALL FUNCTION 'WDYC_GET_OBJECT'
      ...
      lt_fm_exception.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from WDYC_GET_OBJECT' ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'WDYC, definition not found' ).
    ENDIF.
  ...
  METHOD read_definition.
    ...
    IF sy-subrc = 1.
      RETURN.
    ELSEIF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from WDYD_GET_OBJECT' ).
    ENDIF.

    READ TABLE lt_definition INDEX 1 INTO rs_definition-definition.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'WDYD, definition not found' ).
    ENDIF.
  ...
  METHOD read_view.
    ...
    CALL FUNCTION 'WDYV_GET_OBJECT'
      ...
      EXCEPTIONS
        ...
        OTHERS                 = 2.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from WDYV_GET_OBJECT' ).
    ENDIF.
