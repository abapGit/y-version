CLASS ycl_abapgit_object_scp1 DEFINITION
  PUBLIC
...
CLASS ycl_abapgit_object_scp1 IMPLEMENTATION.
  ...
  METHOD enqueue.
    ...
    CALL FUNCTION 'SCPR_SV_ENQUEUE_BCSET'
      ...
      EXCEPTIONS
        ...
        OTHERS            = 3.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |SCP1 locking error| ).
    ENDIF.
...
* copy everything to local, the function module changes the values
    ...
    CALL FUNCTION 'SCPR_TEMPL_MN_TEMPLATE_SAVE'
      ...
      EXCEPTIONS
        ...
        OTHERS                    = 5.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |Error saving SCP1, { sy-tabix }| ).
    ENDIF.
...
* todo, this gives a popup
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |error while deleting SCP1, { sy-subrc }| ).
    ENDIF.
...
* todo, function module SCPR_PRSET_MN_BCSET_SAVE
      ycx_abapgit_exception=>raise( |todo, SCP1| ).
    ELSE.
      save( ls_scp1 ).
    ENDIF.
