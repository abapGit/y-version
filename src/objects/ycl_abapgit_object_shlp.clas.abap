CLASS ycl_abapgit_object_shlp DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
...
CLASS ycl_abapgit_object_shlp IMPLEMENTATION.
  ...
  METHOD yif_abapgit_object~delete.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from RS_DD_DELETE_OBJ, SHLP' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    CALL FUNCTION 'DDIF_SHLP_GET'
      ...
      EXCEPTIONS
        ...
        OTHERS        = 2.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DDIF_SHLP_GET' ).
    ENDIF.
  ...
  METHOD yif_abapgit_object~deserialize.
    ...
    CALL FUNCTION 'DDIF_SHLP_PUT'
      ...
      EXCEPTIONS
        ...
        OTHERS            = 6.
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from DDIF_SHLP_PUT' ).
    ENDIF.
