CLASS ycl_abapgit_object_devc DEFINITION PUBLIC
  ...
  PUBLIC SECTION.
    INTERFACES:
      yif_abapgit_object.
    ALIASES:
      mo_files FOR yif_abapgit_object~mo_files.
    METHODS:
      constructor IMPORTING is_item     TYPE yif_abapgit_definitions=>ty_item
...
CLASS ycl_abapgit_object_devc IMPLEMENTATION.
  ...
  METHOD get_package.
    IF me->yif_abapgit_object~exists( ) = abap_true.
  ...
  METHOD is_empty.
    ...
    DATA: lv_object_name TYPE tadir-obj_name,
          lt_subpackages TYPE yif_abapgit_sap_package=>ty_devclass_tt.

    lt_subpackages = ycl_abapgit_factory=>get_sap_package( iv_package_name )->list_subpackages( ).
...
*          error_in_cts_checks        = 21 downport, does not exist in 7.31
  ...
  METHOD yif_abapgit_object~serialize.
    ...
    li_package = get_package( ).
    IF li_package IS NOT BOUND.
      ycx_abapgit_exception=>raise( |Could not find package to serialize.| ).
    ENDIF.
