CLASS ycl_abapgit_object_msag DEFINITION PUBLIC INHERITING FROM ycl_abapgit_objects_super FINAL.
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      ...
      serialize_longtexts_msag
        IMPORTING it_t100 TYPE ycl_abapgit_object_msag=>tty_t100
...
CLASS ycl_abapgit_object_msag IMPLEMENTATION.
  ...
  METHOD deserialize_texts.
    ...
    LOOP AT lt_t100_texts ASSIGNING <ls_t100_text>.
      ...
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'MSAG: Table T100 modify failed' ).
      ENDIF.
    ENDLOOP.
  ...
  METHOD serialize_longtexts_msag.
    ...
    DATA: lv_object  TYPE dokhl-object,
          lt_objects TYPE STANDARD TABLE OF dokhl-object
                          WITH NON-UNIQUE DEFAULT KEY,
          lt_dokil   TYPE yif_abapgit_definitions=>tty_dokil.
...
* replaced with a copy
    ...
    IF ms_item-obj_name EQ space.
      ycx_abapgit_exception=>raise( 'Error from (copy of) RS_DELETE_MESSAGE_ID' )."blank message id
    ENDIF.

    SELECT SINGLE * FROM t100a INTO lv_t100a WHERE arbgb = ms_item-obj_name.
    IF sy-subrc NE 0.
      ycx_abapgit_exception=>raise( 'Error from (copy of) RS_DELETE_MESSAGE_ID' )."not found
    ENDIF.
    ...
    IF sy-subrc NE 0 OR lv_frozen NE space.
      ycx_abapgit_exception=>raise( 'Error from (copy of) RS_DELETE_MESSAGE_ID' )."can't access
    ENDIF.
...
* fm RPY_MESSAGE_ID_INSERT almost works, but not in older versions
    ...
    LOOP AT lt_t100 ASSIGNING <ls_t100>.
      ...
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'MSAG: Table T100U modify failed' ).
      ENDIF.
    ENDLOOP.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'MSAG: Table T100A modify failed' ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'MSAG: Table T100T modify failed' ).
    ENDIF.
