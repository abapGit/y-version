CLASS ycl_abapgit_transport DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS YCL_ABAPGIT_TRANSPORT IMPLEMENTATION.


  METHOD find_top_package.
* assumption: all objects in transport share a common super package

    DATA: lt_obj   TYPE yif_abapgit_sap_package=>ty_devclass_tt,
          lt_super TYPE yif_abapgit_sap_package=>ty_devclass_tt,
    ...
    ASSERT sy-subrc = 0.
    lt_super = ycl_abapgit_factory=>get_sap_package( <ls_tadir>-devclass )->list_superpackages( ).

    LOOP AT it_tadir ASSIGNING <ls_tadir>.
      lt_obj = ycl_abapgit_factory=>get_sap_package( <ls_tadir>-devclass )->list_superpackages( ).
...
* filter out possibilities from lt_super
    ...
    LOOP AT it_trkorr ASSIGNING <ls_trkorr>.
      CALL FUNCTION 'TR_READ_REQUEST_WITH_TASKS'
        ...
        EXCEPTIONS
          ...
          OTHERS        = 2.
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from TR_READ_REQUEST_WITH_TASKS' ).
      ENDIF.
        ...
        IF <ls_object>-pgmid = 'LIMU'.
          CALL FUNCTION 'GET_R3TR_OBJECT_FROM_LIMU_OBJ'
            ...
            EXCEPTIONS
              ...
              OTHERS         = 2.
          IF sy-subrc <> 0.
            ycx_abapgit_exception=>raise( 'error from GET_R3TR_OBJECT_FROM_LIMU_OBJ' ).
          ENDIF.
  ...
  METHOD zip.
    ...
    DATA: lt_requests TYPE trwbo_requests,
          ...
          lo_repo     TYPE REF TO ycl_abapgit_repo_offline,
          lt_trkorr   TYPE trwbo_request_headers.


    lt_trkorr = ycl_abapgit_ui_factory=>get_popups( )->popup_to_select_transports( ).
    ...
    IF lines( lt_tadir ) = 0.
      ycx_abapgit_exception=>raise( 'empty transport' ).
    ENDIF.

    lv_package = find_top_package( lt_tadir ).
    IF lv_package IS INITIAL.
      ycx_abapgit_exception=>raise( 'error finding super package' ).
    ENDIF.
    ...
    ls_data-dot_abapgit-folder_logic = ycl_abapgit_ui_factory=>get_popups( )->popup_folder_logic( ).

    CREATE OBJECT lo_repo
      EXPORTING
        is_data = ls_data.

    ycl_abapgit_zip=>export( io_repo   = lo_repo
                             it_filter = lt_tadir ).
  ENDMETHOD.
ENDCLASS.
