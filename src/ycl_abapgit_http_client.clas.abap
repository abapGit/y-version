CLASS ycl_abapgit_http_client DEFINITION PUBLIC CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS:
      ...
      close,
      set_digest
        IMPORTING io_digest TYPE REF TO ycl_abapgit_http_digest,
      send_receive_close
  ...
  PRIVATE SECTION.
    DATA: mi_client TYPE REF TO if_http_client,
          mo_digest TYPE REF TO ycl_abapgit_http_digest.

ENDCLASS.



CLASS ycl_abapgit_http_client IMPLEMENTATION.
  ...
  METHOD check_http_200.
    ...
    CASE lv_code.
      WHEN 200.
        RETURN.
      WHEN 302.
        ycx_abapgit_exception=>raise( 'HTTP redirect, check URL' ).
      WHEN 401.
        ycx_abapgit_exception=>raise( 'HTTP 401, unauthorized' ).
      WHEN 403.
        ycx_abapgit_exception=>raise( 'HTTP 403, forbidden' ).
      WHEN 404.
        ycx_abapgit_exception=>raise( 'HTTP 404, not found' ).
      WHEN 415.
        ycx_abapgit_exception=>raise( 'HTTP 415, unsupported media type' ).
      WHEN OTHERS.
        lv_text = mi_client->response->get_cdata( ).
        ycx_abapgit_exception=>raise( |HTTP error code: { lv_code }, { lv_text }| ).
    ENDCASE.
  ...
  METHOD check_smart_response.
    ...
    IF iv_expected_content_type IS NOT INITIAL.
      ...
      IF lv_content_type <> iv_expected_content_type.
        ycx_abapgit_exception=>raise( 'Wrong Content-Type sent by server - no fallback to the dumb protocol!' ).
      ENDIF.
    ENDIF.
    ...
    IF iv_content_regex IS NOT INITIAL.
      ...
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'Wrong Content sent by server' ).
      ENDIF.
    ENDIF.
...
* do not use set_cdata as it modifies the Content-Type header field
  ...
  METHOD set_headers.
    ...
    lv_value = ycl_abapgit_url=>path_name( iv_url ) &&
      '/git-' &&
      iv_service &&
      '-pack'.
    mi_client->request->set_header_field(
