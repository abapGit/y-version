CLASS ycl_abapgit_zip DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS normalize_path
      CHANGING ct_files TYPE yif_abapgit_definitions=>ty_files_tt
      RAISING  ycx_abapgit_exception.
    ...
    CLASS-METHODS encode_files
      IMPORTING it_files       TYPE yif_abapgit_definitions=>ty_files_item_tt
      RETURNING VALUE(rv_xstr) TYPE xstring
      RAISING   ycx_abapgit_exception.
...
CLASS YCL_ABAPGIT_ZIP IMPLEMENTATION.
  ...
  METHOD export.
    ...
    DATA: lo_log     TYPE REF TO ycl_abapgit_log,
          lt_zip     TYPE yif_abapgit_definitions=>ty_files_item_tt,
    ...
    IF ycl_abapgit_factory=>get_sap_package( lv_package )->exists( ) = abap_false.
      ycx_abapgit_exception=>raise( |Package { lv_package } doesn't exist| ).
    ENDIF.
    ...
    LOOP AT lt_files ASSIGNING <ls_file>.
      ...
      cl_gui_frontend_services=>gui_download(
        ...
        EXCEPTIONS
          ...
          OTHERS                    = 24 ).
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from gui_download' ).
      ENDIF.
    ENDLOOP.
  ...
  METHOD export_package.
    ...
    DATA: lo_repo   TYPE REF TO ycl_abapgit_repo_offline,
          ls_data   TYPE yif_abapgit_persistence=>ty_repo,
          li_popups TYPE REF TO yif_abapgit_popups.


    ls_data-key = 'DUMMY'.
    ls_data-dot_abapgit = ycl_abapgit_dot_abapgit=>build_default( )->get_data( ).
  ...
  METHOD filename.
    ...
    IF iv_str CA '/'.
      ...
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'Malformed path' ).
      ENDIF.
      IF ev_path <> '/'.
  ...
  METHOD file_download.
    ...
    cl_gui_frontend_services=>file_save_dialog(
      ...
      EXCEPTIONS
        ...
        OTHERS               = 4 ).                         "#EC NOTEXT
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from file_save_dialog' ).
    ENDIF.
  ...
  METHOD file_upload.
    ...
    cl_gui_frontend_services=>file_open_dialog(
      ...
      EXCEPTIONS
        ...
        OTHERS                  = 5 ).                      "#EC NOTEXT
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from file_open_dialog' ).
    ENDIF.
    IF lv_action = cl_gui_frontend_services=>action_cancel.
      ycx_abapgit_exception=>raise( 'cancelled' ).
    ENDIF.
    ...
    cl_gui_frontend_services=>gui_upload(
      ...
      EXCEPTIONS
        ...
        OTHERS                  = 19 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from gui_upload' ).
    ENDIF.
  ...
  METHOD unzip_file.
    ...
    lo_zip->load( EXPORTING
                  ...
                  EXCEPTIONS
                    ...
                    OTHERS          = 2 ).
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( 'error from zip' ).
    ENDIF.
    ...
    LOOP AT lo_zip->files ASSIGNING <ls_zipfile>.
      ...
      lo_zip->get(
        ...
        EXCEPTIONS
          ...
          OTHERS                  = 3 ).
      IF sy-subrc <> 0.
        ycx_abapgit_exception=>raise( 'error from zip get' ).
      ENDIF.
      ...
      <ls_file>-sha1 = ycl_abapgit_hash=>sha1( iv_type = yif_abapgit_definitions=>c_type-blob
