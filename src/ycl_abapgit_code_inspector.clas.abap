CLASS ycl_abapgit_code_inspector DEFINITION
  PUBLIC
  CREATE PROTECTED
...
CLASS ycl_abapgit_code_inspector IMPLEMENTATION.
  ...
  METHOD create_variant.
    ...
    IF mv_check_variant_name IS INITIAL.
      ycx_abapgit_exception=>raise( |No check variant supplied.| ).
    ENDIF.
    ...
    CASE sy-subrc.
      WHEN 1.
        ycx_abapgit_exception=>raise( |Check variant { mv_check_variant_name } doesn't exist| ).
      WHEN 2.
        ycx_abapgit_exception=>raise( |Parameter missing for check variant { mv_check_variant_name }| ).
    ENDCASE.
...
* TODO, in the future, move this method to the ABAPGIT global package class
  ...
  METHOD yif_abapgit_code_inspector~run.
  ...
  METHOD validate_check_variant.
    ...
    IF sy-subrc <> 0.
      ycx_abapgit_exception=>raise( |No valid check variant { iv_check_variant_name  }| ).
    ENDIF.

  ENDMETHOD.

  METHOD yif_abapgit_code_inspector~get_inspection.
