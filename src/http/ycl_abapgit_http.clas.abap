CLASS ycl_abapgit_http DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS:
      ...
      create_by_url
        ...
        RETURNING VALUE(ro_client) TYPE REF TO ycl_abapgit_http_client
  ...
  PRIVATE SECTION.
    CLASS-METHODS:
      ...
      acquire_login_details
        IMPORTING ii_client        TYPE REF TO if_http_client
                  io_client        TYPE REF TO ycl_abapgit_http_client
                  iv_url           TYPE string
...
CLASS ycl_abapgit_http IMPLEMENTATION.
  ...
  METHOD acquire_login_details.
    ...
    DATA: lv_default_user TYPE string,
          lv_user         TYPE string,
          lv_pass         TYPE string,
          lo_digest       TYPE REF TO ycl_abapgit_http_digest.


    lv_default_user = ycl_abapgit_persistence_user=>get_instance( )->get_repo_login( iv_url ).
    lv_user         = lv_default_user.

    ycl_abapgit_password_dialog=>popup(
      EXPORTING
        iv_repo_url     = iv_url
      CHANGING
        cv_user         = lv_user
    ...
    IF lv_user IS INITIAL.
      ycx_abapgit_exception=>raise( 'HTTP 401, unauthorized' ).
    ENDIF.

    IF lv_user <> lv_default_user.
      ycl_abapgit_persistence_user=>get_instance( )->set_repo_login(
        iv_url   = iv_url
        iv_login = lv_user ).
    ENDIF.

    " Offer two factor authentication if it is available and required
    ycl_abapgit_2fa_auth_registry=>use_2fa_if_required(
      EXPORTING
        iv_url      = iv_url
      CHANGING
        cv_username = lv_user
...
* https://en.wikipedia.org/wiki/Basic_access_authentication
  ...
  METHOD create_by_url.
    ...
    DATA: lv_uri                 TYPE string,
          ...
          lo_proxy_configuration TYPE REF TO ycl_abapgit_proxy_config,
          lv_text                TYPE string.


    CREATE OBJECT lo_proxy_configuration.

    li_client = ycl_abapgit_exit=>get_instance( )->create_http_client( iv_url ).

    IF li_client IS NOT BOUND.

      cl_http_client=>create_by_url(
        EXPORTING
          url                = ycl_abapgit_url=>host( iv_url )
          ssl_id             = 'ANONYM'
    ...
    IF lo_proxy_configuration->get_proxy_authentication( iv_url ) = abap_true.
      ycl_abapgit_proxy_auth=>run( li_client ).
    ENDIF.
    ...
    lv_uri = ycl_abapgit_url=>path_name( iv_url ) &&
    ...
    ycl_abapgit_login_manager=>load( iv_uri    = iv_url
                                     ii_client = li_client ).

    ycl_abapgit_exit=>get_instance( )->http_client( li_client ).
    ...
    IF lv_scheme <> c_scheme-digest.
      ycl_abapgit_login_manager=>save( iv_uri    = iv_url
                                       ii_client = li_client ).
    ENDIF.
...
* also see https://github.com/larshp/abapGit/issues/1432
    rv_agent = |git/2.0 (abapGit { yif_abapgit_version=>gc_abap_version })|.
  ...
  METHOD is_local_system.
    ...
    DATA: lv_host TYPE string,
          lt_list TYPE yif_abapgit_exit=>ty_icm_sinfo2_tt,
          li_exit TYPE REF TO yif_abapgit_exit.
    ...
    li_exit = ycl_abapgit_exit=>get_instance( ).
