CLASS ycl_abapgit_folder_logic DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get_instance
      RETURNING
        VALUE(ro_instance) TYPE REF TO ycl_abapgit_folder_logic .
  PROTECTED SECTION.
    METHODS get_parent
      IMPORTING
        !iv_package     TYPE devclass
...
CLASS YCL_ABAPGIT_FOLDER_LOGIC IMPLEMENTATION.
  ...
  METHOD get_parent.
    ...
    IF sy-subrc <> 0.
      r_parent = ycl_abapgit_factory=>get_sap_package( iv_package )->read_parent( ).
  ...
  METHOD package_to_path.
    ...
    ELSE.
      ...
      IF lv_parentcl IS INITIAL.
        ycx_abapgit_exception=>raise( |error, expected parent package, { iv_package }| ).
      ELSE.
        lv_folder_logic = io_dot->get_folder_logic( ).
        CASE lv_folder_logic.
          WHEN yif_abapgit_dot_abapgit=>c_folder_logic-full.
            lv_len = 0.
            IF iv_package(1) = '$'.
              lv_len = 1.
            ENDIF.
          WHEN yif_abapgit_dot_abapgit=>c_folder_logic-prefix.
...
* similarily with online projects. Alternatively change to FULL folder logic
            ...
            ENDIF.
          WHEN OTHERS.
            ycx_abapgit_exception=>raise( |Invalid folder logic: { lv_folder_logic }| ).
        ENDCASE.

        lv_path = iv_package+lv_len.
        IF strlen( lv_path ) = 0.
          ycx_abapgit_exception=>raise( |Folder logic: length = 0, parent: {
...
* treat as not existing locally
    ...
    WHILE lv_path CA '/'.
      ...
      IF ycl_abapgit_factory=>get_sap_package( rv_package )->exists( ) = abap_false AND
          iv_create_if_not_exists = abap_true.

        ycl_abapgit_factory=>get_sap_package( lv_parent )->create_child( rv_package ).
      ENDIF.
